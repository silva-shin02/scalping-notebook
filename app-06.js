...
function _elEpPosSectionV2(recs, aiOf) {
  ...
  var tbl = React.createElement(_HScrollBox, { style: { marginTop: 8 } },
    React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11 } },
      React.createElement("thead", null, React.createElement("tr", { style: { background: "transparent" } },
        _thE("EP位置"), _thE("件数"), _thE("OS値"), _thE("E後の勝率"), _thE("想定損益"), _thE("見切り率"), _thE("損切り率"))),
  ...
  if (ok) {
    ...
    var hbest = null;
    // 読み取りの基準を表と同じ想定損益(h2)へ統一（旧=H1基準で表とズレていた・監査F4 2026-07-12）
    ["ep0", "ep1", "ep2"].forEach(function(k, i) { var o = st[k]; if (o.h2Cnt && (hbest == null || o.h2 / o.h2Cnt > hbest.v)) hbest = { v: o.h2 / o.h2Cnt, i: i }; });
    if (hbest) items.push(React.createElement("span", null, "1件あたりの想定損益が最も良いのは", _elInsightEmV2(["EP=OS1", "EP=OS2", "EP=OS3"][hbest.i]), "（平均", _elInsightEmV2(Math.round(hbest.v).toLocaleString() + "円"), "）。"));
    // 勝率の傾向
    var wbest = null;
...
function _elTimeOfDaySectionV2(recs, aiOf) {
  ...
  return React.createElement("div", null, bar, tbl, items.length ? _elInsightBoxV2(items, { note: "OS値=寄り足の高値（水準線比）の中央値（主）と平均（副）を併記（OS値は右偏なので典型値は中央値）／E到達率=3本以内にα到達（×見送り含む）／E後の勝率=エントリー（E成立）後にEP損益が利益だった割合（敗率・未達率はE到達率の裏返しなので省略）／損切り率=想定orH1orH2で損切りラインに触れ、その足の終値で撤退した結果が損だったもの（2026-07-27 終値撤退方式・_elIsStopFinal。触れても終値が利益側なら損切りに数えない）。EP/H1損益はE成立（エントリーできた）分のみの平均＋合計。未達（α未到達）・×見送りは母数に含めない。採用α基準。" }) : null);
}
...
// 累積損益（記録順）: 想定損益/実現の累積線。寄与は合計行と同一基準（手じまい=_elHoldFinalParts.main）。2026-07-09 EP/H1系列を廃止＝想定損益に集約。
function _elCumPnlSectionV2(props) {
  ...
  var chart = _elLineChartV2([
    { label: "想定損益", color: "#D97706", pts: pH2 },
...
// α感応度カーブ: α=0〜20円の各値で全記録を再計算した想定損益（＝旧H2・（）外）の合計。読み取りに手じまい最大α。2026-07-09 EP/H1系列を廃止＝想定損益に集約。
...
// 推奨α＝理想α−_EL_ALPHA_OFFSET（2026-07-13 ユーザー方針・2026-07-22 1→2・2026-08-02 2→1・2026-08-02k 1→0＝マージン撤廃）:
//   かつては「理想αちょうどに指値すると指値同値でギリギリ約定しない」ぶんを一律で下げていた。2026-08-02kで0にした理由は2つあり、
//   ①2026-08-02eで同値を全掃引の評価に0円算入したので刺さりにくさは各αの平均/Σに価格として入っている＝一律マージンは二重計上、
//   ②オフセット>0だと「最良と判定したα(理想)」と「実際に置くα(推奨)」がズレる、だった。
//   ⚠️2026-08-10 の全列除外（fillEqSkip）で①の前提は消えた＝平均/Σは「実際に約定した取引だけ」の値になり、刺さりにくさは同値列の件数にしか表れない。
//   それでも0のままなのは②が残るため。マージンを復活させるかどうかは数字を見てからのユーザー判断＝ここでは値を変えていない。
//   理想α=Σ想定損益ベースの★／推奨α=実際に置く値（フォーム/EPナビ/本日採用α/シミュへ流れる）。max(0,…)で負にしない。
var _EL_ALPHA_OFFSET = 0;   // 推奨α＝理想α−この値（0未満にしない）。2026-07-22 1→2 → 2026-08-02 2→1 → 2026-08-02k 1→0（ユーザー決定）。2026-08-10 の同値・全列除外でも据え置き（上のコメント②が理由）。基本α・応用α共通。1以上に戻せば表示の「−N円」注記も自動で復活する。
// 表示用: 「推奨＝理想−N円」注記の断片。オフセット0（＝推奨と理想が一致）なら空文字を返し注記そのものを出さない 2026-08-02k。unitは"円"等の後置文字（省略可）。
function _elAlphaOffTxt(unit) { return _EL_ALPHA_OFFSET > 0 ? ("−" + _EL_ALPHA_OFFSET + (unit || "")) : ""; }
// 理想α選定 2026-08-02k ユーザー方針【平均重視→累計重視へ変更】: 前提ゲート通過候補のうち、Σ想定損益（累計・h2Sum）が最大のα＝理想。同点は平均（1件あたり・avgH2）が大きい方→値が小さい方。
//   旧＝平均最大(2026-07-15f)／その前＝順位和(2026-07-15e)。基本α・応用α・追加α・浮き足%・RN加算で共通（全スイープ統一・ユーザー決定）。
// Σ最大で最良候補を返す 2026-08-02k: cands（ゲート通過済み）のうち累計(sigOf)が最大の候補を返す。同点は平均(avgOf)大→値(aOf)小。
//   Σと平均は連動しない（未到達はh2Cntから落ちる＝高αほど件数が減る）ので、Σ主軸の理想αは平均主軸より低め＝件数を取る方向に出る。関数名_elBordaBestは互換のため据え置き。
function _elBordaBest(cands, sigOf, avgOf, aOf) { if (!cands || !cands.length) return null; var A = aOf || function() { return 0; }; var S = sigOf || function() { return 0; }; var V = avgOf || function() { return 0; }; return cands.slice().sort(function(x, y) { return (S(y) - S(x)) || (V(y) - V(x)) || (A(x) - A(y)); })[0]; }
...
function _ElAnaReachCtl(props) {
  ...
  return React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 5, background: "#F0F9FF", border: "1px solid #BAE6FD", borderRadius: 7, padding: "3px 9px", whiteSpace: "nowrap" } },
    ...
    React.createElement("span", { style: { fontSize: 8.5, color: "#0369A1" } }, "基本αの理想＝この到達率以上・頻度" + _EL_FREQ_MAX + "未満・黒字を満たすαのうちΣ想定損益（累計）が最大のα／推奨＝理想" + _elAlphaOffTxt("円") + "・既定" + _EL_ANA_REACH_DEF + "%（満たすαが無ければ" + _EL_ANA_REACH_FLOOR2 + "%まで引き下げて参考=青★）"));
}
...
function _elBaseAlphaEval(recs, aiOf, a) {
  ...
  (recs || []).forEach(function(r) {
    ...
    if (determinable) {
      scN++;
      if (epStop || h1Stop) stopH1N++;     // EP足 or H1足が損切りラインに【接触】＝スコア用の stopRate。2026-07-29追記:
      //   これは接触基準で、同じ関数の 1553行 stopN（_elIsStopFinal＝触れて且つ終値が損）とは別定義。
      //   スコアは「H1までで測る」設計（1554）なので接触基準のまま据え置く。表示名は「損切り率(H1接触)」に統一済み。
...
// 想定損益(H2)ベースの反実仮想エバリュエータ 2026-07-12: alphaOf(r)→そのレコードの総合α（null=母数外）。_elAlphaEvalByFn(H1版)の想定損益版＝到達(entered)/E成立(decided)/損切り率/利確率(想定損益>0)/試算損益(Σ想定損益h2Sum)/平均(avgH2)/スコアを返す。損益は取引・銘柄別記録と同一基準(_elHoldFinalParts.main=（）外想定損益)。損切り率・利確率の分母＝E成立(_elDynResult ok/ng/draw)。スコア＝0.7×(1−損切り率)+0.3×利確率。浮き足加算率スイープ(_elUkiPctSweep)で使用。
// fillEqSkip（2026-08-02e「0円算入」で導入 → 2026-08-10 ユーザー決定で**全列から除外**へ変更）: 指値同値（掃引αがOS1〜3高値の
//   最大とちょうど一致＝予定EPに触れただけで上抜けていない）を「そのαなら指値が刺さらず取引していない」として扱う。
//   到達(entered)・E成立(decided)・損切り/利確に加えて**想定損益(平均/中央/Σ)・勝ち/負け平均からも外す**
//   ＝どの列の母数にも入れない。件数だけ fillEqN に数えて「同値」列に出す。
//   旧仕様（2026-08-02e〜2026-08-09）は想定損益だけ0円で母数(h2Cnt/h2Vals)に残していたが、
//   「平均/中央/Σには入るが勝ち/負け平均には入らない」と列で扱いが割れて読みにくい、というユーザー判断で全列除外に統一。
//   数値の影響: Σ想定損益は不変（0円を足していただけ）／平均は分母が fillEqN 件減るぶん**上がる**／中央値は0が抜けるぶん動く。
//   ★選定の第1キーはΣなので理想・推奨は基本的に不変（同点時の第2キー＝平均だけ入れ替わりうる）。
//   未指定(falsy)なら除外自体をしない＝同値も約定扱い（従来どおり）。
function _elH2EvalByFn(recs, aiOf, alphaOf, fillEqSkip) {
  var n = 0, entered = 0, decided = 0, stopN = 0, takeN = 0, h2Sum = 0, h2Cnt = 0, fillEqN = 0;
  // 2026-07-16e 追加: medH2(中央値)・avgWin/avgLoss(勝ち/負け平均)を既存1パス内で収集＝プール再走査なし（α毎のsortが1回増えるだけ）。
  // 母数は平均/Σと同じ h2Cnt（_elHoldFinalParts.main が非null＝想定損益が確定した件数）＝decided と一致しないことがある（EP×見送り・H2未達で main=null）。
  // 2026-08-10 以降は同値も h2Cnt に入らない＝**h2Cnt === takeN + lossN が常に成立**（旧: 同値ぶん h2Cnt のほうが多かった）。
  ...
  (recs || []).forEach(function(r) {
    ...
    var c = _aiR.cutLine;
    n++;
    // 指値同値＝そのαでは約定していない扱い（到達/E成立/損切り/利確/想定損益＝どの列の母数にも入れない）2026-08-10
    // 件数だけ fillEqN に数えて「同値」列に出す。旧2026-08-02eは h2Cnt++/h2Vals.push(0) で想定損益にだけ0円算入していたが、
    //   列によって同値の入る/入らないが割れるのが分かりにくいというユーザー判断で全列除外へ統一（h2Cnt＝takeN+lossN が常に成立するようになる）。
    if (fillEqSkip && _elFillEqAt(s, r.item, a, _aiR.alpha)) { fillEqN++; return; }
    var rr = _epResolve(s, a);
  ...
  return { n: n, entered: entered, eRate: n > 0 ? entered / n : 0, decided: decided, stopN: stopN, stopRate: stopRate, takeN: takeN, takeRate: takeRate, h2Sum: h2Cnt ? h2Sum : null, h2Cnt: h2Cnt, avgH2: h2Cnt ? (h2Sum / h2Cnt) : null, score: score, medH2: medH2, avgWin: takeN ? (winSum / takeN) : null, avgLoss: lossN ? (lossSum / lossN) : null, fillEqN: fillEqN };   // fillEqN=指値同値として全列の母数から除外した件数（fillEqSkip未指定なら常に0）2026-08-10（旧: 0円算入した件数）
}
...
// 2026-08-02e 表示専用から**評価へ昇格**（ユーザー決定）: α詳細表2つ（基本α・応用α）では同値を「そのαなら取引していない」として扱う。
// 2026-08-10 その扱いを**全列除外**に統一（ユーザー決定）＝到達(entered)・E成立(decided)・損切り/利確に加えて想定損益(平均/中央/Σ)・
//   勝ち/負け平均の母数からも外す＝_elH2EvalByFnの第4引数fillEqSkip。旧2026-08-02e〜09は想定損益だけ0円算入だった。
// 現在は第4引数trueの呼び出し（α詳細表2つ・RNボード①②③・浮き足%スイープ・RN加算値スイープ）すべてがこの全列除外で揃っている。
// 単一記録がその掃引αで「指値同値」か。_elH2EvalByFn(fillEqSkip)と同値列の単一源＝二重定義しない。
function _elFillEqAt(s, item, a, adoptA) {
...
function _elRnBoardV2(recs, aiOf, holiSet) {
  ...
  // 2026-08-02h 指値同値の扱いをα詳細表と揃える（第4引数true）。RNはEPをキリ番ちょうどに置く運用なので、
  // 「高値がキリ番に触れただけで刺さらなかった」＝同値がとりわけ効く。2026-08-10 以降は全列除外（旧は損益だけ0円算入）。
  ...
  _gated.sort(function(x, y) { return (x.e.h2Sum - y.e.h2Sum) || (x.e.avgH2 - y.e.avgH2); });   // 2026-08-02k 累計重視へ（Σ最大→同点は平均大）＝_elBordaBestと同じ並び。昇順ソートの末尾が★
  ...
  var _pnlAt = function(r, a) {
    ...
    // 呼び出し側はnullを「そのαでは建っていない」として cfOnly/realOnly に振り分け、差額は (pr||0)-(pc||0) で0円算入になる＝②寄与の内訳は「差額を見る表」なので0円扱いのまま（①③の全列除外とは別の話）。
  ...
  var valRows = Object.keys(byVal).map(Number).sort(function(a, b) { return a - b; }).map(function(v) {
    var g = byVal[v];
    var e = _elH2EvalByFn(g, aiOf, _aReal, true);   // 2026-08-02h 同値は全列除外（①スイープと同基準）2026-08-10
  ...
  return React.createElement("div", null,
    ...
    React.createElement("div", { style: { fontSize: 9.5, color: "#94A3B8", marginTop: 4 } }, "上（手前）ほど早く入れて到達が増え、下（超え）ほど1件あたりが良くなるトレードオフ。★＝E成立" + _EL_BASE_MIN_N + "件以上かつΣ黒字の位置でΣ想定損益（累計）が最大（該当なしなら★なし）。RN無しが★なら「そもそもまたぎ不要」のサイン。"),
...
function _elRnThresholdBoardV2(recs, aiOf, holiSet, tier, save) {   // save 2026-08-17e: ★行の「このTを採用」で custom.rnThreshold を書くため
  ...
  for (_t = 0; _t <= _EL_RN_T_MAX; _t++) {
    ...
    rows.push({ t: _t, af: _f, hit: _hit, e: _elH2EvalByFn(pool, aiOf, _f, true) });   // 2026-08-02h 同値は全列除外（①EP位置スイープ・α詳細表と同基準）2026-08-10
  }
  // ★＝①EP位置スイープと同じ軽いゲート（E成立≥_EL_BASE_MIN_N かつ Σ黒字のうちΣ想定損益（累計）最大 2026-08-02k）。推奨基本αの4条件フルゲートは使わない
  ...
  distKeys.filter(function(d) { return d <= _EL_RN_T_MAX; }).forEach(function(d) {
    var g = byDist[d];
    var eOn = _elH2EvalByFn(g, aiOf, _alphaAtT(d), true), eOff = _elH2EvalByFn(g, aiOf, _alphaAtT(0), true);   // 2026-08-02h 同値は全列除外（主表と同基準＝相互検算が崩れないよう両側そろえる）2026-08-10
  ...
  return React.createElement("div", null,
    ...
    React.createElement("div", { style: { fontSize: 9.5, color: "#94A3B8", marginTop: 4, lineHeight: 1.5 } },
      "該当＝そのTで〇になる記録数（Tが上がるほど積み上がる）。★＝E成立" + _EL_BASE_MIN_N + "件以上かつΣ黒字のTでΣ想定損益（累計）が最大（該当なしなら★なし）。T=0が★なら「そもそもRN加算不要」のサイン。RNの間隔は50円なのでTを1上げても該当は全体の約2%ずつしか増えない＝Σの差が小さく見えるため、右端の「T=0比」と下の距離別で判断する。"),
...
// fillEqAiOf（2026-08-02f・任意）: 渡すと指値同値の記録を到達から外す＝_elH2EvalByFn(fillEqSkip=true)と母数の考え方を揃える。
//   引数の有無で opt-in（未指定＝従来どおり同値も到達扱い）＝浮き足%/RN加算/KPI早見など適用範囲外の呼び出しは影響を受けない。
//   同値だけの日はどの記録も到達しないのでその日付キーが立たない＝頻度の分子から正しく落ちる。
function _elEnteredDays(recs, alphaOf, fillEqAiOf) {
  var d = {};
  (recs || []).forEach(function(r) {
    var s = r && r.signal; if (!s || !r.date) return;
    var a = alphaOf(r); if (a == null) return;
    if (fillEqAiOf && _elFillEqAt(s, r.item, a, fillEqAiOf(r).alpha)) return;
    var rr = _epResolve(s, a);
...
// fillEqAiOf（2026-08-02f・任意）: _elEnteredDaysと同じopt-in＝指値同値の記録を到達から外す。
function _elEnteredCells(recs, alphaOf, fillEqAiOf) {
  var d = {};
  (recs || []).forEach(function(r) {
...
// 旧・合成スコア方式の選定（〜2026-07-12の_elBaseAlphaPick本体）。2026-07-13に★基準を「条件式＋Σ想定損益」へ刷新した際、
...
function _elBaseAlphaPickScore(recs, aiOf) {
  ...
  // 【2026-06-26 A】候補に「試算損益(累計ΣH1)>0」を追加＝赤字αは推奨しない。スコアは率(損切り回避/H1勝率)だけで金額を見ないため、損切りに当たらず小負けを重ねるαが上位に来るのを防ぐ。黒字αが皆無なら下のフォールバック(件数最大・参考)へ。
...
// ★選定【2026-07-13 到達率ベースに全面刷新・ユーザー承認】: 母数=素の記録（追加α〇/浮き足〇/RN〇除外・従来どおり）。各α(0〜20円)を想定損益(手じまい)基準 _elH2EvalByFn で評価し、
// 「黒字(Σ想定損益>0)かつ 到達率≥下限(_elAnaReachCur%・既定70・10刻み調整可)かつ 頻度<_EL_FREQ_MAX」のαのうち【Σ想定損益（累計）が最大のα】を理想α（2026-08-02k 累計重視へ変更・同点は平均大→低α。旧＝平均最大2026-07-15f／その前＝最も高いα）。全滅時は到達率下限を引き下げて緩和(na)。
// 推奨α＝max(0,理想−_EL_ALPHA_OFFSET)。オフセットは2026-08-02kで0＝推奨と理想は一致（理想と実際に置く値をズラさない方針。同値は2026-08-10に全列の母数から除外＝件数は「同値」列で読む）。返り値.alpha＝推奨・消費者へ流れる／.idealAlpha＝理想。stats・★は推奨αのもの。
...
function _elBaseAlphaPick(recs, aiOf, spanOverride, poolAll) {   // spanOverride: 頻度ゲートの分母（活動営業日span）を外から指定＝株価帯コンテキストで_pbBandBizDays（その帯だった営業日）を渡す。未指定は従来どおり記録スパン 2026-07-22j
  ...
  var full = _EL_BASE_ALPHAS_FULL.map(function(a) { var e = _elH2EvalByFn(recs, aiOf, function() { return a; }, true); e.a = a; return e; });   // 手じまい基準・0〜20（★選定はこちら）。第4引数true=指値同値を全列の母数から除外（2026-08-10・旧は0円算入）
  ...
  var _freqOk = function(a) { if (!(_fspan > 0)) return true; var ed = (spanOverride != null) ? _elEnteredCells(recs, function() { return a; }, aiOf) : _elEnteredDays(recs, function() { return a; }, aiOf); return ed > 0 && (_fspan / ed) < _EL_FREQ_MAX; };   // 帯基準(spanOverride)時は分母も(銘柄×日)セルで単位一致 2026-07-22k。第3引数aiOf=指値同値を到達から外す 2026-08-02f（★のE成立/到達率と同じ母数観に揃える）
  ...
  // 理想α（idealE.a）から推奨α＝max(0,理想−_EL_ALPHA_OFFSET)を作る。2026-08-02kでオフセット0＝推奨は理想と同じα（過去に1〜2円のフィルマージンを引いていた名残の経路は温存＝定数を戻せば復活）。返り値.alpha=推奨（消費者へ）／.idealAlpha=理想／stats・★は推奨αのもの。
  ...
  if (okCands.length) {
    var _best = _elBordaBest(okCands, function(e) { return e.h2Sum; }, function(e) { return e.avgH2; }, function(e) { return e.a; });   // Σ想定損益（累計）最大 2026-08-02k（旧＝平均最大）
    return _finish(_best, "ok");   // 理想＝Σ最大／推奨＝理想−_EL_ALPHA_OFFSET（指値フィルマージン）
  }
  ...
  if (_rf2 < reachFloor) {
    ...
    if (okLo.length) {
      reachFloor = _rf2;
      var _bestLo = _elBordaBest(okLo, function(e) { return e.h2Sum; }, function(e) { return e.avgH2; }, function(e) { return e.a; });   // Σ想定損益（累計）最大（50%引き下げ）
...
// ===== 推奨応用α（応用〇局面）＝基本αと同じ到達率ベース＋理想−_EL_ALPHA_OFFSET（＝0・指値同値マージンは2026-08-02kで撤廃 1→2→1→0）＋「基本αより大きく」クランプ（2026-07-13 ユーザー承認）=====
// 母数=応用〇（呼び出し側で浮き足〇/RN〇除外を渡す）。各α0〜20円を前提損切り値で評価し「黒字かつ到達率≥下限かつ頻度<_EL_FREQ_MAX」のうちΣ想定損益（累計）が最大のα＝理想（2026-08-02k 累計重視・基本αと同基準）／推奨＝max(0,理想−_EL_ALPHA_OFFSET)。
...
function _elSpecialAlphaPick(pool, aiOf, minIdeal, spanOverride) {   // spanOverride: 頻度ゲートの分母（帯基準_pbBandBizDays）。未指定は記録スパン 2026-07-22j
  ...
  var sweep = _EL_TOTAL_ALPHAS.map(function(a) { var e = _elH2EvalByFn(pool, ai, function() { return a; }, true); e.a = a; return e; });   // 第4引数true=指値同値を全列の母数から除外（2026-08-10・旧は0円算入）＝この表の表示値も理想/推奨もこのsweepが単一源
  ...
  var _freqOk = function(a) { if (!(_fspan > 0)) return true; var ed = (spanOverride != null) ? _elEnteredCells(pool, function() { return a; }, ai) : _elEnteredDays(pool, function() { return a; }, ai); return ed > 0 && (_fspan / ed) < _EL_FREQ_MAX; };   // 帯基準(spanOverride)時は分母も(銘柄×日)セルで単位一致 2026-07-22k。第3引数ai=指値同値を到達から外す 2026-08-02f
  ...
  if (okCands.length) {
    var _best = _elBordaBest(okCands, function(e) { return e.h2Sum; }, function(e) { return e.avgH2; }, function(e) { return e.a; });   // Σ想定損益（累計）最大 2026-08-02k（旧＝平均最大）
    return _finish(_best, "ok");   // 理想＝Σ最大／推奨＝理想−_EL_ALPHA_OFFSET（クランプは_finishで温存）
  }
...
function _elTotalAlphaSectionV2(recs, aiOf, holiSet, onPick, curSel, spanOverride) {   // spanOverride: 頻度列＋★頻度ゲートの分母を帯基準に（株価帯コンテキスト）2026-07-22j
  ...
  var concl = React.createElement("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "baseline", gap: "2px 10px", background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 8, padding: "8px 12px" } },   // 応用αは茶基調（nominでも警告琥珀にせず茶系のまま）2026-07-14f
    ...
    _nomin ? null : React.createElement("span", { style: { fontSize: 11, color: "#555" } },
      "平均想定損益 ", React.createElement("b", { style: { color: _elPnlColor(pick.avgH2) } }, pick.avgH2 != null ? _elPnlFmt(Math.round(pick.avgH2)) : "—"),
  ...
  var _aiAna = _elAnaAiOf(aiOf);   // 前提損切り値のラッパ。旧はH1参考列の評価用（2026-07-15f）、現在は頻度列の同値除外(_elEnteredDays第3引数)で使う 2026-08-02g
  // 2026-08-02g スコア列を最終基準(e.score)へ切替えたため、旧H1評価の並走(_h1ByA=_elBaseAlphaEvalの全α総当たり)は不要になったので撤去。
  var rows = pick.sweep.filter(function(e) { return e.entered > 0 || e.a === a || e.a === ideal; }).map(function(e) {
    ...
    var _ed = (spanOverride != null) ? _elEnteredCells(pool, (function(_a) { return function() { return _a; }; })(e.a), _aiAna) : _elEnteredDays(pool, (function(_a) { return function() { return _a; }; })(e.a), _aiAna);   // 帯基準時は分母も(銘柄×日)セルで単位一致（ピルと同じ）2026-07-22k。第3引数=指値同値を到達から外す 2026-08-02f（★の頻度ゲートと同じ値になる）
    ...
    if (!(e.h2Sum != null && e.h2Sum > 0)) _ng.push(["赤", "Σ想定損益が赤字（または損益なし）"]);
    var pass = !_ng.length;
    ...
    var _cells = [
      ...
      _elv2Td(_elFillEqCell(e.fillEqN)),   // 同値列: 主スイープ(_elH2EvalByFn fillEqSkip=true)が数えた件数をそのまま出す＝表示と★選定で二重計算しない 2026-08-02e
      _elv2Td(_elFreqCell(_span, _ed)),
      _elv2Td(_elCntRateCell(e.takeN, e.takeRate == null ? "—" : _elPctCell(e.takeRate))),   // 2026-08-02m 利確＝件数＋率の横並び
      _elv2Td(_elCntRateCell(e.stopN, e.stopRate == null ? "—" : _elStopRateCell(e.stopRate))),   // 2026-08-02m 損切り＝件数＋率の横並び
      _elv2Td(_elH2AmtCell(e)),
      _elv2Td(_elH2WinLossCell(e)),
      _elv2Td(_elScoreCell(e.score))   // 2026-08-02g 最終基準へ: eは_elH2EvalByFn＝0.7×(1−損切り率(最終))+0.3×利確率(最終)。旧は_elBaseAlphaEvalのH1基準（H1まで・ラインに触れたら損切り）で同じ行の他の列と土俵が違っていた
    ];
    ...
    // 2026-08-02m 「未達」列を撤去（ユーザー要望）。落ちたゲートは行のtitle＝ホバーのツールチップへ移設＝淡色の理由は引き続き辿れる（_ngは淡色判定passにも使うので算出はそのまま）。
    return React.createElement.apply(null, ["tr", { key: e.a, title: (_ng.length ? ("未達: " + _ng.map(function(g) { return g[1]; }).join(" / ")) : "★の全最低条件を満たす"), onClick: onPick ? function() { onPick(e.a); } : null, style: { background: on ? "#FFF7ED" : (_isCur ? "#FEF3C7" : "transparent"), opacity: pass ? 1 : 0.65, cursor: onPick ? "pointer" : "default" } }].concat(_cells));
  });
  var insight = _nomin ? null : _elInsightBoxV2([
    React.createElement("span", null, "応用〇で採用する独立α値の", _elInsightEmV2("理想は応用α " + ideal + "円"), "、", _elInsightEmV2("推奨は " + a + "円" + (_elAlphaOffTxt("") ? "（理想" + _elAlphaOffTxt("") + "）" : "")), "（Σ想定損益 ", _elInsightEmV2(pick.h2Sum != null ? _elPnlFmt(Math.round(pick.h2Sum)) : "—"), "・平均想定損益 ", _elInsightEmV2(pick.avgH2 != null ? _elPnlFmt(Math.round(pick.avgH2)) : "—"), "・損切り率(最終) ",_elInsightEmV2(_pctS(pick.stopRate)), "・E成立 ", _elInsightEmV2((pick.decided || 0) + "件"), "）。"),
    React.createElement("span", { style: { color: "#64748B" } }, "応用αは基本αより大きくクランプ。通常局面の推奨基本αは①基本αゾーン。")
  ], { note: "母数＝応用〇（浮き足〇・RN〇除外）。各応用α0〜20円を前提損切り値" + _elAnaCutCur + "円で評価。理想＝黒字・到達率" + reachP + "%以上・損切り率(最終)≤" + Math.round(_EL_BASE_MAX_STOPRATE * 100) + "%・頻度" + _EL_FREQ_MAX + "未満を満たすαのうち、Σ想定損益（累計）が最大のα（基本αより大きくクランプ）／推奨＝理想" + _elAlphaOffTxt("円") + "。同点は平均（1件あたり）が大きい方→低α。フォーム/EPナビの推奨応用αと同じ算出（銘柄全体母数）。中央値が平均から大きく下なら、その平均は少数の大勝ちで作られている（＝毎回は取れない）。淡色の行は★の最低条件のどれかを満たさない行で、どの条件で落ちたかは行にカーソルを合わせると出る（2026-08-02mに「未達」列から行ツールチップへ移設）。利確・損切りの各列は「件数 率」の並び（分母＝E成立）。想定損益(平均/中央/Σ)の母数＝想定損益が確定した件数でE成立とはズレることがある。**同値はどの列の母数にも入れない**＝到達率・E成立・利確・損切り・想定損益(平均/中央/Σ)・勝ち/負け平均のすべてから外す（2026-08-10 ユーザー決定。旧2026-08-02eは想定損益にだけ0円で算入していたので、平均・中央値だけ同値ぶん分母が多かった）。勝ち/負けの境界は利確率と同じ（プラス＝勝ち／0円のトントンは負け側）。スコア＝旧H1基準の参考値［0.7×(1−H1損切り率)+0.3×H1勝率］＝★選定には不使用（H1勝率・平均H1損益の列は想定損益と重複のため2026-07-16に削除・列構成は基本α詳細表と同一）。同値列＝そのαがOS1〜3高値の最大とちょうど一致した件数（＝予定EPに触れただけで一度も上抜けていない＝指値が刺さらなかった可能性・実際に約定済みの記録は除く）。**同値は「そのαなら取引していない」として全列の母数から外す**（2026-08-10）。Σ想定損益は旧仕様（0円算入）と同額・平均は分母が同値ぶん減るので上がる。したがって平均想定損益は「そのαで実際に約定した1件あたりの実力」であって、刺さらなかった機会のぶんは価格に入っていない＝その頻度は同値列の件数のほうで読む。理想・推奨はΣ最大で選ぶので同値の扱いを変えても基本的に動かない（同点時の第2キー＝平均だけ入れ替わりうる・2026-08-02k 選定基準はΣ最大＝累計重視）。頻度列（到達実日数）からも同値を外しているので、同値だけの日は「到達した日」に数えない＝★の頻度ゲートと同じ値。" });
  return React.createElement("div", null,
    concl,
    ...
    // 2026-08-10 ユーザー決定で「想定損益に0円で算入」→**全列から除外**へ。この表は _elH2EvalByFn(fillEqSkip=true) で回っており（app-06:2132）、
    //   同値の記録は h2Cnt/h2Vals にも入らない（app-06:1675）＝到達率・E成立・利確・損切り・想定損益(平均/中央/Σ)・勝ち/負け平均のどれにも入らない。
    //   Σは旧仕様と同額（0円を足していただけ）／平均は分母が減って上がる。件数は「同値」列にだけ出る。
    _lbl("前提損切り値" + _elAnaCutCur + "円で評価／指値同値はどの列の母数にも入れない（そのαでは取引していない扱い・件数は「同値」列のみ）"),
    _elv2Table(["応用α", "E成立", "到達率", "同値", "頻度", "利確", "損切り", "想定損益(平均/中央/Σ)", "勝ち/負け平均", "スコア"].concat(onPick ? ["選択"] : []), rows),   // 2026-08-02m 「未達」撤去・利確/損切りは件数＋率
    insight);
}
...
function _elAddAlphaRecoScore(recs, aiOf, baseAlpha) {
  ...
  // 黒字(試算損益>0)かつ標本フロア scN≥_EL_BASE_MIN_N を満たす加算（黒字は全段共通の必須条件＝赤字の上乗せは推奨しない・1〜2件の偶然も除外）。evalsはadd昇順なのでこの並びも昇順。
...
// 「各記録日の推奨基本α(recoFn(r.date))＋加算X円で入っていたら手仕舞い(想定損益)でどうだったか」をX=0〜_EL_ADD_SWEEP_MAX(10円)でスイープ。
// X=0円＝足さない（その日の推奨基本αのまま）＝基準行。選定＝追加α条件式（到達率≥50%[_EL_BASE_MIN_ERATE]・損切り率(手じまい)≤40%[_EL_BASE_MAX_STOPRATE]・E成立≥件数条件・黒字）の中で平均想定損益（1件あたり）最大（同点はE成立多→小さいX）。
...
function _elAddAlphaPickDate(pool, aiOf, recoFn, baseAlpha) {
  ...
  var _sigOf = function(row) { return row.h2.h2Sum; }, _avgOf = function(row) { return row.h2.avgH2; }, _xOf = function(row) { return row.X; };   // Σ想定損益（累計）最大 2026-08-02k（旧＝平均最大2026-07-15f・その前＝順位和）
...
function _elCutEval(recs, aiOf, cut) {
  ...
  (recs || []).forEach(function(r) {
    ...
    // 2026-07-25f H1基準→**想定損益（手じまいまで）基準**へ（ユーザー指示）: 旧は_elDynHold（H1まで保有）＋EP/H1足の損切り判定で、
    //   アプリの主指標（想定損益）と推奨損切りの選定基準が食い違っていた。母数ゲートも_elDynResultの決着判定へ統一（旧・並走pickと同型）。
...
// 「利確」「損切り」セル（2026-08-02m ユーザー要望「利確件数・利確率が横並びで表示されるように」）: 件数を薄字で左に添え、右に既存の率セル（色分けはそのまま）を並べる。
// n=件数（takeN/stopN・null可）／rateNode=_elPctCell(利確率)や_elStopRateCell(損切り率)が返すノード。率がnullの行は"—"をそのまま渡す。
function _elCntRateCell(n, rateNode) {
  return React.createElement("span", { style: { display: "inline-flex", alignItems: "baseline", justifyContent: "center", gap: 4, whiteSpace: "nowrap" } },
    React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#94A3B8" } }, (n != null ? n : 0) + "件"),
    rateNode);
}
// 【未配線・2026-08-02m 明記】「未達」列はユーザー要望で3表とも撤去したため、この関数はどこからも呼ばれていない（定義のみ）。
//   落ちたゲートの内訳は各行の title（行ホバーのツールチップ）に移したので情報自体は失われていない。列を戻すなら各表の _cells 末尾に _elv2Td(_elGateBadges(_ng)) とヘッダ"未達"を復活させる。
...
// 「想定損益(平均/中央/Σ)」セル: 平均[太字]／中央[9px・同色]／Σ[8.5pxグレー]の3段。h2=_elH2EvalByFnの返り値。
// 中央値の追加理由（2026-07-16e）: ★選定が損益額の一点張り（2026-07-16e当時は平均avgH2最大／2026-08-02k以降はΣ最大）なので、その平均が1件の大勝ちで作られていないかを検算する手段がこれまで表に無かった。
...
function _elH2WinLossCell(h2) {
  ...
  return React.createElement("div", { style: { lineHeight: 1.15 } },
    ...
    _l(h2.avgLoss, "負け・トントン(" + (h2.h2Cnt != null && h2.takeN != null ? (h2.h2Cnt - h2.takeN) : 0) + "件)の平均＝プラスにならなかった全件（0円のトントンも負け側・利確率の裏返し／指値同値は母数に入らない）"));
}
...
function _elBaseAlphaTrendBody(recs, aiOf, gran) {
  ...
  var rows = buckets.map(function(b, i) {
    ...
    return React.createElement("tr", { key: i },
      ...
      _elv2Td((p.decided != null ? p.decided : 0) + "件"));   // 新基準（想定損益ベース・平均主軸）の統計に統一 2026-07-13
  });
  ...
  ].filter(Boolean), { note: "各期間で「黒字・到達率が下限(既定" + _EL_ANA_REACH_DEF + "%)以上・頻度" + _EL_FREQ_MAX + "未満を満たすαのうちΣ想定損益（累計）が最大のα＝理想、推奨＝理想" + _elAlphaOffTxt("円") + "」（2026-08-02k Σ最大・手じまいベース・前提損切り値" + _elAnaCutCur + "円で評価）。件数が少ない期間も参考(橙「参考」)で表示。0〜20円。件数が少ない期間は振れやすい" }) : null;
  return React.createElement("div", null, chart, _elv2Table(["期間", "推奨基本α", "損切り率(最終)", "利確率", "想定損益(平均/Σ)", "E成立"], rows), insight);
}
...
function _elBaseAlphaSummary(recs, aiOf) {
  ...
  var _sweepHead = ["基本α", "到達率", "損切り率(最終)", "利確率(最終)", "E成立", "想定損益(平均/Σ)"];
  ...
  var cards = _elv2CardRow([
    _elv2Card("推奨基本α", React.createElement(React.Fragment, null, React.createElement("span", { style: { color: na ? "#B45309" : "#0369A1" } }, pick.alpha + "円"), _elReco2Node(pick.alpha2 != null ? (pick.alpha2 + "円") : null, 15, na ? "#B45309" : "#0369A1")), na ? "#B45309" : "#0369A1", na ? "条件緩和の参考" : "Σ想定損益 最大"),
    ...
    _elv2Card("平均想定損益", pick.avgH2 != null ? _elPnlFmt(Math.round(pick.avgH2)) : "—", _elPnlColor(pick.avgH2), "1件あたり・Σ" + (pick.h2Sum != null ? _elPnlFmt(Math.round(pick.h2Sum)) : "—")),
  ...
  return React.createElement("div", null, banner, cards,
    React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "#9A3412", margin: "8px 0 2px" } }, "α別の 到達率・損切り率(最終)・利確率・E成立・想定損益(平均/Σ)（理想＝到達率下限以上・損切り率(最終)≤" + Math.round(_EL_BASE_MAX_STOPRATE * 100) + "%・頻度" + _EL_FREQ_MAX + "未満を満たすαのうちΣ想定損益（累計）が最大のα／★＝推奨＝理想" + _elAlphaOffTxt("") + "・淡色は最低条件（到達率・損切り率・E成立・頻度<5・黒字）のどれかを満たさない・前提損切り値" + _elAnaCutCur + "円）"),
    _elv2Table(_sweepHead, sweepRows),
    _elInsightBoxV2([React.createElement("span", null, "推奨基本αは", _elInsightEmV2(pick.alpha + "円"), "（", (na ? "条件緩和の参考" : ("平均想定損益" + (pick.avgH2 != null ? _elPnlFmt(Math.round(pick.avgH2)) : "—") + "・損切り率" + (stopP != null ? stopP + "%" : "—") + "・利確率" + (takeP != null ? takeP + "%" : "—"))), "）。", "（応用αは「② 応用α」タブで）")], { note: noteSub }));
}
...
function _elBaseAlphaDetailV2(recs, aiOf, holiSet, onPick, curSel, spanOverride, poolAll) {   // spanOverride: 頻度列＋★頻度ゲートの分母を帯基準（_pbBandBizDays）に。株価帯コンテキストでピルと一致させる 2026-07-22j
  ...
  var _h2ByA = {};   // 想定損益デュアル列（利確率(最終)/想定損益・承認① 2026-07-12）: 表示各αの最終基準評価を母数一致(_baseRecs)で並走
  _dispSweep.forEach(function(e) { _h2ByA[e.a] = _elH2EvalByFn(_baseRecs, aiOfAna, function() { return e.a; }, true); });   // 第4引数true=指値同値を0円算入（2026-08-02e）＝★選定(_elBaseAlphaPick)と同じ土俵で表示する
  var sweepRows = _dispSweep.filter(function(e) { return e.entered > 0; }).map(function(e) {
    var _h2r = _h2ByA[e.a];
    var _ed = (spanOverride != null) ? _elEnteredCells(_baseRecs, function() { return e.a; }, aiOfAna) : _elEnteredDays(_baseRecs, function() { return e.a; }, aiOfAna);   // 帯基準時は分母も(銘柄×日)セルで単位一致（ピルと同じ）2026-07-22k。第3引数=指値同値を到達から外す 2026-08-02f（★の頻度ゲートと同じ値になる）
    ...
    if (!(_h2r && _h2r.h2Sum != null && _h2r.h2Sum > 0)) _ng.push(["赤", "Σ想定損益が赤字（または損益なし）"]);
    var pass = !_ng.length;
    ...
    var _cells = [
      ...
      _elv2Td(_elPctCell(_h2r ? _h2r.eRate : e.eRate)),   // 2026-08-02e 表示の到達率も★ゲートと同じ_h2r（同値を除外する版）へ。同値配線前は e.eRate と同値だったので見え方は変わらない
      _elv2Td(_elFillEqCell(_h2r ? _h2r.fillEqN : 0)),   // 同値列: 評価が数えた件数をそのまま出す＝表示と★選定で二重計算しない 2026-08-02e
      _elv2Td(_elFreqCell(_baseSpan, _ed)),
      _elv2Td(_elCntRateCell(_h2r ? _h2r.takeN : null, (!_h2r || _h2r.takeRate == null) ? "—" : _elPctCell(_h2r.takeRate))),   // 2026-08-02m 利確＝件数＋率の横並び
      _elv2Td(_elCntRateCell(_h2r ? _h2r.stopN : null, (!_h2r || _h2r.stopRate == null) ? "—" : _elStopRateCell(_h2r.stopRate))),   // 2026-08-02m 損切り＝件数＋率の横並び
      _elv2Td(_elH2AmtCell(_h2r)),
      _elv2Td(_elH2WinLossCell(_h2r)),
      _elv2Td(_elScoreCell(_h2r ? _h2r.score : null))   // 2026-08-02g 最終基準へ: _h2r=_elH2EvalByFn＝0.7×(1−損切り率(最終))+0.3×利確率(最終)。旧はe.score＝_elBaseAlphaEvalのH1基準（H1まで・ラインに触れたら損切り）で同じ行の他の列と土俵が違っていた
    ];
    ...
    // 2026-08-02m 「未達」列を撤去（応用α表と同時・列構成の一致を維持）。落ちたゲートは行のtitle＝ホバーのツールチップへ移設。
    return React.createElement.apply(null, ["tr", { key: e.a, title: (_ng.length ? ("未達: " + _ng.map(function(g) { return g[1]; }).join(" / ")) : "★の全最低条件を満たす"), onClick: onPick ? function() { onPick(e.a); } : null, style: { background: on ? "#FEF3C7" : (_isCur ? "#EFF6FF" : "transparent"), opacity: pass ? 1 : 0.65, cursor: onPick ? "pointer" : "default" } }].concat(_cells));
  });
  ...
  _mRecs.forEach(function(r) {
    ...
    // 2026-07-25f H1基準→想定損益基準へ（表の★ゲートは元から最終基準_elH2EvalByFnなので、この文章だけH1で食い違っていた）。
  ...
  insight = _elInsightBoxV2([
    React.createElement("span", null, (poolAll ? "★の一律α" : "採用α"), _elInsightEmV2(a + "円"), "の母数は", _elInsightEmV2(scN + "件"), "（OS3までにEP到達し決着）。うち損切り", _elInsightEmV2(stopN + "件"), "・想定損益プラス", _elInsightEmV2(winN + "件"), "・その他", _elInsightEmV2(otherN + "件"), "、対象外", _elInsightEmV2(offN + "件"), "（未到達）。"),
    React.createElement("span", null, "理想α＝", _elInsightEmV2("全条件（到達率" + _reachP + "%以上・損切り率(最終)≤" + Math.round(_EL_BASE_MAX_STOPRATE * 100) + "%・E成立≥" + _EL_BASE_MIN_N + "件・頻度" + _EL_FREQ_MAX + "未満・黒字）を満たすαのうち、Σ想定損益（累計）が最大のα"), "（到達率下限は🎯で調整可）。", _elInsightEmV2("推奨α＝理想" + _elAlphaOffTxt("円")), (_EL_ALPHA_OFFSET > 0 ? "（指値をギリギリで外さないよう理想より少し下げた実際に置く値）。" : "（最良と判定したαと実際に置くαをズラさない方針＝一律マージンは引かない。同値はどの列の母数にも入れず、件数だけ「同値」列に出る）。"), "同点は平均（1件あたり）が大きい方→低α。全条件を満たすαが1つも無ければ『条件適合無し』。"),
    React.createElement("span", null, _elInsightEmV2("中央値"), "が平均から大きく下なら、その平均は少数の大勝ちで作られている（＝毎回は取れない）。", _elInsightEmV2("淡色の行"), "は★の最低条件のどれかを満たさない行で、どの条件で落ちたかは行にカーソルを合わせると出る（2026-08-02mに「未達」列から行ツールチップへ移設）。", _elInsightEmV2("利確・損切り"), "の各列は「件数 率」の並び（率の分母＝E成立）。")
  ], { note: (poolAll ? "**分類「全記録」の母数＝基本・応用を問わず全記録（応用〇・浮き足〇・RN〇も含む）** に各αを一律で当ててシミュレーション。加算あり（浮き足〇/RN〇）の記録も一律αに置き換わるので、この表は「加算ルールを使わず一律αだけで通していたら」の答え。★はこの母数だけの参考値で、フォーム/EPナビ/シミュへ流れる推奨基本αは分類「基本α」（素の記録が母数）の値＝ここでは変わらない。" : "この銘柄のv2・算入記録（素の記録のみ）に各αを当ててシミュレーション") + "（前提損切り値" + _elAnaCutCur + "円＝各記録の実損切り値ではなくこの前提で評価）。E成立・利確率・損切り率(最終)・想定損益(平均/中央/Σ)・勝ち/負け平均＝想定損益(手じまい・EP/H1/H2損切り込み)基準・理想＝全条件を満たすαのうちΣ想定損益（累計）が最大のα（同点は平均が大きい方→低α）・推奨α＝理想" + _elAlphaOffTxt("円") + "。想定損益(平均/中央/Σ)の母数＝想定損益が確定した件数で、隣のE成立とはEP×見送り等でズレることがある。**同値はどの列の母数にも入れない**＝到達率・E成立・利確・損切り・想定損益(平均/中央/Σ)・勝ち/負け平均のすべてから外す（2026-08-10 ユーザー決定。旧2026-08-02eは想定損益にだけ0円で算入していたので、平均・中央値だけ同値ぶん分母が多かった）。勝ち/負けの境界は利確率と同じ（プラス＝勝ち／0円のトントンは負け側）。スコア＝［0.7×(1−損切り率(最終))+0.3×利確率(最終)］＝この行の他の列と同じ想定損益（手じまい・終値撤退）基準の要約値。★選定には不使用（★＝全条件を満たすαのうちΣ想定損益が最大）。2026-08-02gに旧H1基準［H1まで・ラインに触れたら損切り］から切替＝同じ行の中でスコアだけ土俵が違い、行の優劣が入れ替わって見えていたため。同値列＝そのαがOS1〜3高値の最大とちょうど一致した件数（＝予定EPに触れただけで一度も上抜けていない＝指値が刺さらなかった可能性・実際に約定済みの記録は除く）。**同値は「そのαなら取引していない」として全列の母数から外す**（2026-08-10）。Σ想定損益は旧仕様（0円算入）と同額・平均は分母が同値ぶん減るので上がる。したがって平均想定損益は「そのαで実際に約定した1件あたりの実力」であって、刺さらなかった機会のぶんは価格に入っていない＝その頻度は同値列の件数のほうで読む。理想・推奨はΣ最大で選ぶので同値の扱いを変えても基本的に動かない（同点時の第2キー＝平均だけ入れ替わりうる・2026-08-02k 選定基準はΣ最大＝累計重視）。頻度列（到達実日数）からも同値を外しているので、同値だけの日は「到達した日」に数えない＝★の頻度ゲートと同じ値。" });
  }
  return React.createElement("div", null,
    concl,
    _mixNote,
    _elv2Table([(poolAll ? "一律α" : "基本α"), "E成立", "到達率", "同値", "頻度", "利確", "損切り", "想定損益(平均/中央/Σ)", "勝ち/負け平均", "スコア"].concat(onPick ? ["選択"] : []), sweepRows),   // 2026-08-02m 「未達」撤去・利確/損切りは件数＋率
    insight);
}
// 推奨追加α 詳細データ（この銘柄/グループ）2026-07-03: 推奨基本α詳細データ(_elBaseAlphaDetailV2)の追加α版＝結論バー＋加算値別の総当たり（基本α＋加算ごとの到達率/件数/損切り率/H1勝率/試算損益・★＝推奨）＋読み取り。母数＝追加α〇（数値根拠＝底抜け前足浮きは除外・_elAddAlphaRecoと同一）。集計タブ銘柄別パネルで追加α母数トグル〇のとき、畳んだ基本α詳細の下に表示。試算損益＝ΣH1損益（_elBaseAlphaEval.pnl＝_elSimPnlByDay.sumと同値）。
...
function _elAddAlphaDetailV2(recs, aiOf, holiSet, fullRecs) {
  ...
  var concl = React.createElement("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "baseline", gap: "2px 10px", background: na ? "#FEF3C7" : (_hasPick ? "#FFF7ED" : "#F8FAFC"), border: "1px solid " + (na ? "#FCD34D" : (_hasPick ? "#FED7AA" : "#E2E8F0")), borderRadius: 8, padding: "8px 12px" } },
    ...
    _hasPick
      ? React.createElement("span", { style: { fontSize: 11, color: "#555" } },
          "平均想定損益 ", React.createElement("b", { style: { color: _elPnlColor(add.avgH2) } }, add.avgH2 != null ? _elPnlFmt(Math.round(add.avgH2)) : "—"),
  ...
  // 主表（2026-07-13刷新・ユーザー承認）: 日付別カウンターファクタル＝「各記録日の推奨基本α＋加算X円(0〜10)で入っていたら手仕舞い(想定損益)でどうだったか」。
  ...
  var _dateRows = (add && add.sweepDate ? add.sweepDate : []).filter(function(row) { return row.h2.entered > 0 || row.X === 0; }).map(function(row) {
    ...
    var pass = e.decided >= minN && e.eRate != null && e.eRate >= _EL_BASE_MIN_ERATE && e.stopRate != null && e.stopRate <= _EL_BASE_MAX_STOPRATE && e.h2Sum != null && e.h2Sum > 0;
    // 2026-07-16e: 基本α/応用α詳細表に合わせ H1勝率・平均H1損益を削除（想定損益と重複）＋列順を E成立→到達率／利確率→損切り率 に＋中央値・勝ち/負け平均を追加。
  ...
  var insight = _elInsightBoxV2([
    _hasPick
      ? React.createElement("span", null, "各記録日の推奨基本αに", _elInsightEmV2("+" + add.add + "円" + (_zero ? "（＝足さない）" : "")), "が最良（平均想定損益 ", _elInsightEmV2(add.avgH2 != null ? _elPnlFmt(Math.round(add.avgH2)) : "—"), "・損切り率(最終) ", _elInsightEmV2(_pctS(add.stopRate)), "・E成立 ", _elInsightEmV2((add.decided || 0) + "件"), "）。")
    ...
    (_hasPick && !_zero && _zeroRow && _zeroRow.avgH2 != null && add.avgH2 != null)
      ? React.createElement("span", null, "足さなかった場合（+0円）の平均想定損益は", _elInsightEmV2(_elPnlFmt(Math.round(_zeroRow.avgH2))), "＝上乗せの効果は1件あたり", _elInsightEmV2(_elPnlFmt(Math.round(add.avgH2 - _zeroRow.avgH2)), (add.avgH2 - _zeroRow.avgH2) >= 0 ? "#C0392B" : "#1E8449"), "。")
      : null
  ], { note: "母数＝追加α〇（浮き足〇・RN〇除外）。主表＝各記録に「その記録日時点の推奨基本α（前日までの直近50→100件→全期間・現在の★基準で再計算）＋加算X円」を当て、手じまい（想定損益・EP/H1/H2損切り込み）で評価する反実仮想。★＝到達率" + Math.round(_EL_BASE_MIN_ERATE * 100) + "%以上・損切り率(最終)" + Math.round(_EL_BASE_MAX_STOPRATE * 100) + "%以下・E成立" + _EL_BASE_N_PREF + "件（無ければ" + _EL_BASE_MIN_N + "件に緩和）・黒字の中で平均想定損益（1件あたり）最大＝基本αと同じ条件式。中央値が平均から大きく下なら、その平均は少数の大勝ちで作られている（＝毎回は取れない）。勝ち/負けの境界は利確率と同じ（プラス＝勝ち）。参考表＝旧方式（現在の推奨基本α" + base + "円を土台に一様加算・H1基準）。" });
  return React.createElement("div", null,
    concl,
    _uzuLine,
    ...
    _dateRows.length ? _elv2Table(["追加α", "E成立", "到達率", "頻度", "利確率(最終)", "損切り率(最終)", "想定損益(平均/中央/Σ)", "勝ち/負け平均"], _dateRows) : React.createElement("div", { style: { fontSize: 10.5, color: "#bbb", padding: "4px 0" } }, "推奨基本αが算出できる記録がありません（履歴不足）"),
    _lbl("（参考）現在の推奨基本α" + base + "円＋加算の固定スイープ（＋1〜" + _EL_BASE_ADD_MAX + "円・H1基準・旧方式＝記録日に依らず今の推奨基本αを土台に一様加算・★なし）"),
    _elv2Table(["追加α", "合計α", "到達率", "有効件数", "損切り率(H1接触)", "H1勝率", "試算損益"], addRows),
    insight);
}
// 推奨基本α表（銘柄/期間グループ別）: groups=[{label,recs}]・cutFn(r)→損切り値。各グループの推奨基本α(_elBaseAlphaPick・5〜20・
// 全条件ゲート＋Σ想定損益（累計）最大・該当なしは到達率50%緩和)を1値表示＋損切り率/H1勝率の小書き＋追加α目安。旧 _elIdealAlphaTableV2(EP/H1/H2別・0〜50)を置換 2026-06-21→条件再設計 2026-06-22→選定は合成スコア→平均想定損益へ(コメント更新 2026-07-22j)。
...
// 指定αを母数recに一律適用したH1試算損益（_elBaseAlphaEvalのpnlと同基準＝OS1〜3でEP到達しH1損益が判定できる記録のΣ_elDynHold）と、その記録の異なる営業日数。
// 「1営業日あたり」＝Σ試算損益÷エントリー成立日数（記録の無い日・ノーシグナル日は母数に入らず自然に除外）。返り値 { sum, n, days, avg } 2026-06-24。
...
// 試算損益セル: 上段「+X円/日」（1営業日あたり平均）＋下段「1件 +Z円」（1件あたり期待損益＝試算損益÷件数・件数/頻度に左右されず窓間で比較しやすい）2026-06-27（旧: 下段は期間累計）。データ無しは—。利益=赤/損=緑（当アプリ慣習）。
function _elSimPnlCell(sim) {
  ...
  var per = (sim.n > 0) ? Math.round(sim.sum / sim.n) : null;   // 1件あたり期待損益（試算損益÷件数）2026-06-27
...
function _elBaseAlphaPeriodTableV2(recs, aiOf, refDate, includeToday) {
  ...
  W.periods.forEach(function(pd, i) {
    ...
    var basePool = pd.recs.filter(_elIsBaseAlphaPoolRec);   // 試算損益の母数も推奨基本α(_elBaseAlphaPick)と同じ三重除外に揃える 2026-07-13
    ...
    // 推奨追加α（└サブ行・基本αに足した時の損切り率〜試算損益も並べる）2026-06-30。母数は追加α〇プール＝基本行(×・未選択)とは別母集団。
    if (add) {
  ...
  return _elv2Table(["期間", "推奨基本α", "推奨損切り", "損切り率", "H1勝率", "到達率", "有効件数", "試算損益(1日/1件)"], rows);
}
// 推奨追加α値の期間別表（母数＝追加α〇の記録だけ＝基本α表とは別プール）2026-06-24→24g。各期間で「基本α＋推奨追加α」を〇記録に当てた 損切り率/H1勝率/到達率/件数/試算損益。
// 推奨追加α＝_elBaseAlphaA().add（基本αに足して損切りを避けH1黒字にできる最小加算＝試算損益はプラス・改善無しは「推奨無し」）。次点は点線区切りで1行追加。〇記録の無い期間は—。
function _elAddAlphaPeriodTableV2(recs, aiOf, refDate, includeToday) {
  ...
  return _elv2Table(["期間", "推奨追加α", "損切り率(最終)", "利確率(最終)", "到達率", "E成立", "想定損益(平均/Σ)"], rows);
}
...
// 指標（損切り率/勝率/試算損益）は出さずα値だけのシンプル表示＝詳細は各銘柄テーブル(ChartSection)の期間別表(_elBaseAlphaPeriodBlockV2)で（取引タブの重複フル版は2026-06-26削除）。
...
function _elAddAlphaSectionV2(recs, aiOf, data) {
  ...
  var _h2At = function(s, a, cut) { if (a == null || !_enteredAt(s, a)) return 0; var t = _elHoldFinalParts(s, a, cut); return (t && t.main != null) ? t.main : 0; };   // 手じまい（想定損益・（）外）基準 2026-07-13
  ...
  if (!eff.length) effNode = React.createElement("div", { style: { fontSize: 11, color: "#aaa", padding: "6px 0" } }, "基本α・採用αが揃った記録がありません（基本α値の保存が必要）。");
    ...
    effNode = React.createElement(React.Fragment, null,
      ...
      ], { note: "各追加α〇記録で『実際の採用α（基本＋追加）の手じまい損益（想定損益・EP/H1/H2損切り込み）』と『基本αだけだった場合の手じまい損益』を、同じ値動き・同じ損切り値で比較（未達＝取引なし＝0円・2026-07-13にH1基準から手じまい基準へ変更）。差の合計＝追加αの判断が生んだ損益。" })
    );
  }
...
function _elFloatReasonSectionV2(recs, aiOf, data, secH, basePick, recCtx) {
  ...
  // 浮き足加算率スイープ 2026-07-12: 採用α（基本＋追加＋RN）は実績のまま浮き足加算だけをP=0〜100%(10刻み)で振り、想定損益(H2)で評価。全銘柄版(シグナル総合)と同じ_elUkiPctSweepに統一（旧「基本α＋浮き足%・ΣH1最大」から刷新）。母数=浮き値>0の浮き足〇記録。
...
// 浮き足加算率スイープ 2026-07-12: 採用α（基本＋追加＋RN）は実績のまま、浮き足加算だけをP=0〜100%(10刻み)で振り、想定損益で評価。alphaOf(r)=（採用α−現行浮き足加算floor(v/2)）＋floor(浮き値×P/100)＝P=50%で現行(半額)に一致＝実績と同値。母数poolは呼び出し側で浮き足〇&浮き値>0に絞る。best=件数(E成立)≥_EL_BASE_MIN_N&試算損益>0でスコア最大。シグナル総合タブ(全銘柄)＋シグナル別「浮き足」サブタブ(_elFloatReasonSectionV2)で共用。
function _elUkiPctSweep(pool, aiOf, holiSet) {
  ...
  var rows = [];
  for (var P = 0; P <= 100; P += 10) { var _af = _mk(P); rows.push({ P: P, ev: _elH2EvalByFn(pool, aiOf, _af, true), entDays: _elEnteredDays(pool, _af, aiOf) }); }   // 2026-08-02h 同値は全列除外＋頻度の分子からも同値を外す（α詳細表・RNボードと同基準）2026-08-10
  ...
  var _sigOf = function(x) { return x.ev.h2Sum; }, _avgOf = function(x) { return x.ev.avgH2; }, _pOf = function(x) { return x.P; };   // Σ想定損益（累計）最大 2026-08-02k（旧＝平均最大）
...
// 浮き足加算率スイープの推奨バー＋表を描画 2026-07-12→2026-07-18 α詳細表(_elBaseAlphaDetailV2)と同じ列立てに刷新。列＝浮き足%/E成立/到達率/頻度/利確率/損切り率/想定損益(平均/中央/Σ)/勝ち負け平均/スコア。★推奨＝最良（フォーム自動入力に使う値）・次点も表示・件数_EL_BASE_MIN_N未満は薄字「参考」。頻度はsweep.span(活動営業日)÷行のentDays(到達実日数)。
  ...
  var _reco = React.createElement("div", { style: { fontSize: 12, fontWeight: 700, marginBottom: 6, color: best ? "#0369A1" : "#94A3B8" } },
    best
      ? React.createElement("span", null, "推奨 " + cfg.recoWord + " ", React.createElement("span", { style: { fontSize: 16, fontWeight: 800 } }, _k(best) + _u),
          "（損切り" + Math.round((best.ev.stopRate || 0) * 100) + "%・利確" + Math.round((best.ev.takeRate || 0) * 100) + "%・試算損益計" + _elPnlFmt(Math.round(best.ev.h2Sum)) + "・" + best.ev.decided + "件）",
      ...
      : "推奨：件数" + _EL_BASE_MIN_N + "以上で試算損益プラスの" + cfg.recoWord + "は出ていません" + (cfg.noneTail || "（データ不足）"));
...
function _elUkiValSweep(pool, aiOf, holiSet) {
  ...
  var _span = _elBizSpanDays(pool, holiSet);
  var rows = [];
  for (var X = 0; X <= _MAX; X += 1) { var _af = _mk(X); rows.push({ X: X, ev: _elH2EvalByFn(pool, aiOf, _af, true), entDays: _elEnteredDays(pool, _af, aiOf) }); }   // 2026-08-02h 同値は全列除外＋頻度の分子からも同値を外す（%版と同基準）2026-08-10
  var _qual = function(x) { return x.ev.decided >= _EL_BASE_MIN_N && x.ev.h2Sum != null && x.ev.h2Sum > 0 && x.ev.score != null; };
  var _quals = rows.filter(_qual);
...
// 2026-07-25 株価帯レッグ（ユーザー決定「推奨%も帯優先にする」）: stockを渡すと「refDateのその銘柄の株価帯」と同じ帯の記録だけに絞る（銘柄横断・帯判定は_pbDayBandOfが単一源）。
//   帯プールが_EL_BASE_MIN_N未満／帯不明／⚡固有材料日 は従来どおり全銘柄プール＝純フォールバック（推奨αの帯カスケードと同じ思想）。
//   **stock未指定なら従来と完全に同一＝後方互換**。根拠別(_sp)の絞り込みは帯プールの上に重ねて効く。
function _elUkiPctPickScoped(data, refDate, mode, reasons, stock) {
  var _sp = mode === "special";
  var all = _elCollectAllSignals(data) || [];
  ...
  var byBand = false, bandLabel = null;   // 株価帯レッグ 2026-07-25（帯プールが十分なときだけ帯に絞る）
  if (stock && refDate && typeof _pbDayBandOf === "function") {
    var _myB = _pbDayBandOf(data, stock, refDate);
    if (_myB && _myB.idx != null && !_myB.material) {
      var _bMemo = {};
      var _bandPool = pool.filter(function(r) {
        var k = r.stock + "_" + r.date;
        if (!(k in _bMemo)) _bMemo[k] = _pbDayBandOf(data, r.stock, r.date);
        var b = _bMemo[k];
        return !!(b && !b.material && b.idx === _myB.idx);
      });
      if (_bandPool.length >= _EL_BASE_MIN_N) { pool = _bandPool; byBand = true; bandLabel = _pbBandLabel(_myB.idx, _myB.bounds); }
    }
  }
  ...
  if (!pool.length) return { reco: null, runnerUp: null, n: 0, byReason: byReason, fellBack: fellBack, byBand: byBand, bandLabel: bandLabel };
  var sweep = _elUkiPctSweep(pool, function(r) { return _elAlphaInfo(r, data); });
  ...
  if (_sp && _reco != null) {
    var _bp = _elUkiPctPickScoped(data, refDate, "basic", null, stock);   // 2026-07-25 帯も同じスコープで取る（帯の応用推奨を帯の基本推奨でクランプ）
...
// 想定損益の（）外(main)がnullになるのは ①×見送り（スルー宣言がEP足より前）②α未達（EP不成立）③（）外が立たない△エントリー の3つ。
// ①②は「そのαなら取引しなかった」、③は（）外＝0の規約（_elTotAccumと同じ）＝いずれも（）外への寄与は0円。
function _elUkiAltWhy(s, a, mainV) {
  if (a == null || mainV != null) return null;
  if (_epIsXSkip(s, a)) return "スルー";
  if (_elH2Miss(s, a)) return "未達";
  return "△のみ";
}
// 1記録の換算結果 {band, altA, curA, cut, cur(実際の想定損益main), alt(換算の想定損益main), curWhy/altWhy, diff}。
// 2026-07-27（ユーザー決定）: 片側が不成立でも0円として差額に算入＝「αを下げたから取れていた取引」が集計から消えないようにする。
//   旧＝mainがnullなら行ごと換算不可（差額—・合計から除外）→ 応用α換算の効果そのものが落ちていた。
//   真の換算不可はα不明（帯不明＝水準線未入力）だけ＝cur/altがnullになるのはその時のみ。
...
function _elWeeklyTargetSummaryV2(recs, aiOf, dataOpt, scopeStock) {
  ...
  var bases = [{ key: "re", label: "実現損益(100株)" }, { key: "h2", label: "想定損益" }];
  ...
  var weekTable = React.createElement(_HScrollBox, null,
    React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11 } },
      React.createElement("thead", null, React.createElement("tr", { style: { background: "transparent" } },
        _th("週（月〜金）", { textAlign: "left", paddingLeft: 10, color: "#555", borderBottomColor: "#ddd" }), _th("取引日", { color: "#555", borderBottomColor: "#ddd" }), _th("実現損益", { color: "#555", borderBottomColor: "#ddd" }), _th("想定損益", { color: "#555", borderBottomColor: "#ddd" }))),
  ...
  return React.createElement("div", { style: { marginBottom: 14, padding: "10px 12px", border: "1px solid #FB923C", borderRadius: 8, background: "#FFFCF8" } },
    ...
    React.createElement("div", { style: { fontSize: 10, color: "#94A3B8", marginBottom: 8 } }, "全 " + S.days + "取引日 / " + W + "週。平均は1取引日あたり（Σ損益÷取引日）＝休場・祝日に依存しない。5営業日換算/週＝1日平均×5の目安。達成率＝1日の損益が2,500円以上だった割合（達成日÷取引日）。達成日数/週＝達成率×5の換算。実現損益＝確定損益を100株換算（損益÷株数×100・E成立分）／想定損益＝期待度○が途切れた所で手じまい（（）外＝○のみ・旧H2損益と同一基準）・採用α基準・既に100株換算。週別表は各週の実額。上の期間フィルタに連動。"),
    summaryTable,
    weekTable);
}
...
function _elDayStockBenchV2(_ref) {
  ...
  var mk = function(recs) {
    ...
    if (!st) return null;
    var ba = _elBaseAlphaPick(recs, aiOf);            // 推奨基本α（平均想定損益・単一 ※旧コメント「合成スコア」は誤り 2026-07-22j）
  ...
  var METRICS = [
    ...
    { key: "h2", label: "想定損益", cell: function(st) { return pnlMT(st.h2Avg, st.h2Sum, st.h2Cnt); }, dir: "up", num: function(st) { return st.h2Avg; } },
...
//   一方 _epHoldLadder は【2026-07-27の終値撤退方式より前は】「最初に h−α≥C に触れた足の高値」で損失を決めていたので、
//   C を詰めると損切り組の停止足は早まるか同じ＝損失は縮むか同額、と言えた（実測: C20→15 で -12,000円→-12,000円＝同額のケースあり）。
//   2026-07-29 訂正: 現在の _epHoldLadder(app-05.js) は撤退足の【終値】で損益を決めるため、この定理は成り立たない。
//   C を詰めると撤退足が前へ移り、その足の終値が元の撤退足より悪いことがある（高値と終値の大小は独立）。
//   さらに、接触しただけで損切りにならなかった記録（終値が利益側）が、C を詰めた結果ほんとうに損切りへ転ぶこともある。
//   ⇒ C' = 最悪逆行+1 について断定できるのは「この標本では損切り率も生存側の損益も悪化させない」までで、
//     損切り組の損益は再計算しないと分からない。★の位置は据え置くが、表示文からは「負け額は縮むか同額」を外してある。
...
function _elStopMaeSectionV2(entered, aiOf) {
  ...
  if (!rows.length) return React.createElement("div", { style: { color: "#bbb", fontSize: 12, padding: "8px 0" } }, "損切りを免れた記録がありません（E成立 " + (entered || []).length + "件・うちライン接触 " + stopN + "件）");
  var arr = rows.map(function(o) { return o.mae; });
  var srt = arr.slice().sort(function(x, y) { return x - y; });
  ...
  var detTbl = _elv2Table(["日付", "銘柄", "α/損切", "逆行(円)", "損切りまで", "手じまい", "想定損益(100株)"], top.map(_detRow));
  ...
  var items = [
    ...
    (!_thin && canTighten)
      ? React.createElement("span", null, "損切り値", _elInsightEmV2(_cutLbl), "に対し最悪でも", _elInsightEmV2(maxMae + "円"), "で収まっている＝", _elInsightEmV2(floorCut + "円まで詰めても"), "この母数では損切り率も生存側の損益も1円も変わらない", _elInsightEmV2("（損切り組の損益は要確認）", "#B45309"), "。")
      : (_thin
  ...
  return React.createElement("div", null, cards,
    ...
    React.createElement("div", { style: { marginTop: 8 } },
      _elInsightBoxV2(items, { note: "対象＝E成立(judge ok)のEP起算v2記録のうち、採用α・採用損切り値で**損切りラインに触れずに済んだ**もの（" + rows.length + "件／ライン接触 " + stopN + "件）。逆行＝EP足〜手じまい足の高値の最大 − α（水準線比の円・100株換算ではない・**損益ではないので深いほど赤**）。手じまい足＝次足期待度×/未設定で降りた足＝記録表の「想定損益・詳細」「保有」列と同基準。※上の「損切り回数」カードとこの「ライン接触 " + stopN + "件」は基準が違う（カードは終値撤退方式で**損で撤退したものだけ**を数えるので、触れても撤退足の終値が利益側なら損切りに数えない。こちらは触れた時点で母数から外す＝触れた記録はそこで撤退しており逆行の物差しが頭打ちになるため。なお期待度×で先に降りた後の接触は両方とも対象外＝『免れた』側）ため一致しないことがある。損切りになった記録は下の『📐 損切りの上振れ』が担当（そちらの超過幅は全足の非キャップ最高値基準＝母数も物差しも別）。「詰められる下限」はこの標本の実績値＝将来これを超える逆行が出る可能性は排除できない。" })));
}
// recCtx={expKey,setExpKey,onEdit,onGoDate}（2026-07-27追加）: この関数はモジュールスコープで記録帳の state に触れないため、
// 行タップ→明細カード展開に必要なものを呼び出し側から渡す。_elFloatReasonSectionV2 と同じ作法。未指定なら展開なし。
function _elStopOvershootSectionV2(recs, aiOf, data, recCtx) {
  var BIG = 99999;
  var rows = [];
  (recs || []).forEach(function(r) {
    ...
    // 2026-07-29 旧方式（ライン即時約定）との比較列 lineNow を撤去し、比較軸を「保有していたら」へ戻した。
    //  actual = 終値撤退方式の損切り額（撤退足の確定値＝_epHoldLadder.finalPnl）
    //  held   = 損切りせず最後まで保有した場合の損益。差額＝held−actual＝損切りしなければ助かった額。
    // このセクションは「損切りが早すぎないか」の検証が目的なので、比較相手は保有側が本筋。
    rows.push({
  ...
  var cards = _elv2CardRow([
    ...
    _elv2Card("損切り損失(平均)", _elPnlFmt(actualArr.length ? Math.round(_elMean(actualArr)) : null), _elPnlColor(actualArr.length ? _elMean(actualArr) : null), actualArr.length + "件" + (rows.length > actualArr.length ? "（終値未入力" + (rows.length - actualArr.length) + "件を除く）" : "")),   // 2026-07-29 平均の母数は金額が出せた件数(actualArr)。旧表示は rows.length で、件数×平均が合わなかった
  ...
  // 2026-07-27 レイアウト刷新（ユーザー要望）:
  //  ・「超過幅×損益（1円ブロック）」の分布表を撤去。超過幅は _elHoldMaxHigh(s).all＝全足の最高値基準なので
  //    「損切りした瞬間にどれだけ行き過ぎたか」ではなく「その記録が最終的にどこまで逆行したか」を測っており、
  //    実データではほぼ全件が「5円以上」に寄って1行の表にしかならず機能していなかった。
  //  ・一覧は「実際の損切り額／終値まで持っていた場合／差額」＋合計行の4項目に集約。並びは差額の大きい順
  //    （＝損切りしなければ助かっていた順）。旧・超過幅の小さい順は上記の理由で意味を成していなかった。
  //  ・「ベスト」列は落とす（カードと読み取り文には残す）。「保有なら」は2026-07-29に列へ復帰
  //    （旧方式比較列の撤去に伴い、差額＝保有−実際＝損切りしなければ助かった額を並び順の軸に戻した）。
  var _dow = function(ds) { var p = ds.split("-"); return ["日", "月", "火", "水", "木", "金", "土"][new Date(+p[0], +p[1] - 1, +p[2]).getDay()]; };
  var _diffOf = function(o) { return (o.held != null && o.actual != null) ? (o.held - o.actual) : null; };
  var listed = rows.slice().sort(function(a, b) {
    var da = _diffOf(a), db = _diffOf(b);
    if (da == null && db == null) return a.r.date < b.r.date ? 1 : -1;
    if (da == null) return 1;
    if (db == null) return -1;
    return (db - da) || (a.r.date < b.r.date ? 1 : -1);
  });
  var _tot = function(pick) { var t = null; rows.forEach(function(o) { var v = pick(o); if (v != null) t = (t == null ? 0 : t) + v; }); return t; };
  var totActual = _tot(function(o) { return o.actual; });
  var totHeld = _tot(function(o) { return o.held; });
  var totDiff = _tot(_diffOf);
  var listTable = React.createElement("div", null,
    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#9A3412", margin: "8px 0 0" } }, "損切り記録の一覧（差額の大きい順＝損切りしなければ助かっていた順・" + rows.length + "件）"),
    recCtx ? React.createElement("div", { style: { fontSize: 9, color: "#aaa", margin: "1px 0 0" } }, "行をタップすると明細カードが開きます") : null,
    _elv2Table(["日付", "銘柄/シグナル", "α/損切値", "実際の損切り額", "保有していたら", "差額"], listed.map(function(o, i) {
      var ek = "stopov_" + o.r.stock + "_" + ((o.s && (o.s.id || o.s.time)) || i);
      var on = !!(recCtx && recCtx.expKey === ek);
      var df = _diffOf(o);
      var row = React.createElement("tr", { key: ek,
        onClick: recCtx ? function() { recCtx.setExpKey(on ? null : ek); } : null,
        style: { background: on ? "#FFF7ED" : "transparent", cursor: recCtx ? "pointer" : "default" } },
        _elv2Td((on ? "▶ " : "") + o.r.date.slice(5) + "(" + _dow(o.r.date) + ")", { textAlign: "left", paddingLeft: 8, whiteSpace: "nowrap" }),
        ...
        // 実際の損切り額。保有していた方が良かった記録＝損切りが早すぎた疑い＝太字で目立たせる
        _elv2Td(React.createElement(df != null && df > 0 ? "b" : "span", { style: { color: _elPnlColor(o.actual) } }, _elPnlFmt(o.actual))),
        _elv2Td(React.createElement("span", { style: { color: _elPnlColor(o.held) } }, _elPnlFmt(o.held))),
        _elv2Td(React.createElement("b", { style: { color: _elPnlColor(df) } }, _elPnlFmt(df))));
      if (!on || !recCtx) return row;
      return [row, React.createElement("tr", { key: ek + "_c" },
        React.createElement("td", { colSpan: 6, style: { padding: "4px 8px 8px", background: "#FFFCF8", borderBottom: "2px solid #FB923C" } },
          React.createElement(EntryLogCard, { record: o.r, data: data, onEdit: recCtx.onEdit, onGoDate: recCtx.onGoDate })))];
    }).concat([
      React.createElement("tr", { key: "__tot__", style: { borderTop: "2px solid #9A3412", background: "#FAF6F0" } },
        _elv2Td(React.createElement("b", null, "合計"), { textAlign: "left", paddingLeft: 8, color: "#9A3412" }),
        _elv2Td(React.createElement("span", { style: { fontSize: 10, color: "#9A9186" } }, rows.length + "件"), { textAlign: "left" }),
        _elv2Td(""),
        _elv2Td(React.createElement("b", { style: { color: _elPnlColor(totActual) } }, _elPnlFmt(totActual))),
        _elv2Td(React.createElement("b", { style: { color: _elPnlColor(totHeld) } }, _elPnlFmt(totHeld))),
        _elv2Td(React.createElement("b", { style: { color: _elPnlColor(totDiff), fontSize: 12 } }, _elPnlFmt(totDiff))))
    ])));
  ...
  return React.createElement("div", null, cards, listTable, _elInsightBoxV2(items, { note: "**2026-07-27 損切り方式を変更**＝損切り値に触れた瞬間ではなく**触れた足の確定値(終値)で撤退**し、損で撤退したものだけを損切りとする。対象＝実現結果が「損切り」のEP起算(v2)記録＝**手じまい足までに損切りラインへ触れ、その足の終値が損側だったもの**（×見送り・E未達・手じまい後の接触は対象外。**終値が利益側に戻った記録は損切りでなくなるのでこの表には出ない**）。実際の損切り額＝撤退足の確定値((α−終値)×100株・採用α基準)。「保有していたら」＝損切りせず実運用ルール（確定値が前の足より悪化したらその足の確定値で手仕舞い・悪化しなければ最後まで保有）で降りた損益＝**足をまたいで先まで持つので撤退足の終値とは別物**。差額＝保有していたら−実際（プラス＝損切りしなければ助かっていた＝損切りが早すぎた疑い）。ベスト＝各足で最も良い手仕舞い(maxPnl)。超過幅＝最高値(水準線比)−(α＋損切り値)＝**全足の最高値基準**なので「損切りした瞬間の行き過ぎ」ではなく「最終的にどこまで逆行したか」。損益色は赤=利益/緑=損失。" }));
}

// ===== 損切りの分析（記録帳・銘柄別タブ「🛑損切り」／2026-06-22）=====
// エントリー成立(E成立=judge"ok")記録の損切りを多角的に分析。recs=対象v2記録・aiOf=α情報・data。
// ①損切りサマリー ②損切り値(cutLine)別シミュ＝全記録に同じ損切り値を当てた損切り回数/率・想定損益（★=想定損益最大の損切り値＝意思決定表・2026-07-09 EP/H1列廃止） ③損切りの上振れ・早すぎ検証(_elStopOvershootSectionV2を移設) ④シグナル別の損切り率。
function _elStopTabSectionV2(recs, aiOf, data, hideSig, recCtx) {
  ...
  // 2026-07-29 撤退足の終値(確定値)が未入力の記録は損益を出せない＝件数には入るが金額集計から落ちる。
  // その差が「損切り回数」と「損切り損失 合計」の件数の食い違いとして現れるので、件数を明示する。
  var noCloseN = enteredStopN - lossArr.length;
  ...
  var cards = _elv2CardRow([
    ...
    _elv2Card("損切り損失 合計", _elPnlFmt(lossArr.length ? Math.round(lossTotal) : null), _elPnlColor(lossArr.length ? lossTotal : null), lossArr.length + "件" + (noCloseN > 0 ? "（終値未入力" + noCloseN + "件は金額不明）" : "")),
  ...
  var simTbl = _elv2Table(["損切り値", "損切り回数", "損切り率", "想定損益", "平均逆行", "最悪逆行"], sim.map(function(x, i) {
  ...
  var insight = _elInsightBoxV2([
    ...
    React.createElement("span", null, "損切り値を ", _elInsightEmV2(best.cut + "円"), " にすると想定損益が最大（", _elInsightEmV2(_elPnlFmt(Math.round(best.h2))), "・損切り率 ", _elInsightEmV2(Math.round(best.rate * 100) + "%"), "）。狭め＝損切り増・浅い損失／広め＝損切り減・大きい損失のトレードオフ。"),
  ...
  ], { note: "対象＝E成立（エントリーできた）v2記録 " + entered.length + "件。損切り＝想定/H1/H2いずれかで損切りライン（高値−α≥損切り値）に触れ、その足の終値で撤退した結果が損だったもの（触れても終値が利益側なら損切りに数えない＝2026-07-27 終値撤退方式）。※撤退足の終値が未入力の記録は損か利益かを判定できないため、ラインに触れた事実を優先して損切りに算入する（保守側に倒す）。ただし金額は出せないので損失合計・平均からは外れる＝「損切り回数」と「損切り損失 合計」の件数が食い違うのはこのため。損切り値別シミュは全記録に同じ損切り値を当てた場合の合計（採用α基準・100株換算・損益色は赤=利益/緑=損失）。×見送り＝事前に期待度×を宣言した後にα到達した記録を「取引していたら」のEP損益で評価。" });
  ...
  return React.createElement(React.Fragment, null,
    ...
    _h("🎚 損切り値の最適化（損切り値別シミュ）", "※全記録に同じ損切り値を当てたときの損切り回数・想定損益（★=想定損益が最大）。逆行＝その損切り値で生き残った取引が踏まれた深さ＝損切り値との差が余裕。損益色は赤=利益だが、逆行は損益ではなく深いほど悪い（赤=損切りに肉薄）"),
    simTbl,
    _h("📉 逆行（MAE）＝損切りを免れた取引は何円踏まれたか", "※対象＝採用損切り値で損切りにならなかった取引。最悪逆行＋1円がその損切り値を詰められる下限（この母数では損切り率も生存側の損益も変わらない。ただし終値撤退方式では損切り組の額は撤退足の終値次第なので、縮むとは限らない）"),
    _elStopMaeSectionV2(entered, aiOf),
    _h("📐 損切りの上振れ・早すぎ検証", "損切りラインを何円超えて伸びたか＋損切りせず保有/最良手仕舞いなら何円だったか＝損切りが早すぎないかの検証"),
    _elStopOvershootSectionV2(rs, aiOf, data, recCtx),
...
// 最適ホールド本数（記録帳・深掘りタブ／2026-06-14）: EPから+0/+1/+2本…と持ち続けた場合の深さ別の平均損益・ライン接触率・EP比改善率を集計し、
...
function _elHoldDepthSectionV2(recs, aiOf) {
  ...
  return React.createElement("div", null,
    _elv2Table(["手仕舞い位置", "件数(到達)", "平均損益", "合計", "ライン接触率(累積)", "EP比改善率"], body),
    _elInsightBoxV2(items, { note: "各記録をEPからその本数だけ持って手仕舞いした場合の損益（_epHoldLadder・採用α/損切り基準・100株換算）。深い本数はそこまで足がある記録のみの平均＝母数が減る点に注意（合計列も参考に）。ライン接触率(累積)=その本数までに損切りラインへ触れた割合＝終値撤退方式ではその時点で撤退している割合（2026-07-29に「損切り率(累積)」から改称。他セクションの「損切り率」は_elIsStopFinal＝触れて**かつ**損で撤退したものだけなので別物＝同じ名前で2つの意味が混ざるのを解消した）。EP比改善率=EP即手仕舞いより損益が良くなった割合。★=平均損益が最大の本数。" }));
}
...
function _elExpCalibSectionV2(recs, aiOf) {
  ...
  // 2026-07-29 「損切り率」→「ライン接触率」へ改称。この列は _elHoldIsStop/_elHoldIsStop2＝そのレグの高値が
  //   損切りラインに触れたか（接触）で数えており、他セクションの損切り率(_elIsStopFinal＝触れて且つ終値が損)とは
  //   別物。同じ名前で2つの意味が並ぶのを避ける（変更Eの「ライン接触率(累積)」と同じ整理）。計算は変更なし。
  var hHead = ["H期待", "件数", "実損益", "実勝率", "ライン接触率", "予想的中率"];
  ...
  return React.createElement("div", null, h1Table, h2Table, _elInsightBoxV2(items, { note: "H期待の的中=○なら実H1/H2損益>0、×なら≦0。実損益は採用α基準・E成立分のみ。ライン接触率＝そのレグの高値が損切りラインに触れた割合＝**接触基準**で、他セクションの「損切り率」（_elIsStopFinal＝触れて**かつ**その足の終値が損）とは別物。実損益は終値ベースなので、接触しても終値が利益側なら損益はプラスのまま出る。" }));
}
...
// 【2026-07-20b】損益は記録表/損益データ欄の「想定損益」と同一の期待度別算入（_elHoldFinalParts＝（）外main/（）内ref）に統一。
//  旧＝_elHold1TotParts（H1基準）だったが、アプリ全体が2026-07-09〜07-13で想定損益へ移行し推奨α選定(_elBaseAlphaPick)も想定損益基準なのに、シミュだけH1で採点していた＝H2まで持った勝ちを取りこぼす構造的なズレだったため切替。
//  main100=（）外／ref100=（）内差分（△・損切り済のみ）。pnl100はmain100の別名（既存の（）外消費箇所と互換）。
// 返り値 { built, skip('unreached'|'x'|'noalpha'|'fillEq'|null), pnl100/main100/ref100(100株換算・判定不可はnull), stop, indet, a(実効α) }。
// touchA（省略可・2026-08-06 オプション「指値同値は建てない」）= その記録の「触れただけα」= round(_elOsMaxAll)。実効αがこれとちょうど一致する段は約定しなかった可能性が高いので建てない（skip:'fillEq'）。
//   判定は**掃引αで動的に**行う（RN自動加算と同じ思想）＝保存値の指値同値フラグではない。αを下げれば同じ記録が普通に建つ／αを上げれば未到達になる。
//   nullで従来どおり（オプションOFF）。実約定の証拠がある記録はUI側(_touchAlphaOf)でnullを返して除外対象から外す。
function _elKabuTierEval(s, a, cut, touchA) {
  ...
  if (touchA != null && _a === touchA) return { built: false, skip: "fillEq", pnl100: null, main100: null, ref100: null, stop: false, indet: false, a: _a };   // 2026-08-06 指値同値＝予定EPに触れただけ。×見送り/未到達の後に置く＝それらの理由を上書きしない（本来なら建っていた段だけがfillEqになる）
  var stop = _elIsStopFinal(s, _a, _cl);   // 2026-07-25e 損益(_elHoldFinalParts)が手じまい基準なのにフラグだけEP/H1足だったので統一
  var parts = _elHoldFinalParts(s, _a, _cl);   // { main:（）外, ref:（）内差分 } ＝想定損益（手じまい・○が途切れた所で手仕舞い）＝記録表/損益データ欄の「想定損益」と同一基準。2026-07-20b にH1(_elHold1TotParts)から切替＝アプリ全体(07-09〜07-13で想定損益へ移行)・推奨α選定(_elBaseAlphaPick=_elH2EvalByFn)と基準を統一。α可変でも正しい（内部で_epResolve(s,alpha)を引く）
...
function _elKabuAccumTiers(tiers, evalOf, weightOf, collectCells, ctx) {
  var recPnl = 0, recRef = 0, any = false, hasMain = false, anyStop = false, anyIndet = false, anyNoBase = false, anyX = false, anyUnreached = false, anyFillEq = false;
  ...
  for (var i = 0; i < tiers.length; i++) {
    ...
    if (ev.skip === "fillEq" && w > 0) anyFillEq = true;   // 2026-08-06 オプション「指値同値は建てない」で落とした段
  ...
  return { recPnl: recPnl, recRef: recRef, any: any, hasMain: hasMain, anyStop: anyStop, anyIndet: anyIndet, anyNoBase: anyNoBase, anyX: anyX, anyUnreached: anyUnreached, anyFillEq: anyFillEq, cells: cells };
}
// 建玉ゼロの記録に「建たなかった理由」を1つだけ割り当てる（銘柄別内訳が 対象＝建玉あり＋各理由 でぴったり合うように排他化）2026-07-20f。
// 優先順 x > noalpha > fillEq > unreached: ×見送りはαもEP到達も確定したうえでの意思決定＝最も情報が確か／α不明はその方式が適用できないデータ側の問題／
//   指値同値は「到達はしたが約定しなかった可能性」＝到達済みなので未到達より情報が細かい／どちらでもなければ純粋にαへ届かなかった。
function _elKabuNoBuildReason(acc) {
  if (!acc || acc.any) return null;
  if (acc.anyX) return "x";
  if (acc.anyNoBase) return "noalpha";
  if (acc.anyFillEq) return "fillEq";   // 2026-08-06
  if (acc.anyUnreached) return "unreached";
  return "other";   // 有効な取引行が1つも無い（株数0・方式未選択）等
}
function _elKabuLadderCalc(recs, aiOf, tiersOf, touchOf) {
  var rows = [], sum = 0, sumRef = 0, builtRecN = 0, stopRecN = 0, indetRecN = 0, xRecN = 0, noBaseRecN = 0, unreachedRecN = 0, fillEqRecN = 0;
  (recs || []).forEach(function(r) {
    var s = r && r.signal; if (!s) return;
    var cut = aiOf(r).cutLine;
    var tiers = tiersOf(r) || [];
    var _tA = touchOf ? touchOf(r) : null;   // 2026-08-06 「指値同値は建てない」用の触れただけα（オプションOFF/対象外はnull＝従来どおり）
    var acc = _elKabuAccumTiers(tiers, function(t) { return _elKabuTierEval(s, t.a, (t._cut != null ? t._cut : cut), _tA); }, _elKabuTierAdd, true);   // t._cut=取引ごと損切り(EPからの距離・手動/A/B比較)。無指定tier(自動配分)は記録の損切り(cut)へフォールバック 2026-07-24
    ...
    if (_nbr === "fillEq") fillEqRecN++;   // 2026-08-06 指値同値だけで建玉ゼロになった記録
  ...
  return { rows: rows, sum: Math.round(sum), sumRef: Math.round(sumRef), builtRecN: builtRecN, stopRecN: stopRecN, indetRecN: indetRecN, xRecN: xRecN, noBaseRecN: noBaseRecN, unreachedRecN: unreachedRecN, fillEqRecN: fillEqRecN, n: rows.length };
}
...
function _elKabuLadderSimV2(props) {
  ...
  // 2026-07-24 手動の損切りは取引ごと(rows[].stop=「(合計)α値から〇円上」)へ移行＝config単位のstopOff stateは廃止。既定15。
  ...
  // ===== 指値同値（触れただけα）2026-08-06 =====
  // その記録で「実効αがこの値ちょうどなら約定しなかった可能性が高い」α＝round(_elOsMaxAll)。判定規則は注記の_fillEqNと完全に同一（＝可視化と除外で二重定義しない）:
  //   ・OS高値の最大が無い記録は対象外（null）。
  //   ・その記録の採用αと一致し、かつ実際に約定している(_elIsEntered)なら「刺さった証拠がある」ので対象外（null）＝採用α±0で記録どおりを再現する不変条件を壊さない。
  // オプションOFFなら_touchOf自体をnullにして従来の経路に戻す（_elKabuTierEvalの第4引数未指定と同じ）。
  var _touchAlphaOf = function(r) {
    var s = r && r.signal; if (!s) return null;
    var osMax = _elOsMaxAll(s); if (osMax == null) return null;
    var om = Math.round(osMax), adoptA = aiOf(r).alpha;
    if (_elIsEntered(s, r.item) && adoptA != null && Math.round(adoptA) === om) return null;
    return om;
  };
  var _touchOf = fillEqEx ? _touchAlphaOf : null;
  var _fqPoolN = 0; pool.forEach(function(r) { if (_elFillRiskRec(r)) _fqPoolN++; });   // 母数内で「採用α時点の指値同値」に当たる記録数＝オプションの目安表示（実際の除外は掃引αで再判定するので一致するとは限らない）
  // ===== 手動ラダー／A/B比較 の共通コア（実効α・損切り・株数の単一源）2026-07-24 =====
  // ⚠️実効αの算出は _cfgAlphaOf に一本化＝手動の_manTiersOfとA/B比較(_mkCmpCfg)が _cfgTiers 経由で共有。ドリフト（片方だけ直る事故）を構造的に防ぐ。ここを直せば両モードに一律で効く。
  // _cfgAlphaOf(rw,r)→{a(実効α・null=算出不可だが建てる=noalpha),_uki,_rn}｜null(=この記録では建てない＝adopt未入力)。株数・損切りは無関係（③「第1と同じ損切りライン」で第1の実効αを引くのに再利用）。
  var _cfgAlphaOf = function(rw, r) {
    if (rw.method === "adopt") { var ad = _adoptOf(r); if (ad == null) return null; var oa = _kbInt(rw.off); return { a: Math.max(0, ad + (oa == null ? 0 : oa)), _uki: 0, _rn: 0 }; }   // 採用α値±X（合計α起点・浮き足/RN込みなので基底置換も浮き足加算も通さない）
    var _uki = _elUkiAdd(r.signal);
    var a;
    ...
    else if (rw.method === "reco") { var b2 = _recoOfRec(r); if (b2 == null) { a = null; } else { var o = _kbInt(rw.off); a = Math.max(0, b2 + (o == null ? 0 : o)); } }   // 0未満は0クランプ
    else { a = null; }   // 未選択（method空）
    var _bl = _simBaseLevel(r, a);   // 応用〇＝specialAlphaで基底α置換／浮き足〇＝基底0／候補null＝null
    if (_bl == null) return { a: null, _uki: _uki, _rn: 0 };   // 未選択/推奨α不明等→a=null（noalphaタイル）
    var _pre = _bl + _uki, _rnD = rnAuto ? _simRnAt(r, _pre) : 0;   // 実効α＝基底α＋浮き足加算＋自動RN加算
    return { a: _pre + _rnD, _uki: _uki, _rn: _rnD };
  };
  // 1取引×1記録→tier {a,add(株数),_uki,_rn,_addA,_cut(取引ごと損切り=EPからの距離・null=記録の損切り値)}｜null。_cut=各取引の「(合計)α値から〇円上」の〇（rw.stop）。
  var _cfgTierOfRow = function(rw, r) {
    var sh = _kbInt(rw.cum); if (sh == null || sh <= 0) return null;   // 株数無し＝建てない
    var al = _cfgAlphaOf(rw, r); if (al == null) return null;   // adopt未入力＝建てない
    return { a: al.a, add: sh, _uki: al._uki, _rn: al._rn, _addA: null, _cut: _kbInt(rw.stop) };
  };
  ...
  // eRows→tier配列。③「第1と同じ損切りライン」対応: 第2取引以降でrw.stopSame=trueなら、その取引の損切り価格を第1取引と同じライン(高さ)に揃える（建値EPは各自のまま）。
  //   第1の損切りライン(水準線からの距離)=実効α₁+第1の損切り値X₁（X₁空欄は記録の損切り値）。第2の_cut(EPからの距離)=max(1, 損切りライン₁−第2の実効α)。
  var _cfgTiers = function(eRows, r) {
    var stopLine1 = null;   // 第1取引の損切りライン（水準線からの距離）
    if (eRows.length) { var a0 = _cfgAlphaOf(eRows[0], r); if (a0 && a0.a != null) { var x1 = _kbInt(eRows[0].stop); stopLine1 = a0.a + (x1 != null ? x1 : aiOf(r).cutLine); } }
    var out = [];
    eRows.forEach(function(rw, idx) {
      var t = _cfgTierOfRow(rw, r); if (!t) return;
      if (rw.stopSame && idx > 0 && stopLine1 != null && t.a != null) t._cut = Math.max(1, stopLine1 - t.a);   // 第1と同じ損切りライン＝EPからの距離に換算
      out.push(t);
    });
    return _tierSort(out);
  };
  ...
  // A/B比較: 1つの設定（rows配列）→ {eRows, calc, manTot}。手動と同じ_cfgTiers/_cfgTot/aiOfを通す＝手動・設定A・設定Bで実効α・損切りが完全に一致。損切りは取引ごと(rows[].stop)。multiTradeは共通（×なら先頭1取引だけ）。
  var _mkCmpCfg = function(cfgRows) {
    var eRows = multiTrade ? cfgRows : cfgRows.slice(0, 1);
    var manTot = _cfgTot(eRows);
    var calc = pool.length ? _elKabuLadderCalc(pool, aiOf, function(r) { return _cfgTiers(eRows, r); }, _touchOf) : null;   // 損切りは取引ごと(_cut)。_cut無しtierは記録の損切り(aiOf.cutLine)へフォールバック 2026-07-24
    return { eRows: eRows, calc: calc, manTot: manTot };
  };
  ...
  // 2026-07-24 実効α＝_cfgAlphaOf・tier化＝_cfgTiers に共通化（A/B比較と共有）。③第2取引以降の「第1と同じ損切りライン」も_cfgTiersが処理。α不明tierも捨てず_elKabuTierEvalへ渡す（skip:"noalpha"・消費側はc.t.a!=nullでガード）。
  var _manTiersOf = function(r) { return _cfgTiers(effRows, r); };
  ...
  var manCalc = (mode === "manual" && pool.length) ? _elKabuLadderCalc(pool, aiOf, _manTiersOf, _touchOf) : null;   // 損切りは取引ごと(_manTiersOfの_cut)。空欄の取引は記録の損切り(aiOf.cutLine)へフォールバック 2026-07-24
  ...
  var _autoSig = totalN + "|" + pool.length + "|" + _elPSelSig(pSel) + "|" + addFil + "|" + (exFlags.uki ? 1 : 0) + "|" + _elAnaCutCur + "|" + (floatMode ? 1 : 0) + "|" + recoSig.length + "|" + (multiTrade ? 1 : 0) + "|" + (rnAuto ? 1 : 0) + "|" + stkFil + "|" + bandFil + "|" + (fillEqEx ? 1 : 0);   // 指値同値オプションも署名に 2026-08-06（切り替えたら総当たりの結果が変わる）   // multiTrade/rnAutoも署名に＝切り替えたら再計算が必要 2026-07-20g→2026-07-21a。bandFilも 2026-08-02（帯を切り替えたら自動配分の再計算が必要）
  ...
  if (mode === "auto" && totalN > 0 && pool.length && _autoReady) {
    ...
    var _touchAs = pool.map(function(r) { return _touchOf ? _touchOf(r) : null; });   // 2026-08-06 記録ごとに1回だけ算出（ホットループで_elOsMaxAllを毎回引かない）。_evCacheはレンダーごとに作り直すのでオプション切替でキャッシュが残る心配はない
    var _evAt = function(pi, a) { var m = _evCache[pi]; if (!m.hasOwnProperty(a)) m[a] = _elKabuTierEval(pool[pi].signal, a, aiOf(pool[pi]).cutLine, _touchAs[pi]); return m[a]; };
  ...
  var _mtTierNodes = function(cells) {
    ...
    (cells || []).forEach(function(c, j) {
      ...
      else if (ev.skip === "fillEq") txt = React.createElement("span", { style: { color: "#0F6E56" } }, "α" + ev.a + "円×" + c.t.add + "株 指値同値");   // 2026-08-06 オプションONで建てなかった段
  ...
  var _kbConvParts = function(r, nSh) {
    ...
    var _cai = aiOf(r);
    var p = _elHoldFinalParts(s, _cai.alpha, _cai.cutLine);   // 想定損益（手じまい）＝記録表の「想定損益」列と同一関数。2026-07-20b にH1から切替＝「従来」列が記録表と同じ数字になる
  ...
  var _KB_NB = [["unreached", "未到達", "#B45309", "#FFF7ED", "#FED7AA"], ["x", "×見送り", "#1E8449", "#F0FDF4", "#BBF7D0"], ["fillEq", "指値同値", "#0F6E56", "#E1F5EE", "#5DCAA5"], ["noalpha", "α不明", "#6B7280", "#F3F4F6", "#D1D5DB"], ["other", "取引なし", "#6B7280", "#F3F4F6", "#D1D5DB"]];   // fillEq＝2026-08-06追加（色は明細行の指値同値バッジ_elFillRiskNodeと同じ）
  var _kbStockTable = function(calc, nShares) {
    ...
    calc.rows.forEach(function(row) {
      ...
      if (!by[st]) { by[st] = { stock: st, n: 0, built: 0, sim: 0, simHas: false, conv: 0, convHas: false, unreached: 0, x: 0, fillEq: 0, noalpha: 0, other: 0 }; order.push(st); }
      var g = by[st];
      g.n++;
    ...
    var _nbCell = function(g) {
      var chips = _KB_NB.filter(function(k) { return g[k[0]] > 0; }).map(function(k) {
        return React.createElement("span", { key: k[0], title: k[0] === "unreached" ? "掃引αにOS1〜3で届かなかった＝αを下げれば拾える可能性" : k[0] === "x" ? "×見送り宣言後の到達＝実際には降りているので建てない" : k[0] === "fillEq" ? "実効αがOS高値の最大とちょうど一致＝予定EPに触れただけで指値が約定しなかった可能性（オプション「指値同値は建てない」がON）" : k[0] === "noalpha" ? "その銘柄の推奨αが出せない（記録が薄い）／採用α未入力＝その取引は建てない" : "有効な取引行が無い（株数0・方式未選択）",
  ...
  var _kbMasterTable = function(cfgCalc, cfgLabel, nShares) {
    ...
    cfgCalc.rows.forEach(function(row, i) {
      ...
      var _sa = []; (row.cells || []).forEach(function(c) { if (c.t.add > 0 && c.t.a != null && !isNaN(c.t.a)) { var av = Math.round(c.t.a); if (_sa.indexOf(av) < 0) _sa.push(av); } }); _sa.sort(function(p, q) { return p - q; });   // この配分(シミュ)が各記録で使ったα。採用αとの差の理由を可視化 2026-07-03
      var _sc = []; (row.cells || []).forEach(function(c) { if (c.t.add > 0 && c.t.a != null) { var cv = (c.t._cut != null ? c.t._cut : aiOf(r).cutLine); if (cv != null) { cv = Math.round(cv); if (_sc.indexOf(cv) < 0) _sc.push(cv); } } }); _sc.sort(function(p, q) { return p - q; });   // 2026-07-24 シミュ損切＝取引ごとの_cut(EPからの距離)。無指定は記録の損切り値
      ...
      mrecs.push({ oi: i, r: r, s: s, a: aiOf(r).alpha, recoA: _recoOfRec(r), simA: _sa, simUki: _exU, simRn: _exR, simAddOn: _addAs, simCuts: _sc, cfgPnl: row.recPnl, cfgRef: row.recRef, basePnl: _cv.main, baseRef: _cv.ref, cells: row.cells, anyStop: row.anyStop, noBuild: row.noBuild });   // noBuild=建たなかった理由（2026-07-20k・シミュ列に表示）
    });
  ...
  var _notesLine = function(calc) {
    ...
    if (_fq) parts.push("同値除外損益" + _fq + "件（掃引αがOS高値の最大とちょうど一致＝予定EPに触れただけで約定しなかった可能性あり" + (fillEqEx ? ("・オプションONにつきその段は建てていません／うち建玉ゼロ" + (calc.fillEqRecN || 0) + "件") : "・損益からは除外していません") + "）");   // 2026-08-06 オプション「指値同値は建てない」の状態で文言を切替（件数の数え方＝_fillEqNはON/OFFで不変＝該当記録数そのもの）
  ...
  var _mkRowOps = function(cRows, cSet) {
    return {
      upd: function(i, patch) { cSet(cRows.map(function(x, j) { return j === i ? Object.assign({}, x, patch) : x; })); },
      stepCum: function(i, d) { cSet(function(prev) { return prev.map(function(x, j) { if (j !== i) return x; var c = _kbInt(x.cum); c = (c == null ? 0 : c) + d; if (c < 0) c = 0; return Object.assign({}, x, { cum: String(c) }); }); }); },
      stepOff: function(i, d) { cSet(function(prev) { return prev.map(function(x, j) { if (j !== i) return x; var c = _kbInt(x.off); c = (c == null ? 0 : c) + d; if (x.method === "abs" && c < 0) c = 0; return Object.assign({}, x, { off: String(c) }); }); }); },
      stepStop: function(i, d) { cSet(function(prev) { return prev.map(function(x, j) { if (j !== i) return x; var c = _kbInt(x.stop); c = (c == null ? 0 : c) + d; if (c < 0) c = 0; return Object.assign({}, x, { stop: String(c) }); }); }); }
    };
  };
  var _prevOf = function(eRows) {
    var parts = [], tot = 0;
  ...
  // 1取引の編集行。o={upd,stepCum,stepOff,stepStop,trash,accent}。i=取引index（0=第1取引）。
  var _cfgRowRender = function(rw, i, o) {
    var m = rw.method;
    var _sel = React.createElement("select", { value: m || "", onChange: function(e) { o.upd(i, { method: e.target.value }); }, style: { padding: "3px 6px", fontSize: 10.5, fontWeight: 700, color: m ? "#0F766E" : "#aaa", border: "1px solid #ddd", borderRadius: 5, background: "#fff" } }, [React.createElement("option", { key: "_n", value: "" }, "（未選択）")].concat(_METHODS.map(function(mm) { return React.createElement("option", { key: mm.key, value: mm.key }, mm.short); })));
    var _cumF = _cfgStepField(rw.cum, function(e) { o.upd(i, { cum: e.target.value }); }, function() { o.stepCum(i, 100); }, function() { o.stepCum(i, -100); });
    var _inputPart;
    ...
    return React.createElement("div", { key: i, style: { border: "1px solid #e5e7eb", borderRadius: 8, padding: "6px 8px", marginBottom: 6, background: "#fff" } },
      React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 } },
        React.createElement("span", { style: { fontSize: 11, fontWeight: 800, color: o.accent, whiteSpace: "nowrap" } }, multiTrade ? ("第" + (i + 1) + "取引") : "取引"),
  ...
  var _kbCmpTable = function(cA, cB) {
    ...
    var _scOf = function(cells, r) { var sc = []; (cells || []).forEach(function(c) { if (c.t.add > 0 && c.t.a != null) { var cv = (c.t._cut != null ? c.t._cut : aiOf(r).cutLine); if (cv != null) { cv = Math.round(cv); if (sc.indexOf(cv) < 0) sc.push(cv); } } }); sc.sort(function(p, q) { return p - q; }); return sc; };   // 取引ごと損切り(EPからの距離)
    var recs2 = [];
  ...
  // A/B比較の1設定ぶんの編集カラム。tag="A"/"B"・cfg=_mkCmpCfg結果。行UIは手動と共通の_cfgRowRender（方式別入力＋取引ごと損切り＋第1と同じ損切りライン）を使う。
  var _cmpCol = function(tag, cfgRows, cfgSet, cfg, accent, bg) {
    var eRows = cfg.eRows;
    var _ops = _mkRowOps(cfgRows, cfgSet);
    var _addRow = function() { cfgSet(cfgRows.concat([{ method: "", off: "0", cum: "100", stop: "15", stopSame: false, addMethod: "act", addOff: "" }])); };
    ...
    return React.createElement("div", { style: { flex: "1 1 300px", minWidth: 268, border: "2px solid " + accent, borderRadius: 10, padding: "8px 10px", background: bg } },
      React.createElement("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 6, marginBottom: 6, flexWrap: "wrap" } },
        React.createElement("span", { style: { fontSize: 12, fontWeight: 800, color: accent } }, "設定" + tag), _totNode),
      eRows.map(function(rw, i) { return _cfgRowRender(rw, i, Object.assign({ accent: accent, trash: (multiTrade && eRows.length > 1) ? function() { cfgSet(cfgRows.filter(function(x, j) { return j !== i; })); } : null }, _ops)); }),
      ...
      React.createElement("div", { style: { fontSize: 9, color: "#666", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 6, padding: "3px 7px" } }, _prevOf(eRows)));
  };
  // ===== 本体 =====
  ...
  // ===== ⚙️ オプションセクション 2026-08-06（ユーザー要望）=====
  // 上の絞り込みピル（期間/銘柄/株価帯/分類）＝「何を対象にするか」に対し、こちらは「どう建てるか」のルール。
  // 同じ高さで縦に並んでいて読み分けられなかったので、除外(浮き足〇)・指値同値・時間かぶり・RN自動加算を1枠へ集約した（＝旧「除外:」「RN自動加算:」行の移設）。
  // 閉じても要約行に現在の設定が出るので、既定ONの浮き足除外が隠れて見えなくなることはない。
  var _optChip = function(on, label, onClick, color, bg, title) {
    return React.createElement("button", { type: "button", onClick: onClick, title: title || null,
      style: { padding: "3px 10px", fontSize: 10.5, fontWeight: 700, borderRadius: 6, cursor: "pointer", border: "1px solid " + (on ? color : "#ddd"), background: on ? bg : "#fff", color: on ? color : "#666", whiteSpace: "nowrap" } }, (on ? "☑ " : "☐ ") + label);
  };
  var _optBadge = function(txt, color, bg, bd, title) { return React.createElement("span", { title: title || null, style: { fontSize: 9.5, fontWeight: 800, color: color, background: bg, border: "1px solid " + bd, borderRadius: 5, padding: "1px 7px" } }, txt); };
  var _optNote = function(txt) { return React.createElement("span", { style: { fontSize: 9, color: "#aaa" } }, txt); };
  var _optRow = function(key, label, color) {   // 第4引数以降が行の中身（可変長＝配列で渡さないのでkey警告が出ない）
    var kids = Array.prototype.slice.call(arguments, 3).filter(function(x) { return x != null; });
    return React.createElement.apply(null, ["div", { key: key, style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", marginBottom: 5 } },
      React.createElement("span", { style: { fontSize: 10, fontWeight: 800, color: color, minWidth: 74 } }, label)].concat(kids));
  };
  var _optSummary = [
  ...
  var _optSection = React.createElement("div", { style: { border: "1px solid #DDE5EC", borderRadius: 8, background: "#FBFDFE", padding: "5px 8px", marginBottom: 6 } },
    React.createElement("div", { onClick: function() { setOptOpen(!optOpen); }, style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", cursor: "pointer" } },
      React.createElement("span", { style: { fontSize: 10.5, fontWeight: 800, color: "#334155", whiteSpace: "nowrap" } }, "⚙️ オプション"),
      React.createElement("span", { style: { fontSize: 9, color: optOpen ? "#aaa" : "#0F766E", fontWeight: optOpen ? 400 : 700, flex: "1 1 120px" } }, optOpen ? "母数の絞り込みではなく「どう建てるか」のルール" : _optSummary),
    ...
    optOpen ? React.createElement("div", { style: { marginTop: 6, borderTop: "1px dashed #E2E8F0", paddingTop: 6 } },
      ...
      floatMode ? null : _optRow("uki", "除外:", "#B45309",
        ...
        exFlags.uki
          ...
          : _optNote("浮き足〇の記録をシミュ母数から除外（現在 " + pool.length + "件）")),
      // ② 指値同値は建てない 2026-08-06。①と違い母数（件数）は減らさず、その段だけ建てない＝掃引αで毎回判定し直す（保存値のフラグではない）。
      _optRow("fq", "指値同値:", "#0F6E56",
        _optChip(fillEqEx, "建てない", function() { setFillEqEx(!fillEqEx); setAutoExp(null); setMtExp(null); }, "#0F6E56", "#E1F5EE", "実効αがOS高値の最大とちょうど一致した段＝予定EPに触れただけで指値が約定しなかった可能性が高い段を建てない。判定は掃引αで毎回やり直すので、αを下げれば同じ記録が普通に建ち、αを上げれば未到達になる。実際に約定した記録（採用αで刺さっている）は対象外。"),
        fillEqEx
          ? _optBadge("EPちょうど到達の段を建てない", "#0F6E56", "#E1F5EE", "#5DCAA5")
          : _optNote("実効αがOS高値の最大とちょうど一致した段を建てない（＝予定EPに触れただけ）"),
        _fqPoolN > 0 ? _optBadge("母数内 " + _fqPoolN + "件", "#0F6E56", "#F0FDFA", "#99E2CB", "採用α時点で指値同値だった記録の数（目安）。実際の除外は掃引αで再判定するので、この件数と一致するとは限りません。") : null,
        fillEqEx ? _optNote("母数（件数）は減らさず、その段だけ建てません") : null),
      // ③ 時間かぶり除外＝他のP&L集計(_elTotAccum の excluded)と同じ線引きを常時適用（トグルではない）2026-07-20b。0件のときは行ごと出さない。
  ...
  var body;
  if (mode === "manual") {
    var _ops = _mkRowOps(rows, setRows);
    var _addRow = function() { setRows(rows.concat([{ method: "", off: "0", cum: "100", stop: "15", stopSame: false, addMethod: "act", addOff: "" }])); };   // 2026-07-24 空行を足して行内のセレクトで方式を選ぶ（旧: 方式ピッカー）
    var _manTot = _cfgTot(effRows);   // 手動ラダーの合計株数＝各取引の株数の総和
    ...
    body = React.createElement(React.Fragment, null,
      ...
      React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8, alignItems: "stretch" } },
        _cmpCol("A", rowsA, setRowsA, _cfgA, "#0F6E56", "#F0FDFA"),
        _cmpCol("B", rowsB, setRowsB, _cfgB, "#185FA5", "#F3F8FE")),
    ...
    body = React.createElement(React.Fragment, null,
      ...
      _autoGate ? null : (function() {   // 選択配分のマスター表＋注記行（判定不可/推奨α不明/×見送り件数＝手動と同じ_notesLine・従来は自動配分に注記が無かった）2026-07-04c
        ...
        var _selCalc = _elKabuLadderCalc(pool, aiOf, _autoTiersOfFor(_selCb), _touchOf);
        return React.createElement(React.Fragment, null,
    ...
    React.createElement("div", { style: { display: "inline-flex", background: "#DDEDE9", borderRadius: 6, padding: 2, gap: 2 } },
      [[true, "〇"], [false, "×"]].map(function(kv) {
        ...
        return React.createElement("button", { key: String(kv[0]), type: "button",
          ...
          onClick: function() { if (multiTrade === kv[0]) return; setMultiTrade(kv[0]); setAutoExp(null); setMtExp(null); setAddPicker(false); if (kv[0] && rows.length < 2) setRows(rows.concat([{ method: "", off: "", cum: "", stop: "15", stopSame: false, addMethod: "act", addOff: "" }])); },
...
function EntryLogView(_ref_elv2) {
  ...
  var _SIG_TABS = [["band", "💴 株価帯別"], ["stop", "🛑 損切り"], ["spn", "🩹 補正要否"], ["uki", "⚡ 浮き足%"], ["rn", "🔢 RN加算"]];   // 📡シグナル総合のサブタブ 2026-07-12（時間帯/曜日は2026-07-16撤去＝ユーザー不要）。RN→RN加算改名 2026-07-19。株価帯別を浮き足%の左へ移設 2026-07-22i（旧・全銘柄集計の分析軸トグルから移動）。損切りを株価帯別の右に追加 2026-07-27（銘柄別タブの🛑損切りは存続＝両方で見る・全銘柄側は株価帯で区切る＝円建ての損切り値を銘柄横断で混ぜても意味が壊れないように）
  ...
  // 損益（期間別）テーブル＝全銘柄合算をday/week/monthで集計。各損益セルに合計＋平均を併記・損切り(件数/平均額/率)列・行タップでその期間の取引記録を展開。「損益」タブの集計ビュー頭 2026-06-22d。損益基準は_elTotAccum（取引/銘柄別記録と同一）。2026-07-09 EP損益/H1損益列を廃し「想定損益」1列に集約（＝旧H2損益・_elHoldFinalPartsの（）外=○が途切れた所で手じまい/（）内=△含む・値は不変）。
  ...
  // （想定損益・指値同値＝損益スケール／実現損益＝その10倍）ので2段で出す必要がある。
  // ラベルは2026-08-05mのユーザー指示で「想定損益」「実現損益」まで短縮した
  // （指値同値列は想定損益と同じスケールなので上段に含まれる／10倍は数字を見れば分かる）。
  ...
  var _ovGradeLegend = React.createElement("div",
    ...
    _gradeLegendGrid([{ label: "想定損益", real: false }, { label: "実現損益", real: true }], { lowerOnly: true }),
    React.createElement("div", { title: "各グレードは「その期間の合計 ÷ 営業日数（日数列）」で判定しています。表示している金額は合計なので、合計額をそのままグレード表に当てても一致しません。想定損益・同値除外損益・実現損益・（）内の参考値すべて同じ規約です",
  ...
  //  想定損益(_elHoldFinalParts.main＝利確列・想定損益列と同じ基準)の実額平均にする。終値撤退方式では
  ...
  // 利確: E成立(EP到達して決着＝損切り率と同母数wn)のうち想定損益(（）外main)>0で手じまいした件数・率 2026-07-09
  ...
  // 集計ルール自体が変わっている（07-09 EP/H1損益を廃し「想定損益」1列へ集約／07-27 損切りの平均額を理想値→終値撤退の実額へ）ため、
  ...
  // 1記録の想定損益（（）外main）。集計対象外ならnull。🏷銘柄別の損益割合と🎯グレード別の件数が**同じ除外規約**
  ...
  //  ②トレード単位: 記録1件ごとの想定損益を_profitGradeFromPnl（損益スケール S=2501円〜）で判定し月別に数える。
  //     合計＝その月に金額が出た記録数＝🏷銘柄別の「総利益件数＋総損失件数＋D(同値)」。_elFinalPnlOf を共有しているので食い違いようがない。
  //  ①日単位: その日の想定損益合計を同じスケールで判定し、月に何日がA+だったかを数える。
  ...
  var _gradeMonSection = function(rs) {
    ...
    rs.forEach(function(r) {
      var v = _elFinalPnlOf(r); if (v == null) return;
      var ds = String((r && r.date) || ""); if (!ds) return;
      var mk = ds.slice(0, 7), mm = (recM[mk] = recM[mk] || {}), g = _profitGradeFromPnl(v, 1);
      mm[g] = (mm[g] || 0) + 1;
      var d = (dayAcc[ds] = dayAcc[ds] || { sum: 0, n: 0 }); d.sum += v; d.n++;
    });
    Object.keys(dayAcc).forEach(function(ds) {
      var d = dayAcc[ds], mk = ds.slice(0, 7), mm = (dayM[mk] = dayM[mk] || {});
      var g = _profitGradeFromPnl(d.sum, d.n); mm[g] = (mm[g] || 0) + 1;
    });
    var yms = Object.keys(recM).sort().reverse();   // 新しい月が上＝「全体損益（期間別）」の並びに合わせる
    if (!yms.length) return null;
    var _pct = function(n, d) { return d > 0 ? Math.round(n / d * 100) + "%" : "—"; };
    // ヘッダは丸バッジ（_elHoldGradeBadge＝記録一覧の損益バッジと同じ見た目）＋しきい値。列の意味を凡例なしで読めるようにする。
    var _head = [_elv2Th("月")].concat(_EL_GRADES.map(function(g) {
      return React.createElement("th", { key: g, style: { padding: "4px 5px", borderBottom: "1px solid #E4DFD7", whiteSpace: "nowrap", textAlign: "center" } },
        React.createElement("div", null, _elHoldGradeBadge(g)),
        React.createElement("div", { style: { fontSize: 7.5, color: "#A79E92", fontWeight: 600, marginTop: 1 } }, _GRADE_DESC[g]));
    })).concat([_elv2Th("合計")]);
    // セル背景にグレード色(_GRADE_STYLE.bg＝A側=赤系/G側=緑系)を敷く＝0件は白のまま残るので分布の形がそのまま見える。
    var _tbl = function(m, unit) {
      var tot = {}, totN = 0;
      var _cellsOf = function(label, g2, n) {
        var cells = [_elv2Td(label, { textAlign: "left", paddingLeft: 8, fontWeight: 700, color: "#334155" })];
        _EL_GRADES.forEach(function(g) {
          var c = g2[g] || 0;
          cells.push(_elv2Td(c
            ? React.createElement("span", null,
                React.createElement("span", { style: { fontWeight: 700, color: "#334155" } }, c + unit),
                React.createElement("div", { style: { fontSize: 8.5, color: "#94A3B8" } }, _pct(c, n)))
            : React.createElement("span", { style: { color: "#CBD5E1" } }, "—"),
            c ? { background: (_GRADE_STYLE[g] || {}).bg } : null));
        });
        cells.push(_elv2Td(React.createElement("span", { style: { fontWeight: 700, color: "#334155" } }, n + unit)));
        return cells;
      };
    ...
    var _cap = function(t, sub) {
      return React.createElement("div", { style: { margin: "12px 0 0" } },
    ...
    return React.createElement("div", null,
      _cap("① 日単位（1日＝1件）", "その日の想定損益の合計をグレード判定。合計＝その月に記録があった日数"),
      _tbl(dayM, "日"),
  ...
  var _ovPnlTbl = function(rs, g) {
    ...
    var _keyIsOld = function(k, gg) {
      var h = _keyHalf(k);
      if (h) return h === "1";
      return (gg || g) === "month" ? (String(k) < _RULE_MONTH) : (String(k) < _RULE_SINCE);
    };
    var keyOf = function(ds, gg) {
      var _g = gg || g, k = _granKeyOf(ds, _g);
      return (_g === "month" && k === _RULE_MONTH) ? (k + (_isOldRec(ds) ? "#1" : "#2")) : k;
    };
    var labelOf = function(k, gg) {
      var h = _keyHalf(k);
      if (!h) return _granLabelOf(String(k), gg || g);
      return _granLabelOf(_baseKey(k), "month") + (h === "1" ? "（〜" + _RULE_PREV_LBL + "）" : "（" + _RULE_FROM_LBL + "〜）");
    };
    ...
    //   母数rsは一切触らない＝件数/到達/利確/損切り/想定損益/実現/1日平均/合計行は不変（「表示だけ」の据置方針そのまま）。
    ...
    // 2026-07-30b 横スクロール解消（ユーザー要望「幅を詰めれるのでは？」）: 表幅を決めていたのはヘッダの小書きだった
    //   （thが whiteSpace:nowrap なので「○が途切れた所で手じまい・( )=△含む」等が1行で列幅を押し広げていた＝10列で約1,042px）。
    //   2026-08-05n その小書き（_othSub）自体を撤去＝見出しだけにした（ユーザー「到達や利確などの直下の注記はすべて不要」）。
    //   説明は各列の title（ホバー）に残してあるので情報は失われない。左右paddingは4pxのまま。
    ...
    // 想定損益・実現損益とも「1日あたり平均」＝合計÷日数(営業日数)に統一 2026-07-09（旧: 実現損益は1トレード平均avgLine→ユーザー要望で1日平均に）
    // 2026-08-05f days<=1 は平均＝合計で同じ数字が2つ縦に並ぶだけなので出さない（ユーザー指摘
    // 「日別での1日平均額の表示は不要では？」）。粒度(g==="day")ではなく日数で判定しているので、
    // ①日別の各行は消える ②休場続きで1営業日しかない週/月の行も消える ③合計行(_ovTotDays＝
    // 全期間の営業日数)は日別表示中でも従来どおり出る、の3つが同時に成立する。
    ...
    var realCell = function(t, days, ex, noShare) {
      ...
      return otd(React.createElement("span", { style: { display: "inline-flex", flexDirection: "column", alignItems: "center", lineHeight: 1.15 } },
        ...
        p100 != null ? React.createElement("span", { title: "実現損益を1件ずつ100株換算（損益÷株数×100）して合計し、営業日数" + days + "日で割った額。バッジは想定損益と同じ通常スケール（S=2501円〜）＝上段の実額バッジ（10倍スケール）とは土俵が違います",
    ...
    var _stopGrp = function(lbl, n, rate, avg, col, sep, note) {
      return React.createElement("span", { style: Object.assign({ display: "block", lineHeight: 1.15 }, sep ? { borderTop: "1px dashed #E4DFD7", marginTop: 2, paddingTop: 2 } : null) },
        ...
        note ? React.createElement("span", { title: "撤退足の終値が未入力の記録は損益を出せないため、件数には入るが平均・合計からは外れる（ライン接触の事実を優先して損切りには算入＝保守側）", style: { display: "block", fontSize: 8.5, color: "#B45309", fontWeight: 700 } }, note) : null);
    };
    ...
    var evenCell = function(st, ex) {
      ...
      return otd(React.createElement("span", { style: { display: "inline-flex", flexDirection: "column", alignItems: "center", lineHeight: 1.15 } },
        ...
        st.unkN ? React.createElement("span", { title: "損切り以外で撤退足の終値が未入力＝想定損益を出せない記録。利確・損切り・損失・同値のどれにも入れられないため、ここに件数だけ出しています（終値を入れれば該当のバケツに入ります）", style: { fontSize: 9, color: "#B45309", fontWeight: 700 } }, "金額不明" + st.unkN + "件") : null), ex);
    };
    ...
    // 指値同値セル（想定損益の右・件数と除外後損益を1欄に統合 2026-07-20d ユーザー指示）:
    // 上＝該当件数（OS高値の最大＝採用α値＝予定EPを一度も上抜けなかった記録＝指値が約定しなかった可能性）＋対 件数の率、
    // 下＝その記録を除いた想定損益（除外後）。差額は小書きで併記（0＝該当無しのときは出さない＝想定損益と同額が一目で分かる）。
    ...
    var _headTr = function(gg) {
      return React.createElement("tr", { style: { background: "transparent" } },
        ...
        oth(React.createElement("span", { title: "利益（想定損益>0）で手じまいした件数と、E成立母数（＝到達）に対する率" }, "利確")),
    ...
    //   週別トグルなら 週→日→記録、日別トグルなら 日→記録（従来どおり）。入れ子でも列と計算式は同じ（_bizDaysIn/avgCntLine/reachAvg2 に段の粒度ggを渡すだけ）＝
    //   どの段でも同じ見方ができる。展開状態は ovExp（パス文字列のマップ）で段ごとに独立に持つ＝親を開いたまま子・孫を開ける。
    var _NEXT_GRAN = { month: "week", week: "day", day: null };
    var _lvlRows = function(gg, xs, exs, pfx) {
      var _by = {}, _exBy = {};
      xs.forEach(function(r) { var k = keyOf(r.date, gg); (_by[k] = _by[k] || []).push(r); });
      exs.forEach(function(r) { var k = keyOf(r.date, gg); (_exBy[k] = _exBy[k] || []).push(r); });
      var ks = Object.keys(_by).concat(Object.keys(_exBy).filter(function(k) { return !_by[k]; })).sort().reverse();
      var out = [];
      ks.forEach(function(k) {
        ...
        out.push(React.createElement("tr", { key: path + "_d" }, React.createElement("td", { colSpan: gg === "day" ? 9 : 10, style: { padding: "4px 6px 10px", background: "#FFFCF8", borderBottom: "2px solid #FB923C" } },   // 日別は日数列が無いので1つ減らす 2026-08-12g
          nxt
            ...
            ? React.createElement(React.Fragment, null,
                React.createElement("div", { style: { fontSize: 10, color: "#9A3412", fontWeight: 700, margin: "2px 0 4px" } },
                  labelOf(k, gg) + " の" + (nxt === "week" ? "週別" : "日別") + "（" + x.length + "件" + (_extraRow.length ? "＋表示のみ" + _extraRow.length + "件" : "") + "・行タップでさらに" + (nxt === "week" ? "日別" : "取引記録") + "）"),
                React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11, background: "#fff", border: "1px solid #F1E9DF", borderRadius: 6 } },
                  React.createElement("thead", null, _headTr(nxt)),
                  React.createElement("tbody", null, _lvlRows(nxt, x, _extraRow, path + ">"))))
            // 最小の段（日）＝その日の取引記録
            : React.createElement(React.Fragment, null,
                React.createElement("div", { style: { fontSize: 10, color: "#9A3412", fontWeight: 700, margin: "2px 0 4px" } }, labelOf(k, gg) + " の取引記録（" + x.length + "件" + (_extraRow.length ? "＋表示のみ" + _extraRow.length + "件" : "") + "）"),
                ...
                _recTable(x.concat(_extraRow).sort(_byDateAsc), "full", "ovp_" + path + "_", null,
    ...
    var _isRefKey = function(k) { return _keyIsOld(k, g); };
    var rsInc = rs.filter(function(r) { return !_isOldRec(r.date); });
  ...
  var _grpTable = function(groups, headLabel, keyPfx, withOsStats) {
    ...
    head = head.concat([_th("E成立率"), _th("損切り"), _th("想定損益"), _th("実現損益")]);
    return React.createElement(_HScrollBox, null,
      React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11 } },
        React.createElement("thead", null, React.createElement("tr", { style: { background: "transparent" } }, head)),
        React.createElement("tbody", null, rows)));
  ...
  // 記録プールを株価帯グループに割る単一源（💴株価帯別タブ＝_bandAxisBody と ⚡浮き足%タブの帯ピルで共用）2026-07-25。
  // 帯＝日×銘柄(_pbDayBandOf)・⚡固有材料日は帯から外して独立グループ・帯不明は末尾。実帯は0件でも表示／材料・不明は0件なら非表示（旧_bandAxisBody内のインライン実装と同一挙動）。
  var _pbSplitByBand = function(pool) {
  ...
  // _floatMode（シグナル別タブの state）に引きずられないこちらを直接使う。
  var _addFilPure = function(rs) {
  ...
  var _groupPanel = function(recs, stkKey, fixedRecs, useDet, bandSpan) {   // bandSpan: 株価帯別モード時、頻度列＋★頻度ゲートの分母を帯基準（_pbBandBizDays）に＝ピルと一致。非帯（シグナル別/詳細タグ別）はundefined＝記録スパン 2026-07-22j
    ...
    // デュアル評価（承認① 2026-07-12）: 想定損益基準の並走pick＋4月混入バッジ（承認③）。実母数（浮き足/RN/追加α除外）でバッジ件数を出す。
    ...
    return _cardify([
      ...
      (!_floatMode && osDistFil === "yes")
        ? [_elCard(_gDet ? _detCtlRow("gp_ba", _baRecs) : null,
            ...
            _secH("🔬 推奨応用α 詳細データ（応用〇・手仕舞い基準）", "応用〇の記録だけを母数に、独立α値0〜20円を手仕舞い基準で評価（★＝到達率" + _EL_ANA_REACH_DEF + "%以上［無ければ" + _EL_ANA_REACH_FLOOR2 + "%まで緩和し参考］・損切り率(最終)" + Math.round(_EL_BASE_MAX_STOPRATE * 100) + "%以下・E成立" + _EL_BASE_MIN_N + "件以上・頻度" + _EL_FREQ_MAX + "未満・黒字を満たすαのうちΣ想定損益（累計）最大）。母数＝応用〇（浮き足・RN除外）", _ctl("gp_baAdd", _baRecs)),
  ...
  if (_isSigTotal) {
    ...
    if (sigSub === "band") {
      ...
      // ⚡浮き足%の株価帯別 2026-07-25（ユーザー要望・案A）: タブ先頭に帯ピル［すべて／💴帯…／⚡材料あり／帯不明］。
      // 選んだ帯はKPI早見・加算率ボード・記録一覧の3つ全部に効く＝画面内で母数が食い違わない（ユーザー決定「全部そろえる」）。既定"all"＝従来の全銘柄共通の見え方のまま。
      // 帯グルーピングは_pbSplitByBand＝💴株価帯別タブと同一源（帯＝日×銘柄・⚡固有材料日は帯から外す）。
      // ボードへ渡す母数を_v2recsAllData→_ukiRecsに変更。_sigUkiPoolは_v2recsAllData（v2かつデータ算入）由来なのでボード内の再フィルタ結果は"all"時は従来と同一。
      var _ukiSplit = _pbSplitByBand(_sigUkiPool);
      var _ukiPills = [{ key: "all", label: "すべて", recs: _sigUkiPool }].concat(_ukiSplit.groups);
      ...
      var _altT = { cur: 0, alt: 0, diff: 0, n: 0, skip: 0, ne: 0 };
      _altRows.forEach(function(o) {
        if (o.alt == null || o.cur == null) { _altT.skip++; return; }   // 真の換算不可＝帯不明（水準線未入力）のみ
        _altT.n++; _altT.cur += o.cur; _altT.alt += o.alt; _altT.diff += o.diff;
        if (o.curWhy || o.altWhy) _altT.ne++;   // 片側でも無エントリー＝0円で算入した件数（内訳表示用）2026-07-27
      });
      var _altAmt = function(v) { return (v == null) ? React.createElement("span", { style: { color: "#ccc" } }, "—")
        : React.createElement("span", { style: { fontWeight: 700, color: _elPnlColor(v) } }, _elPnlFmt(v)); };
      // 無エントリー側は「0円」をグレーで＋理由バッジ（未達/スルー/△のみ）＝合計に0円で入っていることが行から読める 2026-07-27。
      var _altAmtWhy = function(v, why) {
        if (v == null || !why) return _altAmt(v);
        return React.createElement("span", null,
          React.createElement("span", { style: { fontWeight: 700, color: "#b5b0a8" } }, "0円"),
          React.createElement("span", { style: { display: "block", fontSize: 9, fontWeight: 700, color: "#c4bfb6" } }, why));
      };
      ...
      var _altBodyRows = _altRows.map(function(o, i) {
        ...
        return React.createElement("tr", { key: "alt" + i, style: { background: (o.diff != null && o.diff > 0) ? "#FFF7F5" : (o.diff != null && o.diff < 0) ? "#F4FBF5" : "transparent" } },
          ...
          _elv2Td(_altAmtWhy(o.cur, o.curWhy)),
          _elv2Td(_altAmtWhy(o.alt, o.altWhy)),
      ...
      _altBodyRows.push(React.createElement("tr", { key: "alttot", style: { background: "#FFFBF0", borderTop: "2px solid #FB923C" } },
        _elv2Td(React.createElement("span", { style: { fontWeight: 800 } }, "合計"), { textAlign: "left", paddingLeft: 8 }),
        _elv2Td(React.createElement("span", { style: { fontSize: 10, color: "#666" } }, _altT.n + "件" + (_altT.ne ? "（うち無エントリー0円算入 " + _altT.ne + "件）" : "") + (_altT.skip ? "（換算不可" + _altT.skip + "件除く）" : "")), { colSpan: 5 }),
      ...
      var _ukiBody = (ukiSub === "alt")
        ? _cardify([
            _secH("🔁 応用α換算（" + _ukiScopeLbl + "）", "※ 浮き足%を使わず「株価帯別の応用α＋RN加算」を採用αにしていたら各取引がどうなっていたかを再計算。帯は記録の水準線価格で判定（〜" + _EL_UKI_ALT_BANDS[0].max + "=" + _EL_UKI_ALT_BANDS[0].a + "円／" + (_EL_UKI_ALT_BANDS[0].max + 1) + "〜" + _EL_UKI_ALT_BANDS[1].max + "=" + _EL_UKI_ALT_BANDS[1].a + "円／" + (_EL_UKI_ALT_BANDS[1].max + 1) + "〜=" + _EL_UKI_ALT_BANDS[2].a + "円）。損切り値は記録の採用値のまま。損益は想定損益（手じまいまで）の（）外。エントリー不成立（α未達・スルー・（）外なしの△）は「そのαなら取引しなかった＝0円」として両側とも算入＝差額は全件で計算。水準線未入力（帯不明）の記録だけ換算不可"),
            _elv2CardRow([
            ...
            _elv2Table(["日付・時刻", "銘柄", "シグナル", "水準線（株価帯）", "実際の採用α", "換算α", "想定損益（実際）", "想定損益（換算）", "差額"], _altBodyRows)])
        : (ukiSub === "list")
        ? _cardify([
    ...
    if (_isAllStock) {
      ...
      _tabBody = _cardify([
        ...
        _sinceRecs.length ? [
          _secH("🎯 グレード別の件数（月別）",
            "想定損益を損益スケール（S=2501円〜／D=0円／G+=−2501円〜）でグレード判定して月ごとに数える。①その日の合計ごと ②記録1件ごと の2表。母数は" + _EL_SINCE_LBL + "の記録のみ＝上の「全体損益（期間別）」の合計行と同じ期間。境界をまたぐ月は「2026/06（6/29〜）」＝その月の一部だけを数えています。カレンダーの日付バッジは実現損益スケール（10倍・S=25001円〜）なので数字は一致しません",
            _elSinceBadge()),
          _gradeMonSection(_sinceRecs)] : null,
        _v2recsAmt.length ? _fillRiskSection(_v2recsAmt) : null,
        _v2recsNewRule.length >= 2 ? [
          _secH("📈 累積損益（記録順）", "想定損益/実現損益の累積推移・上の期間別表の合計行と同一基準（6/29より前＝旧ルール期間は除外）"), React.createElement(_elCumPnlSectionV2, { recs: _v2recsNewRule, aiOf: _ai, data: data, scopeStock: _collScope })] : null,
        _v2recsNewRule.length >= 2 ? [
    ...
    if (!_selSigRecs.length) {
      ...
      if (_alSel === "base") {
        _alBody = [
          ...
          _secH("🔬 推奨基本α 詳細データ", "推奨値が出た根拠＝α別の総当たり（各αのE成立/到達率/頻度/利確率/損切り率/想定損益）。基本αはシグナル共通母数で算出" + (_reasonSel !== "all" ? "（根拠「" + _reasonLabel + "」で絞込）" : ""), _detCtl("al_baD", _alReasonRecsFull)),
        ...
        _alBody = [
          ...
          _alphaTable ? [_secH("🎯 α意思決定表", "※実効α基準＝記録の採用α（浮き足/応用/RN込み）を一様にそのαへ置換して再計算（0〜20円・損切り値は各記録の採用値・★＝想定損益（手じまい）が最大のα）", _detCtl("al_tbl", _alReasonRecsScoped)),
    ...
    _tabBody = React.createElement(React.Fragment, null, React.createElement("div", { style: { fontSize: 10, color: "#9A3412", background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 10, padding: "5px 9px", marginBottom: 8 } }, "🎯 月間・週間タブはこの銘柄の全シグナル合算（時系列の俯瞰）。上のシグナル軸の選択では絞り込まれません。"), _elWeeklyTargetSummaryV2(_perRecs, _ai, data, _collScope), (function() {
      ...
      //   even＝想定損益ちょうど±0（利確>0・見切り<0のどちらにも入らない第4バケツ）／unk＝損切り以外で撤退足の終値が未入力＝金額を出せない記録。
      var _ratesOf = function(rs) {
        var ok = 0, ng = 0, miss = 0, stop = 0, soft = 0, draw = 0, take = 0, even = 0, unk = 0;   // take=利確(E成立かつ想定損益>0で手じまい) 2026-07-09
      ...
      var _periodKpi = function(rs) {
        ...
        return React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 8, marginBottom: 10 } },
          ...
          _kpiCard("想定損益", _yenNR(t.hold2, t.hold2Cnt, t.hold2Ref, t.hold2RefCnt, _pkDays), null, t.hold2Cnt + "件・○途切れで手じまい"),
      ...
      var _metInfo = { real: { label: "実現損益", get: function(t) { return t.real || 0; } }, h2: { label: "想定損益", get: function(t) { return t.hold2 || 0; } } };
      ...
      var _per = _chartKeys.map(function(k, i) { var t = _periodTot(_byP[k]), rr = _ratesOf(_byP[k]); if (i % _step === 0 || i === _chartKeys.length - 1) _xt.push({ i: i, label: _labelOf(k) }); return { label: _labelOf(k), value: _mi.get(t), win: rr.takeRate }; });   // 破線=利確(想定損益>0)に統一 2026-07-09
      ...
      var _metBtns = React.createElement("div", { style: { display: "flex", flexWrap: "wrap", marginBottom: 8 } },
        React.createElement("div", { style: { display: "inline-flex", background: "#EFEBE4", borderRadius: 10, padding: 3, gap: 2 } },
          [["real", "実現損益"], ["h2", "想定損益"]].map(function(m) {
      ...
      var _charts = [
        _metBtns,
        _secH("📊 損益バー＋利確（" + _mi.label + "）", "各期間の損益を上下バー（赤=利益/緑=損失）＋利確率(想定損益>0)を破線(右軸)で重ねる"),
      ...
      // ⚠️**実現損益と想定損益をそのまま引き算してはいけない**。単位も母数も違う:
      //   ・単位: 想定損益＝(α−終値)×**100株**の仮想値／実現損益＝実額（株数入り）。→ 想定損益に「基礎株数÷100」を掛けて揃える。
      //   ・母数: 想定損益には**見送り・要審議の仮想損益も入っている**が、実現損益は実エントリー分だけ。
      //           1列の差にすると「執行のロス」と「そもそも入っていない分」が混ざって読めなくなる。
      //   なので ①理論値／②実績／③執行ロス は「実エントリー済み＆実現損益あり＆想定損益が出せる」記録だけの**同一母数**で出し、
      ...
      var _mwCore = function(rs) {
        var theo = 0, realM = 0, n = 0;                               // ①②③の同一母数（実エントリー＆実現損益あり＆想定損益あり）
      ...
      _keys.forEach(function(k) {
        ...
        _rows.push(React.createElement("tr", { key: k, onClick: function() { setPerExp(on ? null : k); }, style: { cursor: "pointer", background: on ? "#FFF7ED" : "transparent" } },
          ...
          _tdP((rr.even || rr.unk) ? React.createElement("span", { style: { display: "inline-flex", flexDirection: "column", alignItems: "center", lineHeight: 1.15 } },
            ...
            rr.unk ? React.createElement("span", { title: "損切り以外で撤退足の終値が未入力＝想定損益を出せない記録。利確・同値・損切り・見切りのどれにも入れられないため件数だけ出しています（終値を入れれば該当のバケツに入ります）", style: { fontSize: 9, color: "#B45309", fontWeight: 700 } }, "金額不明" + rr.unk + "件") : null)
      ...
      return _cardify([
        ...
        _secH("💴 " + _unitLbl + "間の損益（基礎取引株数で単位をそろえた比較）",
          "※「理論値」は想定損益（100株の仮想値）に基礎取引株数を掛けたもの、「実績」は実額。基礎取引株数は" + (gran === "month" ? "この表の欄に直接入力できます（空欄＝既定" + _SN_BASE_SHARES_DEF + "株＝従来のグレードと同じ）" : "月間タブで設定します") + "。グレードは1日換算（合計÷日数）で判定しています"), _granBtns,
        _elCard(React.createElement(_HScrollBox, null,
          React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11 } },
            React.createElement("thead", null, React.createElement("tr", { style: { background: "transparent" } },
              ...
              _thP(React.createElement("span", { title: "①理論値＝実エントリーした記録の想定損益（＝○が途切れた所で手じまい・100株の仮想値）に基礎取引株数を掛けた額。ルール通りに執行できていたら取れていたはずの額です。換算は記録ごとにその記録の月の株数で行っています" }, "①理論値")),
              _thP(React.createElement("span", { title: "②実績＝実際に約定した実現損益の実額。下段は「実現損益が入力されている件数／実エントリー件数」。ここが低いと右の執行ロスは一部の記録しか見ていないことになります" }, "②実績")),
              _thP(React.createElement("span", { title: "③執行ロス＝①−②。⚠️実エントリー済み・実現損益あり・想定損益が出せる記録**だけ**の同一母数で計算しています（母数がずれると「執行のロス」と「そもそも入っていない分」が混ざるため）。スリッページや手じまいタイミングのズレがここに出ます。歩留まり＝実績÷理論値" }, "③執行ロス")),
              _thP(React.createElement("span", { title: "④見送りコスト＝見送った記録の想定損益に基礎取引株数を掛けた額。プラス＝取っていれば儲かった（機会損失）／マイナス＝見送って正解。①②③とは別母数です（見送りは実エントリーしていないので実現損益が無い）" }, "④見送りコスト")),
        ...
        _elCard(React.createElement(_HScrollBox, null,
          React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 11 } },
            React.createElement("thead", null, React.createElement("tr", { style: { background: "transparent" } },
              ...
              _thP(React.createElement("span", { title: "利益（想定損益>0）で手じまいした件数と、E成立母数（＝到達）に対する率" }, "利確")),
              _thP(React.createElement("span", { title: "想定損益がちょうど±0で手じまいした件数（対E成立）。利確（>0）・見切り（<0）のどちらにも入らない第4のバケツ（2026-07-29e・全体損益（期間別）と同基準）" }, "同値")),
              _thP("損切り率"), _thP("見切り率"),
              _thP("想定損益"), _thP("実現損益"))),
    ...
    // ここでやるのは「収益の前提のオートフィル値」を作って渡すことだけ＝月次シミュ自体は前提の入力だけで完結する（記録に依存しない）。
    // 実績＝想定損益(_elHoldFinalParts.main)の合計 ÷ 記録のあった日数。想定損益は値幅×100で**既に100株換算**なので
    // 株数で割り直さない。実現損益ではなく想定損益を使うのは、実現損益がほぼ未記録で母数にならないため。
    // 母数は_sinceCut(_v2recsAll)＝全銘柄・全シグナルの計算算入記録をルール統一後に絞ったもの＝🧮シミュと同じ線引き。
    var _projRecs = _sinceCut(_v2recsAll), _projSum = 0, _projCnt = 0, _projDaySet = {};
    for (var _pi = 0; _pi < _projRecs.length; _pi++) {
      var _pr = _projRecs[_pi]; if (!_pr || !_pr.signal) continue;
