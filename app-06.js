...
function _elOsHistV2(_ref) {
  ...
  var colNodes = bars.map(function(b) {
    ...
    return React.createElement("div", { key: b.key, title: b.full + ": " + b.cnt + "件 (" + pct + "%)" + (_isMark ? "（現在の推奨基本α " + markVal + "円）" : "") + (_histClickable ? "（クリックで取引一覧）" : ""), onClick: click,
      ...
      React.createElement("div", { style: { width: "100%", height: (b.cnt ? Math.max(2, Math.round(b.cnt / maxC * barH)) : 2) + "px", background: b.cnt ? b.color : "#eee", borderRadius: "2px 2px 0 0", outline: _isSel ? "2px solid #9A3412" : (_isMark ? "2px solid #0369A1" : (b.band ? "1.5px dashed rgba(120,53,15,0.5)" : "none")), outlineOffset: 1 } }));
  });
  var xNodes = bars.map(function(b) {
    var _isMark = markKey != null && b.key === markKey;
    return React.createElement("div", { key: b.key, style: { flex: "1 1 0", minWidth: 0, textAlign: "center", fontSize: 9, color: _isMark ? "#0369A1" : (b.band ? "#9A3412" : "#999"), fontWeight: (_isMark || b.band) ? 700 : 400, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
      _isMark ? React.createElement("div", { style: { fontSize: 8, color: "#0369A1", lineHeight: 1 } }, "▲") : null, b.x);
  });
  ...
  var markCap = (markVal != null && markKey != null && bars.some(function(b) { return b.key === markKey; }))
...
var _EL_ADD_MIN_ERATE = 0.5;     // 推奨追加α 到達率フロア: EP到達率(OS2まで)がこの値未満の加算は第1・第2段では推奨しない＝届かない高αを除外（基本αの_EL_BASE_MIN_ERATEと同思想・独立つまみ）。第3段(最後の砦)には課さない。後で調整可（2026-06-30）。
...
//   第1: 損切り率0%（完全回避）かつ到達率≥_EL_ADD_MIN_ERATE(50%)の中で【最小加算】／第2: 損切り率≤_EL_ADD_STOPRATE_2(30%)かつ到達率≥フロアの中で【最小加算】／第3(それも無し): 黒字の中で【損切り率が最も低い】加算（同率は最小加算・第3段は安全網なので到達率フロアは課さない）。
...
function _elAddAlphaReco(recs, aiOf, baseAlpha) {
  ...
  var pool, p;
  var _reachOk = function(x) { return x.e.eRate != null && x.e.eRate >= _EL_ADD_MIN_ERATE; };   // EP到達率(OS2まで)が50%(既定)以上か。第1・第2段のみに課す＝届かない高αを除外（基本αと同思想）。第3段は安全網なので非課。
...
function _elBaseAlphaPeriodBlockV2(data, stock, refDate) {
  ...
  return React.createElement("div", { style: { marginTop: 8, padding: "8px 10px", borderRadius: 8, background: "#F0F9FF", border: "1px solid #BAE6FD" } },
    ...
    React.createElement("div", { style: { fontSize: 9, color: "#64748B", marginBottom: 6 } }, "追加α〇（要）を明示した記録だけが母数（基本α表とは別プール）。各期間で『基本α＋推奨追加α』を当てた 損切り率/H1勝率/到達率/想定損益。推奨追加α＝基本αに+1〜30円（1円刻み）足して想定損益が黒字・標本3件以上を満たす中から損切りを避けられる最小の加算（第1:損切り率0%／第2:30%以下、いずれも到達率50%以上。どちらも無ければ損切り率が最も低いものを最後の砦に＝この段は到達率不問。黒字化する加算が無ければ「推奨無し」）。想定損益も同じく1営業日あたり平均／期間累計。〇記録の無い期間は—。"),
...
function _elBaseAlphaDayBlockV2(recs, aiOf, refDate) {
  ...
  return React.createElement("div", { style: { marginTop: 10, padding: "10px 12px", borderRadius: 8, background: "#F0F9FF", border: "1px solid #BAE6FD" } },
    React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: "#0369A1", marginBottom: 6 } }, "🎯 推奨基本α値（" + refDate + "）"),
    headNode,
...
function EntryLogView(_ref_elv2) {
  ...
  var _tabs = _isAllStock
    ? [["sum", "📊 集計"], ["period", "📆 期間"]]
    : [["sum", "📊 集計"], ["period", "📆 期間"], ["alpha", "📐 α値"], ["stop", "🛑 損切り"], ["miss", "❌ 未達"], ["signal", "🎯 シグナル別"], ["deep", "🔬 深掘り"]];
  ...
  if (view === "sum") {
    if (_isAllStock) {
      ...
      _tabBody = React.createElement(React.Fragment, null,
        ...
        sumMode === "month"
          ? React.createElement(React.Fragment, null,
              _sumMonthNav,
              _sumMonthRecs.length ? _sumStockContent(_sumMonthRecs)
    ...
    if (!_v2recsAll.length) {
      _tabBody = React.createElement("div", { style: { color: "#bbb", textAlign: "center", padding: "20px 0", fontSize: 12 } }, "EP起算（v2）の記録がありません");
    } else {
      // α値タブ＝タブ内サブタブ式（2026-06-29）: 基本α(青)/追加α(茶橙)/共通ツール(グレー)を別画面に分離し「ごっちゃ」を解消。
      // 各サブタブ冒頭にゾーン見出し＋推奨値サマリー。サマリーはB1/A群と同じ _elBaseAlphaA(_v2recsAll) 由来＝値が一致（二重表示のズレを回避）。
      var _alA = _elBaseAlphaA(_v2recsAll, _ai);
      ...
      if (_alAdd && _alAdd.improved) {
        _alAddSum = React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: "#9A3412" } }, "推奨追加α +" + _alAdd.add + "円", React.createElement("span", { style: { fontSize: 11, fontWeight: 700, marginLeft: 6, color: "#94A3B8" } }, "（基本＋追加＝計" + _alAdd.total + "円）"), _alAdd.add2 != null ? React.createElement("span", { style: { fontSize: 11, fontWeight: 700, marginLeft: 4, color: "#9A3412" } }, "次点 +" + _alAdd.add2 + "円") : null);
      } else {
      ...
      if (_alSel === "base") {
        _alBody = React.createElement(React.Fragment, null,
          _alZoneHead("#0369A1", "#F0F9FF", "#BAE6FD", "基本αゾーン ― まず取る土台（最低限とる利幅）", React.createElement(React.Fragment, null, _alBaseSum, React.createElement("div", { style: { fontSize: 10, color: "#94A3B8", marginTop: 3 } }, "このスコープ全体（" + _v2recsAll.length + "件）の推奨。期間別の推移は下の「期間推移」で。"))),
        ...
        _alBody = React.createElement(React.Fragment, null,
          _alZoneHead("#64748B", "#F8FAFC", "#E2E8F0", "共通ツール ― 基本/追加に依らないα全体の検証", React.createElement("div", { style: { fontSize: 10, color: "#64748B" } }, "α=0〜20円で全記録を再計算し、利益が最大になるα（★）を確認して最終微調整。")),
          _alphaTable ? React.createElement(React.Fragment, null, _secH("🎯 α意思決定表", "α=0〜20円で再計算・損切り値は各記録の採用値・★=H1/H2の利益最大α"), _alphaTable) : null,
          _secH("📉 α感応度カーブ", "α=0〜20円で全記録を再計算した合計の推移（意思決定表のグラフ版）"),
          _elAlphaCurveSectionV2(_v2recsAll, _ai));
      }
      _tabBody = React.createElement(React.Fragment, null,
        React.createElement("div", { style: { fontSize: 11, color: "#64748B", marginBottom: 6 } }, (_isAllStock ? "全銘柄合算" : "この銘柄（" + _selStock + "）") + "のα分析。EP起算（v2）の" + _v2recsAll.length + "件で算出。下のタブで 基本α／追加α／共通ツール を切替（追加α分析トグルの影響を受けず常に全件）。" + (_isAllStock ? "（銘柄をまたいだα傾向の参考。銘柄ごとは各銘柄タブで）" : "")),
        _alphaPills,
        _alBody);
    }
  } else if (view === "stop") {
