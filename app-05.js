...
// 追加α3状態(addAlphaUsed)対応 2026-06-25: 未選択(true/false以外)は「（未選択）」表記・×不要(add=0)は基本αのみで内訳非表示・〇要(add>0)は「（基本+追加）」。
// 不算入(includeInTotal===false=_elIsExcluded)は最優先で「（不算入）」水色表記＝α値欄でも除外を明示（時間欄の不算入バッジに加えて）2026-06-25b。
function _elAlphaBreakdownNode(s, dispAlpha) { if (!s) return null; var _bdStyle = { fontSize: 9, color: "#94A3B8", fontWeight: 600, lineHeight: 1.1, marginTop: 1, fontVariantNumeric: "tabular-nums" }; if (_elIsExcluded(s)) return React.createElement("div", { style: { fontSize: 9, color: "#0284C7", fontWeight: 700, lineHeight: 1.1, marginTop: 1 } }, "（不算入）"); var _bn = function(v) { return (v != null && v !== "" && !isNaN(Number(v))) ? Number(v) : null; }; var base = _bn(s.baseAlphaVal), add = _bn(s.addAlphaVal), total = _bn(s.alphaVal); if (base == null) { if (total != null && add != null) base = total - add; else return null; } if (add == null) add = (total != null) ? (total - base) : 0; if (base < 0 || add < 0) return null; if (dispAlpha != null && Number(dispAlpha) !== base + add) return null; if (s.addAlphaUsed !== true && s.addAlphaUsed !== false) return React.createElement("div", { style: _bdStyle }, "（未選択）"); if (add === 0) return null; return React.createElement("div", { style: _bdStyle }, "（" + base + "+" + add + "）"); }
