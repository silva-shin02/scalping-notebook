...
// 2026-06-25d: 「（未選択）」は《基本α値を持つ新形式記録(baseAlphaVal有り)》かつ《算入記録(!_elIsExcluded)》だけが対象。①旧記録(baseAlphaVal無し=base==nullで早期null返し)には出さない＝追加α3状態の概念が無くαを入力済みの旧記録に誤って付くのを防ぐ。②不算入記録(_elIsExcluded)にも出さない＝分析から除外なので追加α状態は無意味・ユーザー指摘「ちゃんと選択したのに不算入記録で（未選択）が出る」を解消。先回の「（不算入）」表記・旧記録への（未選択）付与はどちらも撤回。
function _elAlphaBreakdownNode(s, dispAlpha) { if (!s) return null; var _bn = function(v) { return (v != null && v !== "" && !isNaN(Number(v))) ? Number(v) : null; }; var base = _bn(s.baseAlphaVal), add = _bn(s.addAlphaVal), total = _bn(s.alphaVal); if (base == null) { if (total != null && add != null) base = total - add; else return null; } if (add == null) add = (total != null) ? (total - base) : 0; if (base < 0 || add < 0) return null; if (dispAlpha != null && Number(dispAlpha) !== base + add) return null; var _bdStyle = { fontSize: 9, color: "#94A3B8", fontWeight: 600, lineHeight: 1.1, marginTop: 1, fontVariantNumeric: "tabular-nums" }; if (s.addAlphaUsed !== true && s.addAlphaUsed !== false && !_elIsExcluded(s)) return React.createElement("div", { style: _bdStyle }, "（未選択）"); if (add === 0) return null; return React.createElement("div", { style: _bdStyle }, "（" + base + "+" + add + "）"); }
