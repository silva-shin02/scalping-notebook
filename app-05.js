function Calendar(_ref60) {
  ...
  // 2026-08-17 日次バッジを想定損益（100株換算）基準へ。旧＝signal.realizedPnlだけを見ていたので、
  //   実現損益が未記録の日は「エントリーしたのに0円・グレードD」で出ていた（item.pnlも拾っていなかった）。
  //   集計は_snDailyPnlMapへ一本化＝ホームの4チップ・📊今月の損益パネルと必ず同じ数字になる。
  //   グレードも実額スケール(_profitGradeFromPnlReal)→通常スケール(_profitGradeFromPnl)へ。想定損益は100株換算なので帯が10倍ずれていた。
...
// 2026-07-25f 実現OSもOS1〜5へ拡張（想定損益・実現最高・保有時間と同じ手じまい足まで見る）: EPがOS2/OS3にずれた記録やH3/H4まで保有した記録で、
...
function _epHoldLadder(s, alpha, cutLine) {
  ...
  var items = [], stopDepth = -1, stopPnl = null, maxPnl = null, maxDepth = 0, finalPnl = null;
  ...
  // 撤退済みなら想定損益は撤退足の確定値で固定する。旧実装は高値から必ず算出できたので暗黙に成立していたが、
  // 終値ベースでは撤退足に終値が無いと null になり、明示しないと撤退前の含み益が最終値として残ってしまう。
  if (stopDepth >= 0) finalPnl = stopPnl;
  // 2026-07-29 旧方式（ラインに触れた瞬間に約定）の損益 stopLinePnl と、その算出用の stopHigh を撤去。
  // 方式変更（2026-07-27）の効果検証が済み、比較列も落としたため保持する理由がなくなった。
  return { epIdx: r.epIdx, items: items, stopDepth: stopDepth, stopPnl: stopPnl, maxPnl: maxPnl, maxDepth: maxDepth, finalPnl: finalPnl };
}
...
// 2026-07-25 depth引数を追加（3以上＝H3/H4行の足）。未指定は従来どおり isH2 で1/2＝**depth<=2は完全に不変**。
function _epHoldView(s, alpha, isH2, depth) {
  var base = isH2 ? _h2sig(s) : s;
  if (!_epIsV2(s)) return base;
  var _r = _epResolve(s, alpha != null ? alpha : _epOwnAlpha(s));
  var _dv = (depth != null) ? depth : (isH2 ? 2 : 1);
  var leg = _r ? ((_dv === 1) ? _r.h1 : (_dv === 2) ? _r.h2 : ((_r.epIdx >= 0) ? (_r.legs[_r.epIdx + _dv] || null) : null)) : null;
...
// 【損切り判定の単一源】手じまいまでに損切りしたか＝**想定損益と同じ基準** 2026-07-25d。
...
// ===== 手じまいまで（H2固定→N段カスケード）2026-07-25 =====
// ユーザー要望「想定損益欄、H2までにするのではなく手じまいまでにできる？」。旧モデルは EP→H1→H2 の2段固定で、
// EPがOS1/OS2の記録は次足期待度が○のまま続いても H2（EPの2本後）で打ち切っていた。フォームは nextExp1〜4（保有判断4回ぶん）と
// OS1〜OS5 を保存済みなので、○が続く限り最後の足まで伸ばせる。**EPがOS3の記録は上限2＝従来と完全に同じ**。
// depth d ＝ EPから d 本後の足（0=EP足）。足iの保有判断は _epNextExpAt(s, i-1)（有効域 x=0〜3）。
// 手じまい深さの上限: 足数と _epNextExpAt の有効域の両方で決まる（epIdx=0かつOS5まである記録で最大4）。非v2/α無しは従来の2。
function _elHoldMaxDepth(s, alpha) {
  if (!s || !_epIsV2(s) || alpha == null) return 2;
  var r = _epResolve(s, alpha);
  if (!r || r.epIdx < 0) return 2;
  return Math.max(0, Math.min(r.legs.length - 1 - r.epIdx, 4 - r.epIdx));
}
// depth d までのどこかで損切りに当たったか。d<=1 は _elHoldIsStop（EP/H1）へ委譲＝従来と同値。
function _elHoldIsStopAt(s, alpha, cutLine, d) {
  if (d <= 1) return _elHoldIsStop(s, alpha, cutLine);
  if (!_epIsV2(s) || alpha == null) return false;
  var r = _epResolve(s, alpha);
  if (!r || r.judge !== "ok") return false;
  var cut = cutLine != null ? cutLine : 15;
  for (var k = 0; k <= d; k++) { var lg = r.legs[r.epIdx + k]; if (lg && lg.h != null && (lg.h - alpha) >= cut) return true; }
  return false;
}
// depth d で手じまいした場合の損益。d<=1→_elDynHold／d===2→_elDynHold2 に委譲＝従来の正本と完全一致（新規はd>=3のみ）。
// d>=3 の規約は _elDynHold2 と同型: 手前の足で既に損切り済みなら損切りルール非適用（確定値ベース）・この足で損切りなら損切り額。
function _elDynHoldAt(s, alpha, cutLine, d) {
  if (d <= 1) return _elDynHold(s, alpha, cutLine);
  if (d === 2) return _elDynHold2(s, alpha, cutLine);
  if (!_epIsV2(s) || alpha == null) return null;
  var r = _epResolve(s, alpha);
...
//   （想定損益_elHoldFinalParts・保有時間_elRideVals と同じ手じまい足に揃える）。**exitD<=2の記録は従来と完全に同一**。
function _elRealizedOutcome(s, alpha, cutLine) {
  ...
  if (_elIsStopFinal(s, alpha, cutLine)) return "stop";   // 2026-07-25d 損切りは想定損益と同じ基準（手じまいより後の足の損切りは数えない）
...
function _epOsChainCell(s, alpha, cutLine) {
  ...
  if (_epIsV2(s)) {
    ...
    //   保有していない足は出さない（_elRideVals.exitD＝想定損益と同じ手じまい足）。E未達/×見送り/旧記録は従来どおり3本。
...
// 「時刻が異なれば早い方(=ペアの先頭)を残す（損益に依らず＝リアルタイムで先に入った方）／同時刻(分が同じ)のときだけ（）外想定損益
// (_elHoldFinalPartsのmain・採用α/採用損切り)が小さい方を残す（同額は先を残す）」＝残さない方（遅い方／同時刻なら損益大）を合計額から除外。どちらかmain無しのペアは除外なし・単独記録は除外なし。
...
// 選抜の保存先＝data.collPick { <グループID>: <選抜されたkey> } 2026-08-19b。
// ⚠️**1グループ1エントリ**にしてあるのは同期のため。旧案の { key:1 } フラット集合だと選び直しが「delete＋add」になるが、
//   _mergeRemoteMeta(app-01)は入れ子オブジェクトに _v が無いと rv===undefined を out[key]=lv に落とす＝**削除が伝播しない**。
//   結果、端末Aの旧選抜と端末Bの新選抜が和集合になり、「先にソートされる方」が勝って**選び直しが黙って元に戻る**。
//   gidキーなら選び直しは同じキーの**上書き**（primitiveは _mergeRemoteMeta が return remote で伝播する）＝削除が要らない。
// gidは (scope, 日付, 先頭メンバーkey) から作る。メンバーが増減してgidがずれた場合に備え、解決時は
// 「collPickの**値**として現れているメンバー」も拾う（_pickValsの逆引き）＝取りこぼしても選抜が消えない。
function _elCollGid(scopeStock, date, firstKey) { return (scopeStock || "") + "\u0001" + date + "#" + firstKey; }
function _elCollPickMap(data) { return (data && data.collPick) || _EL_COLL_EMPTY_PICK; }
// 自動確定の境界日。この日より**前**の日付のグループは、手動選抜が無ければ旧ルールで自動確定して扱う（移行案あ）。
// これを日付で持つことで、Firebaseから後から古い履歴が届いても正しく自動確定される（一括書き込みの移行処理が不要になる）。
function _elCollPickSince(data) { return (data && data.collPickSince) || null; }
...
//    既存データは data.collPickSince（境界日）より前を旧ルールでその場確定するので、過去の日が未確定にはならない（移行案あ）。
...
function _elCollisionExcludedSet(data, scopeStock) {
  ...
  Object.keys(charts).forEach(function(ck) {
    ...
    c.signals.forEach(function(sig) {
      ...
      (byDay[date] = byDay[date] || []).push({ key: _elCollKey(stock, date, s), stock: stock, time: s.time || "", min: mn, exitMin: _exMin, hasEp: !!_rv, main: _elHoldFinalParts(s, _own, cut).main });
    });
  });
  ...
  Object.keys(byDay).forEach(function(d) {
    ...
    while (i < arr.length) {
      ...
      i = j;
      // main無しは2種類あるので区別する。"miss"＝EP未到達＝真の未達（選べば枠は0円）。
      // "ref"＝EPには到達したが（）外の想定損益が無い（_elHold2TotPartsFixedが{main:null,ref:X}を返すケース）。
      // 後者を「未達・0円」と説明すると嘘になる（（）内参考値は従来どおり集計される）ので表示を分ける。
      for (var q = 0; q < list.length; q++) { if (list[q].main == null) na[list[q].key] = list[q].hasEp ? "ref" : "miss"; }
      if (list.length < 2) continue;   // 単独＝奪い合いが無いので選抜も除外も不要（従来どおり）
      var gid = _elCollGid(scopeStock, d, list[0].key);
      var picked = null;
      var _pv = _pickRef[gid];
      for (var p = 0; p < list.length; p++) { if (list[p].key === _pv) { picked = list[p]; break; } }
      if (!picked) { for (var p2 = 0; p2 < list.length; p2++) { if (_pickVals[list[p2].key]) { picked = list[p2]; break; } } }   // gidずれの取りこぼしを拾う
      // 手動選抜が無いとき自動確定するのは次の2つ。どちらでもなければ「選抜待ち」＝合計から外す。
      //  ①銘柄別scope: 選抜UI（ラジオ）は全銘柄横断のグループにしか出ないので、銘柄別スコープ固有のグループは
      //    そのままだと**どこからも選べない**まま永久に合計から消える。手動選抜は横断側の1本に任せ、こちらは常に自動確定する。
...
// （）外の想定損益を持たない記録。選抜対象には含む 2026-08-19。
...
// その種別。"miss"＝EP未到達＝未達／"ref"＝EP到達済みだが（）外損益なし／null＝該当なし 2026-08-19b。
function _elCollNaKind(data, r, scope) { return (r && r.signal && _elCollisionExcludedSet(data, scope).na[_elCollKey(r.stock, r.date, r.signal)]) || null; }
...
// recs配列中の「合計から落ちた」件数（KPI等の可視化用）。選抜待ちも**数に含める** 2026-08-19b。
// ⚠️一度は選抜待ちを除いて数えていたが、警告バーを持たないビュー（記録帳KPI・銘柄タブ等）で
//   「被り除外0件」と出るのに金額だけ消える＝無言の欠損になるため差し戻した。組数は_elCollPendingGroupCountで別に出す。
...
function _elCollSetPick(save, data, r, scope) {
  ...
  save(function(prev) {
    ...
    // gidがずれて残った古いエントリ（値がこのグループのメンバー）を掃除してから、gidキーを上書きする。
    // 通常の選び直しは delete を伴わない**上書き**になるので、_mergeRemoteMeta の削除非伝播に引っかからない。
    var mem = {};
    g.members.forEach(function(k) { mem[k] = 1; });
    Object.keys(pick).forEach(function(gk) { if (mem[pick[gk]]) delete pick[gk]; });
    pick[g.gid] = key;
...
function _elCollAutoPickKey(list) {
  ...
  // ⚠️母数はまず「想定損益を持つ記録」に絞る。ここを絞らずに最早分だけで選ぶと、グループ先頭が未達だった場合に
  //   未達が枠を取り、同じグループの実損益（例: +8,000円）が丸ごと合計から消える。
  //   旧実装は main無しが絡むペアを除外対象にしなかった＝金額を落とさなかったので、これは退行になる。
  var pool = [];
  for (var q = 0; q < list.length; q++) { if (list[q].main != null) pool.push(list[q]); }
  if (!pool.length) pool = list;   // 全員未達ならその中から
  var t0 = pool[0].min, best = null;   // listはmin昇順なのでpoolも昇順
  for (var i = 0; i < pool.length && pool[i].min === t0; i++) {
    var m = pool[i];
    if (!best) { best = m; continue; }
    if (m.main != null && best.main != null && m.main < best.main) best = m;   // 同時刻は（）外想定損益が小さい方
  }
  return best ? best.key : pool[0].key;
}
...
function _elCollPickNode(data, r, save, scope) {
  ...
  var _nk2 = _elCollNaKind(data, r, scope), isNa = !!_nk2;
...
function _elCollMarkNode(data, r, scope, opts) {
  ...
  var _nk = _elCollNaKind(data, r, scope);
  if (_nk === "miss") {
    _bs.push(_mk("n", "未達", "#64748B", "#F1F5F9", "#CBD5E1",
      "未達（EPに到達せず建玉なし）。選抜はできますが、選ぶとこの枠は0円として確定します"));
  } else if (_nk === "ref") {
    _bs.push(_mk("n", "想定なし", "#64748B", "#F1F5F9", "#CBD5E1",
      "EPには到達しているが（）外の想定損益が無い記録。選抜はできますが、選ぶとこの枠の（）外合計は0円になります（（）内の参考値は従来どおり集計されます）"));
  }
  ...
  (items || []).forEach(function(it) {
    ...
    if (ppN != null) {
      if (_epIsTriEntry(s, a)) {
        ...
        if (_elPlanIsStop(s, a, c)) t.planStop = true;
...
// 詳細損益セル（EP/H1/最終の縦積み・（）外main＋（）内=△参考）。本日/今週/銘柄別今週の3表で共用＝_elTotAccumで全表と同一基準。recsMは被り除外後の記録・aiAlpha(r)/aiCut(r)=採用α/損切り・badgeFn(grade)=グレードバッジ・allMiss=全E未達でQ0。最終行＝想定損益列と同値。2026-07-10（2026-07-25「H２」→「最終」＝手じまいまでに拡張）。
function _elDetailPnlStackNode(recsM, aiAlpha, aiCut, badgeFn, allMiss, days) {
  ...
  // allMissFirst=EP行のみ（旧EPセルはallMiss時に計画0でもQ0優先）。H1/H2は「値があれば値・無ければ参考/Q0/—」＝旧H1・最終セルと完全一致＝詳細のH2は想定損益列と常に同値。
...
//  ・H1期待度 未設定 → ×と同じ扱い（想定額main・参考無し）。○ → H1結果(想定額キャップ後)をmain。
...
function _elHold2TotPartsFixed(s, alpha, cutLine) {
  if (!s) return { main: null, ref: null };
  if (_epIsXSkip(s, alpha)) return { main: null, ref: null };  // EP×（×見送り）→ 完全に算入無し
...
// 【想定損益＝手じまいまで】2026-07-25: 表・合計・分析すべての「想定損益」正本。旧 _elHold2TotParts（EP→H1→H2の2段固定）を
...
function _elHoldFinalParts(s, alpha, cutLine) {
  ...
  if (!s || !_epIsV2(s) || alpha == null) return parts;
  var maxD = _elHoldMaxDepth(s, alpha);
  if (maxD < 3) return parts;                                  // EPがOS3等＝伸ばす余地なし（従来と同一）
  var r = _epResolve(s, alpha);
  if (!r || r.judge !== "ok" || r.epIdx < 0) return parts;      // ×見送り/E未達は従来どおり
  if (_epIsXSkip(s, alpha) || _elH2Miss(s, alpha)) return parts;
  var cut = cutLine != null ? cutLine : 15;
  if (_elHoldIsStopAt(s, alpha, cut, 2)) return parts;          // H2までに損切り＝そこで手じまい済み
  var _e1 = _epNextExpAt(s, r.epIdx), _e2 = _epNextExpAt(s, r.epIdx + 1);
  if (_e1 !== "○" && _e1 !== "△") return parts;               // H1で撤退済み
  if (_e2 !== "○" && _e2 !== "△") return parts;               // H2で撤退済み
  var chain = !_epIsTriEntry(s, alpha) && _e1 === "○" && _e2 === "○";   // 完全○チェーン（_elHold2TotPartsFixedの（）なし条件と同じ）
  for (var d = 3; d <= maxD; d++) {
    var lg = r.legs[r.epIdx + d];
    ...
    //   想定損益は前の足の値（実測例では+400円の利益）」という食い違いになり、負け平均から抜けて勝ち平均に入っていた。
...
// 【基準】記録帳・💰全体損益と同じ「想定損益」_elHoldFinalParts(...).main。**単位は元から100株換算**なので
      ...
      : React.createElement(React.Fragment, null,
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(128px,1fr))", gap: 12, alignItems: "start" } },
          ...
          _cell("1日あたり",
            ...
            "想定損益 " + _snYen(agg.final) + " ÷ 経過営業日 " + agg.bizDone + "日。記録の無かった日も分母に入れています（他の欄の「1日平均」と同じ割り方）"),
          _cell("実現損益",
            ...
            agg.realCnt > 0
              ...
              : "この月は実現損益が1件も入っていません。上の想定損益は記録から出した値です"),
          _cell("勝率",
            ...
            "想定損益がプラスの件数 ÷ 想定損益のある件数（同値は分母に含む）"),
          _cell("最大DD",
...
// 2026-07-25 depth引数を追加（3以上＝H3/H4行）。未指定は従来どおり isH2 で1/2＝**depth<=2の経路は1行も変わらない**。
function _elHoldParts(s, alpha, cutLine, isH2, depth) {
  var _d = (depth != null) ? depth : (isH2 ? 2 : 1), _deep = _d >= 3;
  var hs = _deep ? _epHoldView(s, alpha, true, _d) : _epHoldView(s, alpha, isH2);
  // 損切り済み（H2: 想定orH1損切り／H1: 想定損切り）→ 明細を出さず損益列に「ー（ランク 損切額）※損切り済」のみ。
  if (alpha != null) {
    var _psP = _elPlanIsStop(s, alpha, cutLine);
    if (_deep ? _elHoldIsStopAt(s, alpha, cutLine, _d - 1) : (isH2 ? _elHoldIsStop(s, alpha, cutLine) : _psP)) {
      var _samtP = _deep ? _elDynHoldAt(s, alpha, cutLine, _d - 1) : (_psP ? _elDynPlanned(s, alpha, cutLine) : _elDynHold(s, alpha, cutLine));
  ...
  if (holdPnl != null) {
    var hp = holdPnl;
    var pp = _deep ? _elDynHoldAt(s, alpha, cutLine, _d - 1) : (isH2 ? ((alpha != null) ? _elDynHold(s, alpha, cutLine) : _elSignedVal(s.holdPnl, s.holdPnlSign)) : ((alpha != null) ? _elDynPlanned(s, alpha, cutLine) : _elSignedVal(s.plannedPnl, s.plannedPnlSign)));
    var dynHP = (function() {
      if (hp == null) return hs.holdProfit;
      if (!isH2 && (res === "miss" || res === "draw")) return hp > 0 ? "yes" : hp < 0 ? "no" : "none";
      if (pp == null) return isH2 ? (hp > 0 ? "yes" : hp < 0 ? "no" : "none") : hs.holdProfit;
    ...
    var symMap = { yes: ["○", "#1E8449"], mid: ["△", "#B45309"], none: ["ー", "#888"], no: ["×", "#C0392B"] };
    var sym = symMap[dynHP];
    var _stopLeftP = _deep
      ? ((alpha != null && _elHoldIsStopAt(s, alpha, cutLine, _d - 1)) ? _elDynHoldAt(s, alpha, cutLine, _d - 1) : null)
      : (isH2
...
function _elHoldStackInner(s, alpha, cutLine) {
  ...
  var _stopRow = function(rk, lblNode, amount, topB, isH2, depth) {
    var hs = _epHoldView(s, alpha, isH2, depth);   // depth未指定は従来（isH2で1/2）2026-07-25
  ...
  if (_h2miss) {
    ...
    // H2○でもH1=△ or EP△でチェーンが切れていれば参考(level1)で（）囲み。合計(_elHoldFinalParts)の（）内計算と一致。
    rows.push(_row("h2", "H２", h2exp, p2, (hexp === "×" || hexp === "損切り済" || exp === "×") ? 2 : ((exp === "△" || exp === "損切り済" || hexp === "△" || _epTriH2) ? 1 : 0), true));
  }
  // 2026-07-25 H3・H4行（想定損益を手じまいまで伸ばしたので内訳も伸ばす）: **その足まで実際に保有した記録だけ**行を足す＝
  //   1段前の保有判断が○/△かつ足がある場合のみ。×/損切り済/未設定で降りた記録・EPがOS3の記録は1行も増えない（従来と同じ見た目）。
  var _maxDS = _elHoldMaxDepth(s, alpha);
  if (!_h2miss && !_h2StopDone && _maxDS >= 3 && alpha != null) {
    var _rS = _epResolve(s, alpha);
    if (_rS && _rS.judge === "ok" && _rS.epIdx >= 0 && (hexp === "○" || hexp === "△") && (exp === "○" || exp === "△")) {
      var _chainS = !_epIsTriEntry(s, alpha) && hexp === "○" && exp === "○";
      var _cutS = cutLine != null ? cutLine : 15;
      var _lblJP = { 3: "H３", 4: "H４" };
      for (var _dS = 3; _dS <= _maxDS; _dS++) {
        var _lgS2 = _rS.legs[_rS.epIdx + _dS];
        if (!_lgS2 || (_lgS2.h == null && _lgS2.c == null)) break;
        var _peS = _epNextExpAt(s, _rS.epIdx + _dS - 1);
        if (_peS !== "○" && _peS !== "△") break;   // ここで手じまい済み＝行を出さない
        var _lblS = _lblJP[_dS] || ("H" + _dS);
        if (_elHoldIsStopAt(s, alpha, _cutS, _dS - 1)) { rows.push(_stopRow("h" + _dS, _lblS, _elDynHoldAt(s, alpha, cutLine, _dS - 1), true, true, _dS)); break; }
        var _pS = _elHoldParts(s, alpha, cutLine, true, _dS);
        var _peNode = React.createElement("span", { style: { color: _expCol[_peS] || "#666" } }, _peS);
        rows.push(_row("h" + _dS, _lblS, _peNode, _pS, (_peS === "○" && _chainS) ? 0 : 1, true));
        if (_peS !== "○") _chainS = false;
        if (_lgS2.h != null && (_lgS2.h - alpha) >= _cutS) break;   // この足で損切り＝ここで手じまい
      }
    }
  }
...
// 想定損益(_elHoldFinalParts.main)と同じ手仕舞い足の確定値。損切り記録は「最高↑26 → 損切（H1）」。手仕舞い足＝損切り足／非損切りは次足期待度○のみ次足へ（H2まで）。
...
function _elRideVals(s, alpha, cutLine) {
  ...
  //  最高(mx)/確定(exitC)に期待度×以降の足が混入して想定損益額(_elHoldFinalParts)・打ち切り最高値(_elOsMaxCapped)と食い違っていた。
  //  損切りは期待度手じまいより後には起こり得ないので_expDでキャップする。△(中間)は継続扱い＝×/損切り済/未設定のみで降りる（_elOsMaxCapped/_elHoldFinalPartsと同規約）。
  ...
  // 2026-07-25 H2固定→N段: H2まで○/△で続いていれば、○/△が続く限り最後の足まで手じまいを伸ばす（想定損益_elHoldFinalPartsと同じ深さ）。
  var _maxD = _elHoldMaxDepth(s, alpha);
  if (_expD === 2) { for (var _dd = 3; _dd <= _maxD; _dd++) { if (_cutExp(_epNextExpAt(s, r.epIdx + _dd - 1))) break; _expD = _dd; } }
  ...
  if (_stopD == null) { for (var _sd = 3; _sd <= _maxD; _sd++) { var _lgS = legs[r.epIdx + _sd]; if (_lgS && _lgS.h != null && (_lgS.h - alpha) >= cut) { _stopD = _sd; break; } } }   // 2026-07-25 3段目以降の損切り足
  var exitD, stopped;
...
function _elDetailFlowStack(s, alpha, cutLine) {
  ...
  else if (_epIsXSkip(s, alpha)) _hPart = React.createElement("div", { style: { display: "inline-flex", alignItems: "center", gap: 3, whiteSpace: "nowrap", opacity: 0.7, justifyContent: "flex-start" } },
  ...
  // 2026-07-13 「最高↑→決済↓」行(_elRideSummaryNode)は想定損益欄(_elRideMiniNode)に移設したため削除。
...
// 記録表の「想定損益」セル＋「OS・損益詳細」セル(colSpan2)の共通td枠(2026-07-14 系統2小抽出)。セル内容(_elHold2AmtNode/_elRideMiniNode/_elDetailFlowStack)は既に共有・td枠だけが各表で重複していたので集約。border/padは表ごとに異なる(app-02/06=_bb・"1px 3px"／app-04=#f0ede6・"4px 6px")ため引数化＝各呼出は従来と同一バイトを渡す。Fragmentなので<tr>内で従来の2td並びと同一DOM。
...
// 2026-07-29 「損切り値ちょうどで約定していたら」の損失額（_elCapLossYen/_elCapNoteAmt/_elCapNote）を削除。
//   終値撤退方式（2026-07-27 cacf2f5）ではラインは撤退トリガーであって約定値ではないため、
//   −(損切り値×100)固定という理想値は起こり得ない仮定になった。表示自体は 1dfff48 で停止済みで、
//   残っていた集計（planCap/planCapSum/holdCapSum/hold2CapSum/_totPlanCap/_totHoldCap）も読み出しゼロだった。
//   実際の損切り額は撤退足の終値ベース＝_epHoldLadder.finalPnl / _elHoldFinalParts を使う。
...
function _elCalcStats(records, data, simResolve) {
  ...
  records.forEach(function(r) {
    ...
    if (hp != null) {
      ...
      // H結果損益(想定額キャップ): 想定が損切りの行は想定額(ppN)に置換し、本日/今週・明細の合計と統一。
    ...
    // H2合計（結果損益）: _elHoldFinalParts で統一（損切り→想定額・非損切り○/△→_elDynHold2・非損切り×→参考）。本日/今週・明細・各集計表の合計を一致させる。
    var _h2t = _elHoldFinalParts(s, _liveA ? _ai.alpha : null, _liveA ? _ai.cutLine : 15);
...
// 2026-08-05l グレード凡例。**_GRADE_DESC / _GRADE_DESC_REAL から生成する**ので、
// しきい値を新たに二重持ちしない（既存の凡例はベタ書きが6か所あり、変更のたびに追随が要る）。
// 2026-08-05m 複数スケールを並べたとき列がずれたので、**「ラベル＋9段」を1つのgridに入れて縦ぞろえ**する。
//   行ごとに別のflexにすると、桁数の違い（2501~ と 25001~）でA以降のバッジが右へずれていく。
//   grid化すると各段が同じ列に入り、列幅は広い方（実現損益）に合わせて自動で決まる。
// rows = [{ label: "想定損益", real: false }, ...]。lowerOnly=trueで下限だけ出して幅を詰める。
// 置き場所が狭いので、レンジ全文はtitle属性へ入れてホバーで読めるようにしてある。
function _gradeLegendGrid(rows, opts) {
  var o = opts || {};
  var order = ["S", "A", "B", "C", "D", "E", "F", "G-", "G+"];
  var cells = [];
  (rows || []).forEach(function(r, ri) {
    cells.push(React.createElement("span", { key: "lb" + ri,
      style: { fontWeight: 700, color: "#6B6459", whiteSpace: "nowrap", paddingRight: 3 } }, r.label || ""));
    var desc = r.real ? _GRADE_DESC_REAL : _GRADE_DESC;
    order.forEach(function(g) {
      var gs = _GRADE_STYLE[g] || _GRADE_STYLE.Z;
      var full = desc[g] || "";
      // lowerOnly: 「2001~2500円」「2501円~」のどちらの書式でも下限だけ残す（円は落として桁を揃える）
      var txt = o.lowerOnly ? full.replace(/円/g, "").replace(/~.*$/, "~") : full;
      cells.push(React.createElement("span", { key: "c" + ri + "_" + g, title: g + "：" + full,
  ...
  return React.createElement("div", {
    style: Object.assign({ display: "inline-grid", gridTemplateColumns: "auto repeat(9, auto)",
      columnGap: 7, rowGap: 2, alignItems: "center", justifyItems: "start",
      fontSize: 9, color: "#8a8478", lineHeight: 1.3 }, o.style || {})
  }, cells);
}
...
// 「取引」カテゴリタグ日のマーカー（早見表/損益データ表の想定損益欄で共用 2026-07-23）: ノーシグナル→DNFピル / それ以外(有効シグナルなし等)→Z（取引なし）丸バッジ。タグ名はtooltip。無ければnull。
...
function _elCalcChartGrades(signals, alpha, cutLine, exclFn, itemOf) {
  ...
  (signals || []).filter(function(sig) { return _elInclTotal(sig); }).forEach(function(sig) {
    ...
    var _h2tg = _elHoldFinalParts(s, _aSig, _c);  // 想定損切り→想定額(結果損益)、非損切り○/△→_elDynHold2、非損切り×→参考
...
function EntryRecordForm(_ref_erf) {
  ...
  var _useStateAAU = useState(_initAlphaKind),
    ...
    fAlphaKind = _useStateAAUA[0], setFAlphaKind = _useStateAAUA[1];
  // 追加α〇の「根拠」（複数選択可＝指標線支え/底抜け前足浮き/その他＋ユーザー追加。signal.specialReasons[]に保存・旧addAlphaReason(文字列)も読む）2026-06-22→22d複数選択化。
  // 初期化: 保存配列をマスター内(プリセット選択)とマスター外(その他テキスト)に分ける。
  var _reasonMasterInit = (data && data.custom && Array.isArray(data.custom.specialReasons)) ? data.custom.specialReasons : _DEF_SPECIAL_REASONS;
  var _savedReasons = Array.isArray(initSig.specialReasons) ? initSig.specialReasons.filter(function(x) { return x; }) : [];
  ...
  // 並び=custom.specialReasonsの配列順（未設定時は既定配列を実体化して保存）・fRsnInput={val}=追加/{old,val}=改名・値の正本は_rsnValRef（確定時_fiFlushAll）。
  ...
  // 浮き足専用α化 2026-07-14g（第2弾-B）: 浮き足〇のとき基本α/応用αを使わず浮き足加算%のみ。浮き足基本/応用の2段（応用は根拠つき）＝基本α/応用αと同じ使い分け。
  var _uUkiSp = useState(initSig.ukiSpecial === true), fUkiSpecial = _uUkiSp[0], setFUkiSpecial = _uUkiSp[1];   // 浮き足応用フラグ（大きめ%・根拠つき）
  var _uUkiRsn = useState(Array.isArray(initSig.ukiReasons) ? initSig.ukiReasons.slice() : []), fUkiReasons = _uUkiRsn[0], setFUkiReasons = _uUkiRsn[1];   // 浮き足応用の根拠
    ...
    React.createElement("div", {
      ...
      React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 7, marginBottom: 8 } },
        ...
        (function() {
          ...
          return React.createElement("div", {
            style: { display: "inline-flex", flexDirection: "column", alignItems: "flex-start", gap: 7, padding: "6px 10px", borderRadius: 6, background: "#F0FDF4", border: "1px solid #86EFAC", fontSize: 12 }
          },
            ...
            (_ukiOn && fUkiSpecial) ? React.createElement("div", { style: { display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6 } },   // 2段目: 浮き足応用の根拠（浮き応用時のみ・トグルの真下）2026-07-21。マスターはcustom.specialReasonsを共有。
              React.createElement("span", { style: { fontSize: 10.5, fontWeight: 700, color: "#15803D" } }, "浮き足応用の根拠"),
              (function() {
                var _rm = (data && data.custom && Array.isArray(data.custom.specialReasons)) ? data.custom.specialReasons : _DEF_SPECIAL_REASONS;
                return _rm.map(function(_rn) {
                  var _on = (fUkiReasons || []).indexOf(_rn) >= 0;
                  return React.createElement("button", { key: _rn, type: "button", onClick: function() { setFUkiReasons(function(p) { var _a = (p || []).slice(); var _i = _a.indexOf(_rn); if (_i >= 0) _a.splice(_i, 1); else _a.push(_rn); return _a; }); },
                    style: { padding: "2px 9px", fontSize: 10.5, fontWeight: _on ? 800 : 600, borderRadius: 12, cursor: "pointer", border: _on ? "1.5px solid #15803D" : "1px solid #ddd", background: _on ? "#EAF3DE" : "#fff", color: _on ? "#15803D" : "#999", whiteSpace: "nowrap" } }, _rn);
                });
              })()) : null,
            _ukiOn ? React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },   // 3段目: 率（推奨%＋手入力）＋📊詳細表 2026-07-21
              ...
              React.createElement("button", { type: "button", onClick: function() { _setUkiTblOpen(true); }, title: "浮き足加算率の詳細データ表（全銘柄・前日まで＝推奨%と同母数）", style: { fontSize: 10, fontWeight: 700, color: "#15803D", background: "#fff", border: "1px solid #86EFAC", borderRadius: 5, padding: "2px 8px", cursor: "pointer", whiteSpace: "nowrap" } }, "📊 詳細表")
            ) : null,
            _ukiOn ? React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },   // 4段目: 底抜け前足−底抜けライン＋浮き○円→＋×円 2026-07-21（底抜け前足はグレー＝底抜けラインに統一）
              React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 3 } },
                React.createElement("span", { style: { fontSize: 11, color: "#64748B", fontWeight: 700 } }, "底抜け前足"),
                React.createElement("div", { style: { display: "flex", alignItems: "stretch", border: "1px solid #CBD5E1", borderRadius: 4, overflow: "hidden" } },
                  React.createElement("input", {
                    ...
                    placeholder: "—",
                    style: { padding: "3px 6px", fontSize: 13, fontWeight: 800, color: "#334155", border: "none", outline: "none", background: "#fff", width: 60, textAlign: "right", boxSizing: "border-box" }
                  }),
                  _stepBtn(function() { _stepPrev(1); }, function() { _stepPrev(-1); })
                )
              ),
              React.createElement("span", { style: { fontSize: 13, color: "#94A3B8", fontWeight: 700 } }, "−"),
              _lvPriceBox("lv_uki", "底抜けライン"),
              React.createElement("span", { style: { fontSize: 12, fontWeight: 800, color: "#15803D", whiteSpace: "nowrap", background: "#EAF3DE", borderRadius: 5, padding: "3px 9px" } }, "浮き " + _fUkiDiffShown + "円"),
              React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: "#15803D", whiteSpace: "nowrap" } }, "→ ＋" + _fUkiAdd + "円")
            ) : null
          );
        })(),
      ...
      (function() {
        ...
        return React.createElement(React.Fragment, null,
          (fUkiUsed !== "○" && fAlphaKind === "base") ? React.createElement("div", { title: "推奨基本α値＝選んだ母数（既定=銘柄全体／各シグナル）の推奨基本α。母数プルダウンで切替、詳細表で閲覧。", style: { fontSize: 13, color: "#334155", lineHeight: 1.3, margin: "0 0 4px" } },
      ...
      (fUkiUsed !== "○" && fAlphaKind === "special") ? (function() {
        ...
        var _addR = function(nm) {
          ...
          save(function(prev) {
            var cur = (prev.custom && Array.isArray(prev.custom.specialReasons)) ? prev.custom.specialReasons : _DEF_SPECIAL_REASONS.slice();
            if (cur.indexOf(nm) >= 0) return prev;
            return Object.assign({}, prev, { custom: Object.assign({}, prev.custom || {}, { specialReasons: cur.concat([nm]) }) });
          });
        };
        var _delR = function(nm) {
          save(function(prev) {
            var cur = (prev.custom && Array.isArray(prev.custom.specialReasons)) ? prev.custom.specialReasons : _DEF_SPECIAL_REASONS.slice();
            return Object.assign({}, prev, { custom: Object.assign({}, prev.custom || {}, { specialReasons: cur.filter(function(x) { return x !== nm; }) }) });
          });
          if (fAddReasons.indexOf(nm) >= 0) setFAddReasons(fAddReasons.filter(function(x) { return x !== nm; }));
        };
        // 根拠選択肢の改名: マスター(specialReasons)の名前を変更し、過去記録(全charts.signalsのspecialReasons[]・旧addAlphaReason文字列)も一括追従。2026-06-23。
          ...
          save(function(prev) {
            ...
            Object.keys(charts).forEach(function(ck) {
              ...
              var sigs = c.signals.map(function(s) {
                if (!s) return s;
                var ns = s, hit = false;
                if (Array.isArray(s.specialReasons) && s.specialReasons.indexOf(oldNm) >= 0) {
                  var _rl = s.specialReasons.map(function(x) { return x === oldNm ? newNm : x; });
                  ns = Object.assign({}, ns, { specialReasons: _rl.filter(function(x, i) { return x && _rl.indexOf(x) === i; }) }); hit = true;
                }
            ...
            return Object.assign({}, prev, {
              custom: Object.assign({}, prev.custom || {}, { specialReasons: newMaster }),
              charts: newCharts
            });
          });
              ...
              return React.createElement("span", { key: rsn, "data-rsn": rsn,
                ...
                onPointerUp: function() {
                  ...
                  if (d && d.started && d.list) {
                    ...
                    save(function(prev) { return Object.assign({}, prev, { custom: Object.assign({}, prev.custom || {}, { specialReasons: _fin }) }); });
      ...
      (function() {
        // 合計α値バー（案B 2026-07-03→2026-07-13応用α化→2026-07-14f 浮き足を合計α値に戻す）: 内訳（base-levelα＝基本α or 応用α ＋浮き足＋RN）を常設表示。予定EPは水準線＋_fAlpha（合計α＝浮き足込み）。
        return React.createElement("div", {
      ...
      (function() {
        ...
        (function() {
          ...
          var p = _refCutPick;
          return React.createElement("span", { title: "実現H1損益をほぼ維持できる範囲で最小（タイト）の損切り値（この銘柄の前日までの算入記録から）。バッジ＝想定損益（手じまい）基準で同じ選定をした並走値（デュアル評価 2026-07-12）", style: { fontSize: 11, fontWeight: 600, color: "#7F1D1D", whiteSpace: "nowrap" } },
        ...
        return React.createElement(React.Fragment, null,
          React.createElement("div", { style: Object.assign({}, SH_, { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }) },
            ...
            React.createElement("div", {
              ...
              React.createElement("span", { title: "水準線＋合計α値（基本／応用＋浮き足＋RN）＝自動計算。エントリー予定価格の目安。", style: { fontSize: 12, fontWeight: 800, color: "#1E3A8A", fontVariantNumeric: "tabular-nums", minWidth: 36, textAlign: "right" } },
            ...
            React.createElement("div", {
              ...
              React.createElement("span", { title: "予定EP＋損切り値＝ここまで逆行したら撤退する価格の目安（自動計算）。この価格は撤退の引き金であって約定値ではなく、触れた足の終値で撤退する（終値が損なら損切り）。", style: { fontSize: 12, fontWeight: 800, color: "#991B1B", fontVariantNumeric: "tabular-nums", minWidth: 36, textAlign: "right" } },
...
function EntryLogCard(_ref_elc) {
  ...
  // 想定損益(_elHold2AmtNode)＋OS・損益詳細(_elDetailFlowStack)を右パネルに集約。旧planPnl/holdPnl/_dispResult/_dispHPは不要化。
    ...
    React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-start", flexWrap: "wrap" } },
      ...
      React.createElement("div", { style: { flex: "0 1 auto", maxWidth: "100%", background: "#FFFBF5", border: "1px solid #F0E6D2", borderRadius: 6, padding: "6px 10px", display: "flex", flexDirection: "column", gap: 3, overflowX: "auto" } },
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } },
          React.createElement("span", { style: { fontSize: 9, color: "#B08968", fontWeight: 700, flexShrink: 0 } }, "想定損益"),
