...
function EntrySignalSection(_ref_es) {
  ...
  var _esRPnlDisp = function(v, grade, showZ, sub) { return _elRPnlDispW(v, grade, 60, showZ, sub); };   // sub=100株換算の下段（実現損益欄だけ渡す）2026-08-04
  var _esRPnlDispABAll = _elRPnlDispABAll;
  
  var _esTotReal = null, _esTotPlan = null, _esTotMax = null, _esTotHold = null;
  var _esTotRealRaw = null, _esTotRealHasSh = false;   // 2026-08-04 実現損益の合計は実額を主に出す。_esTotRealは100株換算の合計＝下段の参考値。株数入りが1件でもあれば下段を出す。
  ...
  _recsForTot.forEach(function(r) {
    ...
    // pp/mp/hp/_h2tes は _elDynPlanned/_elDynHold/_elHoldFinalParts の動的値＝既に100株換算済み。
    ...
    if (rpN != null) { _esTotReal = (_esTotReal || 0) + rpN; _esTotRealCnt++; _esTotRealRaw = (_esTotRealRaw || 0) + Math.round(rp); if (_sh > 0) _esTotRealHasSh = true; }
    ...
    var _h2tes = _elHoldFinalParts(s, _esAlpha(s), _esCut(s));
  ...
  var _esTotRealGrade = _esTotRealCnt > 0 ? _profitGradeFromPnlReal(_esTotRealRaw != null ? _esTotRealRaw : 0, _esTotRealCnt) : null;   // 実額スケールのグレードには実額合計を渡す 2026-08-04
      ...
      React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5 } },
        ...
        (function() {
          ...
          return React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" } },
            ...
            _renderGradeLegend2Row([["S","25001円~"],["A","20001~25000円"],["B","10001~20000円"],["C","1~10000円"],["D","0円"],["E","-1~-10000円"],["F","-10001~-20000円"],["G-","-20001~-25000円"],["G+","-25001円~"],["Z","取引なし"]])
          );
        })(),
        ...
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" } },
          ...
          _renderGradeLegend2Row([["S","2501円~"],["A","2001~2500円"],["B","1001~2000円"],["C","1~1000円"],["D","0円"],["E","-1~-1000円"],["F","-1001~-2000円"],["G-","-2001~-2500円"],["G+","-2501円~"],["Z","取引なし"]])
        )
      )
    ),
      ...
      : (function() {
          ...
          var totRow = React.createElement("tr", { key: "__estot__", style: { background: "#FFF7ED" } },
            ...
            React.createElement("td", { style: { padding: "1px 3px", textAlign: "center", fontSize: 11, whiteSpace: "nowrap", borderTop: "2px solid #FB923C", borderLeft: "1px solid #f0ede6", borderBottom: "1px solid #f0ede6", background: "#FFFBF0" } },
              React.createElement("div", { style: { fontSize: 8, fontWeight: 700, color: "#9A3412", marginBottom: 1, lineHeight: 1.1 } }, "想定損益"),
            ...
            React.createElement("td", { style: { padding: "1px 3px", textAlign: "center", fontSize: 11, whiteSpace: "nowrap", borderTop: "2px solid #FB923C", borderLeft: "1px solid #f0ede6", borderBottom: "1px solid #f0ede6" } },
              React.createElement("span", { style: { display: "inline-flex", alignItems: "center", whiteSpace: "nowrap" } }, _esLane(null, 26), _esTotRealCnt > 0 ? _esRPnlDisp(_esTotRealRaw, _esTotRealGrade, false, _esTotRealHasSh ? _esTotReal : null) : React.createElement("span", { style: { color: "#ccc" } }, "—"))
            ),
          ...
          tblItems.forEach(function(tblItem) {
            ...
            // 2026-08-04 実現損益は「実額」を主に出す。_profitGradeFromPnlReal は実額スケール（通常の10倍）なので、換算値を渡すと1桁ずれたランクが付いていた。
            var realGrade = entered ? ((realPnl != null) ? _profitGradeFromPnlReal(realPnl, 1) : null) : "Z";
            ...
            dataRows.push(
              React.createElement("tr", { key: rKey,
                ...
                React.createElement("td", { style: { padding: "1px 4px", textAlign: "center", fontSize: 11, whiteSpace: "nowrap", borderBottom: "1px solid #f0ede6", borderRight: isCustomMode ? "1px solid #f0ede6" : "none" } }, _esRPnlDisp(entered ? realPnl : 0, realGrade, !entered, (entered && _sh > 0) ? realPnlN : null)),   // 2026-08-05 採用αチップはα値欄へ移設
...
function WeeklyPnlPanel(_wpp) {
  ...
  var _amtCell = function(v, cnt, ref, refCnt, isReal, allMiss, days, sub) {
    refCnt = refCnt || 0;
    ...
    var _mn = React.createElement("span", { style: { display: "inline-flex", alignItems: "center", whiteSpace: "nowrap" } }, g ? _elGradeBadge18(g) : null, React.createElement("span", { style: { fontWeight: 700, color: _elPnlColor(v) } }, _elPnlFmt(v)), refCnt > 0 ? _elHold2RefSuffix(v, ref, refCnt) : null);
    if (sub == null) return _mn;
    return React.createElement("span", { style: { display: "inline-flex", flexDirection: "column", alignItems: "center", whiteSpace: "nowrap" } },
      _mn, React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 2, whiteSpace: "nowrap" } },
        _elGradeBadge18(_profitGradeFromPnl(sub, 1)),   // 100株換算は通常スケール 2026-08-05
  ...
  var _detailTotRowFor = function(_list) {
    ...
    return React.createElement("tr", { key: "wpp_tot", style: { background: "#FFF7ED" } },
      ...
      React.createElement("td", { style: { padding: "1px 3px", textAlign: "center", fontSize: 11, borderTop: "2px solid #FB923C", whiteSpace: "nowrap", background: "#FFFBF0" } }, _lblTot("想定損益"), _amtCell(_t.hold2, _t.hold2Cnt, _t.hold2Ref, _t.hold2RefCnt, false, _allMiss, _totDays)),
  ...
  // sub=100株換算の下段（実現損益列だけ渡す）2026-08-04
  // 2026-08-05b 下段を [ラベル][バッジ][額] の3列gridへ（日別ページの今週の損益データと同じ並び・同じ縦ぞろい）。
  // 旧は [バッジ][ラベル][額] のinline-flexでバッジも額も揃っていなかった。
  var _wkPnlCell = function(grade, sum, sub) {
    if (!grade || grade === "Z" || sum == null) return React.createElement("span", { style: { color: "#ccc" } }, "—");
    var rows = [{ label: null, grade: grade, amount: sum }];
    if (sub != null) rows.push({ label: "100株", grade: _profitGradeFromPnl(sub, 1), amount: sub });   // 100株換算は通常スケール 2026-08-05
    var cells = [];
    rows.forEach(function(r, i) {
      cells.push(React.createElement("span", { key: "l" + i, style: { fontSize: 9, color: "#94A3B8", fontWeight: 700, lineHeight: 1.15, justifySelf: "end", whiteSpace: "nowrap" } }, r.label || null));
      cells.push(React.createElement("span", { key: "b" + i, style: { display: "inline-flex", alignItems: "center" } }, r.grade ? _wkBadge(r.grade) : null));
      cells.push(React.createElement("span", { key: "a" + i, style: { fontWeight: 700, color: _elPnlColor(r.amount), whiteSpace: "nowrap" } }, _elPnlFmt(r.amount)));
    });
  ...
  // 集約していた_wkTagsも呼び出し元が無くなったので削除した。
  var _sumRow = function(label, labelColor, recs, isTotal, rowKey) {
    ...
    return React.createElement("tr", {
      ...
      _td(_allExcl ? React.createElement("span", { style: { color: "#ccc" } }, "—") : _wkPnlCell(_profitGradeFromPnlReal(_rowDays > 0 ? Math.round(_stM.sumPnlRaw / _rowDays) : _stM.sumPnlRaw, (_ent > 0 && _stM.sumPnlRaw !== 0) ? _ent : 0), _ent > 0 ? _stM.sumPnlRaw : null, (_ent > 0 && _stM.realHasShares) ? _stM.sumPnl : null))
    );
  };
  var _idealEl = _recs.length ? React.createElement("div", { style: { marginTop: 8, marginBottom: 8, padding: "8px 10px", borderRadius: 8, background: "#F0F9FF", border: "1px solid #BAE6FD" } },
    ...
    React.createElement("div", { style: { fontSize: 9, color: "#64748B", marginBottom: 6 } }, "今週(月〜金)の全記録に同じαを当てて、件数フロア（最も件数の多いαの半分以上）かつ到達率50%以上かつ試算損益がプラスのαから 損切り率(EP〜H1)の低さ×0.7＋H1勝率×0.3 の合成スコアが最大のα（薄い高α・約定しにくい高α・赤字αは除外・データ不足時は件数最大を参考表示）。応用α目安＝応用〇局面で採用する独立α値（応用〇の記録から算出）。"),
      ...
      React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", fontSize: 10 } },
        React.createElement("thead", null,
          React.createElement("tr", { style: { background: "#f5f4f0" } },
            ...
            _wkTh(React.createElement("span", { style: { color: "#374151" } }, "到達")), _wkTh(React.createElement("span", { style: { color: "#1E8449" } }, "利確")), _wkTh(React.createElement("span", { style: { color: "#D97706" }, title: "想定損益が±0（トントン）" }, "△")), _wkTh(React.createElement("span", { style: { color: "#DC2626" }, title: "確定損" }, "確損")), _wkTh(React.createElement("span", { style: { color: "#7F1D1D" }, title: "損切り" }, "損切")), _wkTh(React.createElement("span", { style: { color: "#6B7280" } }, "未達")), _wkTh(React.createElement("span", { style: { color: "#0284C7" }, title: "不算入＋スルー（集計に算入しない記録）" }, "除外")),
            _wkTh(React.createElement("span", { title: "○が途切れた所（×/△/損切り）で手じまいした最終PnL＝（）外。（）内=△も保有し続けた場合。旧H２結果損益と同一基準" }, "想定損益")), _wkTh(React.createElement("span", { title: "EP損益（○のみ）／H1損益／想定損益を縦積み。最終＝○が続く限り手じまい足まで保有した損益＝想定損益列と同値" }, "詳細損益")), _wkTh("実現損益"))),
